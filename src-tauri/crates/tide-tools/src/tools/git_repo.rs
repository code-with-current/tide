...
use super::arg_str;
use super::arg_u64;
use super::git::{clip_output, commits_touching_paths, diff_patch, log_line, short_oid, show_commit_patch};
...
const OPS: &[&str] = &["info", "branches", "files", "read", "log", "show", "blame", "search"];
...
impl RepoArgs {
    fn from_json(args: &Value) -> Self {
        Self {
            ...
            ref_: args.get("ref").and_then(|v| v.as_str()).unwrap_or("HEAD").to_string(),
            commit: args.get("commit").and_then(|v| v.as_str()).map(String::from),
...
fn validate_ref(ref_: &str) -> Option<String> {
    ...
    if ref_.is_empty() || ref_.starts_with('-') || ref_.contains("..") || ref_.contains(' ') || !re.is_match(ref_) {
        return None;
    }
    Some(ref_.to_string())
}
...
fn validate_path_arg(p: Option<&str>) -> Option<String> {
    let Some(p) = p else { return Some(String::new()) };
...
fn parse_remote(repo: &str) -> Option<RemoteTarget> {
    ...
    let https = HTTPS.get_or_init(|| Regex::new(r"^https://([^/]+)/([^/]+)/([^/]+?)(?:\.git)?/?$").unwrap());
    let ssh = SSH.get_or_init(|| Regex::new(r"^git@([^:]+):([^/]+)/([^/]+?)(?:\.git)?$").unwrap());
    let ssh_url = SSH_URL.get_or_init(|| Regex::new(r"^ssh://git@([^/]+)/([^/]+)/([^/]+?)(?:\.git)?$").unwrap());
    let m = https.captures(repo).or_else(|| ssh.captures(repo)).or_else(|| ssh_url.captures(repo))?;
    Some(RemoteTarget {
...
fn raw_get(url: &str) -> Result<String, RestError> {
    let resp = http::get(url, &[("User-Agent", "Tide/1.0 (coding agent)")], REST_TIMEOUT).map_err(map_http_err)?;
    if !resp.is_ok() {
        return Err(RestError(format!("HTTP {}", resp.status)));
    }
    Ok(resp.body)
...
fn rest_resolve_sha(t: &RemoteTarget, ref_: &str, bases: &RestBases) -> Result<String, RestError> {
    ...
    let sha = if t.host == "github.com" {
        let j = api_get(
            &format!("{}/repos/{}/{}/commits/{}", bases.github_api, t.owner, t.repo, encode_uri_component(ref_)),
            &[],
        )?;
        ...
        let j = api_get(
            &format!("{}/projects/{}/repository/commits/{}", bases.gitlab_api, proj, encode_uri_component(ref_)),
            &[],
        )?;
...
fn rest_op(op: &str, t: &RemoteTarget, a: &RepoArgs, bases: &RestBases) -> Result<ToolOutcome, RestError> {
    ...
    match op {
        "info" => {
            let j = if github {
                api_get(&format!("{}/repos/{}/{}", bases.github_api, t.owner, t.repo), &[])?
            } else {
            ...
            let lines = if github {
                vec![
                    ...
                    format!("default_branch: {}", j["default_branch"].as_str().unwrap_or("")),
                    format!("description: {}", j["description"].as_str().unwrap_or("")),
                    format!("stars: {} · forks: {}", j["stargazers_count"].as_u64().unwrap_or(0), j["forks_count"].as_u64().unwrap_or(0)),
                ...
                vec![
                    ...
                    format!("default_branch: {}", j["default_branch"].as_str().unwrap_or("")),
                    format!("description: {}", j["description"].as_str().unwrap_or("")),
                    format!("stars: {} · forks: {}", j["star_count"].as_u64().unwrap_or(0), j["forks_count"].as_u64().unwrap_or(0)),
                    format!("last_activity_at: {}", j["last_activity_at"].as_str().unwrap_or("")),
                ]
            };
        ...
        "branches" => {
            if github {
                ...
                let list = j
                    ...
                    .unwrap_or_default();
                Ok(ToolOutcome::executed(clip_output(&list, MAX_OUTPUT, "branch list")).with_meta("rest · github"))
            } else {
                let j = api_get(&format!("{gl}/branches?per_page=100"), &[])?;
                let list = j
                    .as_array()
                    .map(|arr| arr.iter().filter_map(|b| b["name"].as_str()).collect::<Vec<_>>().join("\n"))
                    .unwrap_or_default();
                Ok(ToolOutcome::executed(clip_output(&list, MAX_OUTPUT, "branch list")).with_meta("rest · gitlab"))
            }
        }
        "files" => {
            ...
            if github {
                ...
                let filter = a.file.as_deref().map(|f| format!("{}/", f.trim_end_matches('/'))).unwrap_or_default();
                let names: Vec<&str> = j["tree"]
                    .as_array()
                    .map(|arr| {
                        arr.iter()
                            .filter(|e| e["type"].as_str() == Some("blob"))
                            .filter(|e| filter.is_empty() || e["path"].as_str().is_some_and(|p| p.starts_with(&filter)))
                ...
                let out = if names.is_empty() { "(no files)".to_string() } else { format!("{}{trunc}", names.join("\n")) };
                Ok(ToolOutcome::executed(clip_output(&out, MAX_OUTPUT, "file list")).with_meta(format!("rest · {} files", names.len())))
            } else {
                let j = api_get(
                    &format!("{gl}/tree?ref={}&recursive=true&per_page=100", encode_uri_component(&sha)),
                    &[],
                )?;
                let filter = a.file.as_deref().map(|f| format!("{}/", f.trim_end_matches('/'))).unwrap_or_default();
                let names: Vec<&str> = j
        ...
        "read" => {
            ...
            let enc_path = clean.split('/').map(encode_uri_component).collect::<Vec<_>>().join("/");
            let text = if github {
                raw_get(&format!(
                    "{}/{}/{}/{}/{}",
                    bases.github_raw, t.owner, t.repo, encode_uri_component(&ref_), enc_path
                ))?
            } else {
                raw_get(&format!(
                    "{}/{}/{}/raw/{}/{}",
                    bases.gitlab_raw, t.owner, t.repo, encode_uri_component(&ref_), enc_path
                ))?
            };
            Ok(ToolOutcome::executed(clip_output(&text, MAX_OUTPUT, "file contents"))
        ...
        "log" => {
            ...
            if github {
                    ...
                    .map(|arr| {
                        arr.iter()
                            .map(|c| {
                                format!(
                                    "{} {} {}\n  {}",
                                    &c["sha"].as_str().unwrap_or("")[..10.min(c["sha"].as_str().unwrap_or("").len())],
                                    c["commit"]["author"]["date"].as_str().unwrap_or(""),
                                    c["commit"]["author"]["name"].as_str().unwrap_or(""),
                                    c["commit"]["message"].as_str().unwrap_or("").split('\n').next().unwrap_or("")
                                )
                            })
                            .collect::<Vec<_>>()
                            .join("\n")
                ...
                Ok(ToolOutcome::executed(clip_output(&(if out.is_empty() { "(no commits)".to_string() } else { out }), MAX_OUTPUT, "log"))
                    ...
                    .map(|arr| {
                        arr.iter()
                            .map(|c| {
                                format!(
                                    "{} {} {}\n  {}",
                                    &c["id"].as_str().unwrap_or("")[..10.min(c["id"].as_str().unwrap_or("").len())],
        ...
        "show" => {
            ...
            let Some(commit) = a.commit.clone() else {
                return Err(RestError("show requires a commit or base..head range".into()));
            };
            if github {
                let j = api_get(&format!("{gh}/commits/{}", encode_uri_component(&commit)), &[])?;
                let files = j["files"]
                    .as_array()
                    .map(|arr| {
                        arr.iter()
                            .map(|f| {
                                ...
                                let indented = patch.lines().map(|l| format!("  {l}")).collect::<Vec<_>>().join("\n");
                                format!(
                                    "{:<10} {} {}\n{}",
                ...
                let body = if files.is_empty() { "(no files)".to_string() } else { files };
                Ok(ToolOutcome::executed(clip_output(&format!("{head}\n{body}"), MAX_OUTPUT, "patch")).with_meta("rest · github"))
            } else {
                let j = api_get(&format!("{gl}/commits/{}/diff", encode_uri_component(&commit)), &[])?;
                let out = j
                    .as_array()
                    .map(|arr| {
                        arr.iter()
                            .map(|d| format!("{}\n{}", d["new_path"].as_str().unwrap_or(""), d["diff"].as_str().unwrap_or("")))
                            .collect::<Vec<_>>()
                            .join("\n\n")
                    })
                    .unwrap_or_default();
                Ok(ToolOutcome::executed(clip_output(&(if out.is_empty() { "(empty diff)".to_string() } else { out }), MAX_OUTPUT, "patch"))
        ...
        "blame" => {
            ...
            if github {
                return Err(RestError("github blame needs GraphQL+token — falling back to git".into()));
            }
            let sha = rest_resolve_sha(t, &ref_, bases)?;
            let j = api_get(
                &format!("{gl}/blame?ref={}&filepath={}", encode_uri_component(&sha), encode_uri_component(&clean)),
                &[],
            )?;
            let out = j
                .as_array()
                .map(|arr| {
                    arr.iter()
                        .map(|b| {
                            ...
                            let first = b["lines"].as_array().and_then(|l| l.first()).and_then(|v| v.as_u64()).unwrap_or(0);
                            let last = b["lines"].as_array().and_then(|l| l.last()).and_then(|v| v.as_u64()).unwrap_or(0);
                            format!("{} ({author}) lines {first}-{last}", &id[..10.min(id.len())])
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
                })
                .unwrap_or_default();
            Ok(ToolOutcome::executed(clip_output(&(if out.is_empty() { "(no blame data)".to_string() } else { out }), MAX_OUTPUT, "blame"))
                .with_meta("rest · gitlab"))
        }
        "search" => {
            ...
            if !github {
                return Err(RestError("gitlab code search unavailable via REST — falling back to git".into()));
            }
            let q = encode_uri_component(&format!("{query} repo:{}/{}", t.owner, t.repo));
            let j = api_get(&format!("{}/search/code?q={q}&per_page=30", bases.github_api), &[])?;
            let out = j["items"]
                .as_array()
                .map(|arr| arr.iter().filter_map(|i| i["path"].as_str()).collect::<Vec<_>>().join("\n"))
                .unwrap_or_default();
            Ok(ToolOutcome::executed(clip_output(&(if out.is_empty() { "(no matches)".to_string() } else { out }), MAX_OUTPUT, "results"))
                .with_meta(format!("rest · {} matches", j["total_count"].as_u64().unwrap_or(0))))
        }
...
fn fetch_into_bare(url: &str, dir: &Path) -> Result<(), String> {
    let repo = Repository::init_opts(
        dir,
        RepositoryInitOptions::new().bare(true).mkdir(true),
    )
    ...
    remote.fetch(refspecs, Some(&mut fo), None).map_err(|e| e.to_string())?;
    ...
    let head = {
        let conn = remote
            ...
            .map_err(|e| e.to_string())?;
        conn.default_branch().ok().and_then(|b| b.as_str().ok().map(String::from))
    };
...
fn clone_op(op: &str, source: &RepoSource, a: &RepoArgs, cache_root_override: Option<&Path>) -> ToolOutcome {
    let (repo, is_local) = match source {
        ...
        RepoSource::Remote(url) => (
            clone_dir_for(url, cache_root_override).and_then(|dir| Repository::open_bare(&dir).map_err(|e| e.to_string())),
            false,
        ),
    };
    let repo = match repo {
    ...
    let git_meta = if is_local { "git · local" } else { "git · clone" };
    ...
    let Some(vfile) = validate_path_arg(a.file.as_deref()) else {
        return ToolOutcome::failed(format!("Invalid path: {}", a.file.clone().unwrap_or_default()));
    };
    let limit = a.limit.unwrap_or(30).clamp(1, 200);

    let resolve_commit = |r: &str| -> Option<git2::Commit<'_>> {
        repo.revparse_single(r).ok().and_then(|o| o.peel_to_commit().ok())
    };
    ...
    match op {
        ...
        "branches" => {
            ...
            if let Ok(refs) = repo.references() {
                for r in refs.flatten() {
                    let Some(name) = r.name().ok().filter(|n| !n.is_empty()) else { continue };
                    let short = name
            ...
            lines.sort();
            let out = if lines.is_empty() { "(no output)".to_string() } else { lines.join("\n") };
        ...
        "files" => {
            ...
            let filter = if vfile.is_empty() { String::new() } else { format!("{}/", vfile.trim_end_matches('/')) };
            ...
            names.sort();
            let out = if names.is_empty() { "(no output)".to_string() } else { names.join("\n") };
        ...
        "read" => {
            ...
            match repo.revparse_single(&format!("{vref}:{vfile}")) {
                Ok(obj) => match obj.peel_to_blob() {
                    ...
                    Err(_) => ToolOutcome::failed(format!("git show failed: {vfile} is not a file")),
                },
        ...
        "log" => {
            ...
            let commits = if vfile.is_empty() {
                ...
                commits_touching_paths(&repo, start.id(), std::slice::from_ref(&vfile), limit as usize).map_err(|e| e.to_string())
            };
            ...
            let out = commits
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let out = if out.is_empty() { "(no output)".to_string() } else { out };
        ...
        "show" => {
            ...
            if target.contains("..") {
                ...
                let [base, head] = parts[..] else { unreachable!() };
                ...
                match diff {
                    Ok(diff) => {
                        let out = format!("{}\n{}\n\n{}", log_line(&head), log_line(&base), diff_patch(&diff, MAX_OUTPUT));
                        ToolOutcome::executed(clip_output(&out, MAX_OUTPUT, "show")).with_meta(git_meta)
                    }
                    Err(e) => ToolOutcome::failed(format!("git show failed: {e}")),
                }
            } else {
                match resolve_commit(&target) {
                    Some(commit) => match show_commit_patch(&repo, &commit, MAX_OUTPUT) {
                        Ok(out) => ToolOutcome::executed(clip_output(&out, MAX_OUTPUT, "show")).with_meta(git_meta),
                        Err(e) => ToolOutcome::failed(format!("git show failed: {e}")),
                    },
        ...
        "blame" => {
            ...
            match blame_at(&repo, commit.id(), &vfile) {
                Ok(out) => ToolOutcome::executed(clip_output(&out, MAX_OUTPUT, "blame")).with_meta(git_meta),
...
fn collect_blobs<'repo>(
    ...
    for entry in tree.iter() {
        let Some(name) = entry.name().ok() else { continue };
...
fn blame_at(source: &Repository, ref_oid: git2::Oid, rel_path: &str) -> Result<String, String> {
    ...
    let entry = tree.get_path(Path::new(rel_path)).map_err(|e| e.to_string())?;
    ...
    std::fs::write(alt_dir.join("alternates"), source_objects.display().to_string()).map_err(|e| e.to_string())?;
    ...
    for (i, raw) in lines.into_iter().enumerate() {
        ...
        let (sha, author, date) = hunk
            .map(|h| {
                ...
                (
                    ...
                    sig.as_ref().and_then(|s| s.name().ok().map(String::from)).unwrap_or_else(|| "?".into()),
                    sig
                        .as_ref()
        ...
        out.push(format!("{} ({author} {date}) {text}", &sha[..10.min(sha.len())]));
    }
    if content.is_empty() {
            ...
            Err(e) => {
                ...
                return match r.status {
                    crate::OutcomeStatus::Executed => {
                        ...
                        r.with_meta(format!("{meta} · rest fallback ({reason})").trim().to_string())
                    }
...
mod tests {
    ...
    fn seed_repo() -> (tempfile::TempDir, Repository) {
        ...
        repo.tag("v1", head.as_object(), &sig, "tag v1", false).unwrap();
        drop(head);
        (tmp, repo)
    }
    ...
    fn local_repo_outside_workspace_is_rejected() {
        ...
        let out = run("info", &outside.path().display().to_string(), json!({}), Some(ws.path()));
    ...
    fn invalid_op_and_arg_validation() {
        ...
        assert_eq!(out.status, crate::OutcomeStatus::Failed);
        assert!(out.output.starts_with("Invalid op 'clone'. Valid: "), "{}", out.output);
        assert!(out.output.contains("info, branches, files, read, log, show, blame, search"));
        ...
        for bad_ref in ["-x", "a..b", "has space", "semi;colon"] {
            ...
            assert!(out.output.starts_with("Invalid ref"), "{bad_ref}: {}", out.output);
        }
        // Path validation.
        for bad_file in ["-flag", "../escape", "a/../.."] {
            ...
            assert!(out.output.starts_with("Invalid path"), "{bad_file}: {}", out.output);
        }
        // read without a file.
    ...
    fn unrecognized_remote_url_shape() {
        let out = run("info", "https://only-owner", json!({}), Some(Path::new("/")));
        assert_eq!(out.status, crate::OutcomeStatus::Failed);
    ...
    fn local_ops_info_branches_files_read() {
        ...
        assert!(out.output.starts_with(&head.id().to_string()), "{}", out.output);
        assert!(out.output.contains("Tester <tester@example.com>"), "{}", out.output);
        ...
        assert!(out.output.contains(&format!("master {}", short_oid(head.id()))), "{}", out.output);
        ...
        let out = run("read", ".", json!({"file": "src/main.rs"}), Some(tmp.path()));
        ...
        let first_sha = repo.head().unwrap().peel_to_commit().unwrap().parent(0).unwrap().id().to_string();
        let out = run("read", ".", json!({"file": "README.md", "ref": first_sha}), Some(tmp.path()));
    ...
    fn local_ops_blame_and_search() {
        ...
        assert_eq!(out.status, crate::OutcomeStatus::Executed);
        assert!(out.output.starts_with("src/main.rs:1:fn main()"), "{}", out.output);

        let out = run("search", ".", json!({"query": "fn +main", "regex": true}), Some(tmp.path()));
        assert!(out.output.contains("src/main.rs:1:"), "{}", out.output);

        let out = run("search", ".", json!({"query": "no-such-token"}), Some(tmp.path()));
        assert_eq!(out.output, "(no matches)");
        assert_eq!(out.meta.as_deref(), Some("git · 0 matches"));

        // Invalid regex fails cleanly.
        let out = run("search", ".", json!({"query": "([", "regex": true}), Some(tmp.path()));
        assert_eq!(out.status, crate::OutcomeStatus::Failed);
    }
    ...
    fn mock_api(routes: Vec<(&'static str, &'static str)>) -> MockApi {
        ...
        std::thread::spawn(move || {
            for stream in listener.incoming().flatten() {
                ...
                let body = routes.iter().find(|(p, _)| path == *p).map(|(_, b)| b.to_string());
                let resp = match body {
        ...
        MockApi { base: format!("http://{addr}"), requests }
    }
    ...
    fn rest_github_info_branches_log_build_correct_urls() {
        let api = mock_api(vec![
            ("/repos/acme/widget", r#"{"full_name":"acme/widget","default_branch":"main","description":"widgets","stargazers_count":42,"forks_count":7,"pushed_at":"2026-08-01"}"#),
            ("/repos/acme/widget/branches", r#"[{"name":"main","protected":true},{"name":"dev"}]"#),
            ("/repos/acme/widget/commits", r#"[{"sha":"aaaaaaaaaabbbbbbbbbbccccccccccccdddddddd","commit":{"author":{"date":"2026-08-01T10:00:00Z","name":"Ann"},"message":"fix: thing\n\nbody"}}]"#),
        ]);
        ...
        assert_eq!(api.requests.lock().unwrap()[1], "/repos/acme/widget/branches?per_page=100");
    ...
    fn rest_github_files_and_read() {
        let api = mock_api(vec![
            ("/repos/acme/widget/commits/main", r#"{"sha":"1111111111222222222233333333334444444444"}"#),
            ("/repos/acme/widget/git/trees/1111111111222222222233333333334444444444", r#"{"truncated":true,"tree":[{"type":"blob","path":"src/a.rs"},{"type":"blob","path":"src/b.rs"},{"type":"tree","path":"src"}]}"#),
        ...
        let out = rest_op("read", &t, &args("read", json!({"ref": "main", "file": "src/a.rs"})), &bases).unwrap();
    ...
    fn rest_github_show_search_and_unsupported_fall_back() {
        let api = mock_api(vec![
            ("/repos/acme/widget/commits/abc", r#"{"sha":"cccc","commit":{"author":{"name":"Bo","date":"2026-08-02"},"message":"feat"},"files":[{"status":"modified","changes":3,"filename":"x.rs","patch":"@@ -1 +1 @@\n-old\n+new"}]}"#),
            ("/search/code", r#"{"total_count":2,"items":[{"path":"a.rs"},{"path":"b.rs"}]}"#),
        ]);
        let bases = mock_bases(&api);
        let t = gh_target();
        ...
        let out = rest_op("search", &t, &args("search", json!({"query": "todo"})), &bases).unwrap();
        assert_eq!(out.output, "a.rs\nb.rs");
        assert_eq!(out.meta.as_deref(), Some("rest · 2 matches"));
        assert!(api.requests.lock().unwrap().last().unwrap().starts_with("/search/code?q=todo%20repo%3Aacme%2Fwidget"));

        // github blame has no REST path — the error names the fallback.
        let err = rest_op("blame", &t, &args("blame", json!({"file": "x.rs"})), &bases).unwrap_err();
        ...
        assert!(err.reason() == "HTTP 404" || err.reason().contains("HTTP 4"), "{}", err.reason());
    }

    #[test]
    fn rest_gitlab_read_and_blame() {
        let api = mock_api(vec![
            ("/api/v4/projects/acme%2Fgl/repository/blame", r#"[{"commit":{"id":"ddddddddddaaaaaaaaaabbbbbbbbbbcccccccccc","author_name":"Cy"},"lines":[1,3]}]"#),
            ("/api/v4/projects/acme%2Fgl/repository/commits/main", r#"{"id":"eeeeeeeeeeaaaaaaaaaabbbbbbbbbbcccccccccc"}"#),
        ...
        let out = rest_op("read", &t, &args("read", json!({"ref": "main", "file": "lib.rs"})), &bases).unwrap();
        assert_eq!(out.output, "pub fn lib() {}\n");

        let out = rest_op("blame", &t, &args("blame", json!({"ref": "main", "file": "lib.rs"})), &bases).unwrap();
        assert!(out.output.starts_with("dddddddddd (Cy) lines 1-3"), "{}", out.output);
        ...
        assert_eq!(reqs[1], "/api/v4/projects/acme%2Fgl/repository/commits/main");
    ...
    fn bare_clone_cache_fetches_and_reuses() {
        ...
        assert!(bare.is_bare());
        let head = bare.head().expect("HEAD set to default branch").peel_to_commit().unwrap();
        ...
        let out = clone_op("read", &RepoSource::Remote(url.clone()), &a, Some(cache.path()));
        assert_eq!(out.status, crate::OutcomeStatus::Executed, "{}", out.output);
        assert!(out.output.contains("second line"), "{}", out.output);
    ...
    fn cache_lru_eviction_keeps_newest_entries() {
        ...
        assert!(!left.contains(&"entry00".to_string()), "oldest must go: {left:?}");
        assert!(!left.contains(&"entry01".to_string()), "second-oldest must go: {left:?}");
        assert!(left.contains(&"entry11".to_string()), "newest must stay: {left:?}");
    }
    ...
    fn validate_ref_accepts_and_rejects() {
        ...
        assert_eq!(validate_ref("refs/heads/x-y_z.1").as_deref(), Some("refs/heads/x-y_z.1"));
    ...
    fn validate_path_arg_semantics() {
        ...
        assert_eq!(validate_path_arg(Some("/leading/slash")).as_deref(), Some("leading/slash"));
        assert_eq!(validate_path_arg(Some("..")).as_deref(), None);
    ...
    fn remote_parsing_shapes() {
        ...
        assert_eq!((t.host.as_str(), t.owner.as_str(), t.repo.as_str()), ("github.com", "O", "R"));
