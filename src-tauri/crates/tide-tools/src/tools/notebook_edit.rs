...
fn to_string_indent_1(v: &serde_json::Value) -> String {
    ...
    v.serialize(&mut ser).expect("Value serialization is infallible");
...
impl Tool for NotebookEditTool {
    ...
    fn execute(&self, ctx: &ToolContext, args: serde_json::Value) -> Result<ToolOutcome, ToolError> {
        ...
        let mode = if mode.is_empty() { "replace".to_string() } else { mode };
        let cell_type = {
            let ct = arg_str(&args, "cell_type");
            if ct.is_empty() { "code".to_string() } else { ct }
        };
        let source = args.get("source").and_then(|v| v.as_str());
        let cell_index = args.get("cell_index").and_then(|v| v.as_i64()).unwrap_or(-1);
...
mod tests {
    ...
    fn round_trip_replace_keeps_shape_and_splits_source() {
        ...
        assert_eq!(out.output, "Edited n.ipynb: replaced cell 0. Notebook now has 2 cells.");
