...
pub(crate) fn run_directory_tree(rel_path: &str, workspace_root: &std::path::Path) -> ToolOutcome {
...
fn build_tree(
    ...
    for (name, is_dir) in entries {
        ...
        *entry_count += 1;
        ...
        nodes.push(TreeNode { name, is_dir, children });
    }
    nodes
}
...
impl Tool for DirectoryTreeTool {
    ...
    fn execute(&self, ctx: &ToolContext, args: serde_json::Value) -> Result<ToolOutcome, ToolError> {
        Ok(run_directory_tree(&arg_str(&args, "path"), &ctx.workspace_root))
    }
}
