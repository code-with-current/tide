...
pub(crate) fn run_multi_edit(
    ...
    for (i, op) in edits.iter().enumerate() {
        if op.old_string.is_empty() {
            return ToolOutcome::failed(format!(
                "Edit {i}: missing old_string. File unchanged."
            ));
        }
        let mut occurrences: Vec<usize> = Vec::new();
        let mut from = 0;
        while let Some(rel) = current[from..].find(&op.old_string) {
            let idx = from + rel;
            occurrences.push(current[..idx].split('\n').count());
            from = idx + 1;
        }
    ...
    .with_meta(format!("{} edits · +{additions} \u{2212}{deletions}", applied.len()))
...
pub(crate) fn build_multi_diff(before: &str, after: &str, path: &str) -> Vec<DiffHunk> {
    ...
    while i < max_len {
        ...
        let header = format!("@@ -{},{} +{},{} @@ {path}", ctx_start + 1, span, ctx_start + 1, span);
                    ...
                    "edits": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                "old_string": {
                                    "type": "string",
                                    "description": "Exact text to find (must be unique at apply time)."
                                },
                                "new_string": {
                                    "type": "string",
...
mod tests {
    ...
    fn applies_ordered_edits_atomically() {
        ...
        let out = run_multi_edit(
            "a.ts",
            &[op("const a = 1;", "const a = 10;"), op("const c = 3;", "const c = 30;")],
            tmp.path(),
        );
        ...
        assert_eq!(
            out.output,
            "Applied 2 edits to a.ts: +2 \u{2212}2 lines."
        );
    ...
    fn one_bad_edit_leaves_file_untouched() {
        ...
        assert_eq!(std::fs::read_to_string(tmp.path().join("x.txt")).unwrap(), original);
    }
    ...
    fn later_edit_matches_text_created_by_earlier_edit() {
        ...
        assert_eq!(out.status, OutcomeStatus::Executed);
        assert_eq!(std::fs::read_to_string(tmp.path().join("c.txt")).unwrap(), "1 3\n");
    }
