...
pub fn resolve_inside_workspace(workspace_root: &Path, target: &str) -> Result<PathBuf, PathEscapeError> {
...
/// escapes.
pub fn assert_resolved_inside(workspace_root: &Path, resolved_abs: &Path) -> Result<(), PathEscapeError> {
...
pub fn resolve_under_skill_root(target: &str) -> Result<PathBuf, PathEscapeError> {
    let Some(home) = home_dir() else {
        return Err(PathEscapeError {
            message: "Resolved path is not under a skill root (~/.claude, ~/.agent, or ~/.zcode): ".to_string() + target,
            requested_path: Some(target.to_string()),
            workspace_root: None,
        });
    };
    ...
    for dir in [".claude", ".agent", ".zcode"] {
        ...
        let inside = if case_insensitive {
            real.to_string_lossy().to_lowercase().starts_with(&format!(
                "{}/",
