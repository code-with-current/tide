...
use tide_store::sessions_v2::{SessionListOptsV2, SessionWindowOptsV2, SessionsV2};
use tide_store::sessions_v2_write::SCHEMA;
...
fn tool_payload(i: usize) -> String {
    serde_json::json!({
        "toolName": "bash",
        "input": { "command": format!("cargo test -p tide-store --lib {}", i % 97) },
...
fn seed() -> TempDb {
    let dir = std::env::temp_dir().join(format!("tide-store-perf-gate-{}", std::process::id()));
    ...
    conn.execute_batch("BEGIN").unwrap();
    {
        ...
        insert_session
            .execute(params![MAIN_SESSION, WORKSPACE, "Perf gate main session", "test-model", "test-provider", t0, t0])
            .unwrap();
        for i in 0..MESSAGE_COUNT {
            ...
            let mut part = |kind: &str, data: String| {
                insert_part
                    .execute(params![format!("p_{i}_{seq}"), id, MAIN_SESSION, seq, kind, data, t])
                    .unwrap();
                seq += 1;
            };
            if is_user {
...
fn perf_gate_list_and_window_at_500_messages() {
    ...
    let page = store.list_sessions(WORKSPACE, SessionListOptsV2::default()).unwrap();
    assert_eq!(page.sessions.len(), 1 + SIBLING_SESSIONS);
    assert_eq!(page.next_cursor, None, "6 sessions fit one default page");
    let window = store
        .session_messages(MAIN_SESSION, SessionWindowOptsV2 { limit: Some(WINDOW_SIZE), ..Default::default() })
        .unwrap();
    assert_eq!(window.messages.len(), WINDOW_SIZE);
    assert!(window.messages.iter().all(|m| (2..=3).contains(&m.parts.len())));
    assert_eq!(window.next_before, Some(message_id(MESSAGE_COUNT - WINDOW_SIZE)));
    let headers = store.list_session_headers(WORKSPACE, "ws_perf").unwrap();
    assert_eq!(headers.len(), 1 + SIBLING_SESSIONS);
    assert_eq!(headers.iter().find(|h| h.id == MAIN_SESSION).unwrap().message_count, MESSAGE_COUNT as i64);
    println!(
    ...
    measure("gate 3: list_session_headers (legacy sidebar)", HEADERS_BUDGET_MS, || {
        let headers = store.list_session_headers(WORKSPACE, "ws_perf").unwrap();
