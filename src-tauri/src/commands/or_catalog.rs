...
pub struct ProviderModelMeta {
    ...
    #[serde(rename = "context_length", default, skip_serializing_if = "Option::is_none")]
...
pub struct MetaPricing {
    ...
    #[serde(rename = "input_cache_read", default, skip_serializing_if = "Option::is_none")]
    pub input_cache_read: Option<String>,
    #[serde(rename = "input_cache_write", default, skip_serializing_if = "Option::is_none")]
...
pub struct MetaReasoning {
    ...
    #[serde(rename = "default_enabled", default, skip_serializing_if = "Option::is_none")]
...
fn parse_iso_ms(s: &str) -> Option<u64> {
    ...
    let mut days = 365 * (y.saturating_sub(1970))
        + (1970..y).filter(|yy| leap(*yy)).count() as u64;
...
pub async fn bootstrap(data_dir: &Path) {
    bootstrap_with(data_dir, OPENROUTER_MODELS_URL).await
}

pub async fn bootstrap_with(data_dir: &Path, url: &str) {
    {
...
fn find_in_catalog(model_id: &str, catalog: &[ProviderModelMeta]) -> Option<ProviderModelMeta> {
    ...
    let hit = catalog
        ...
        .or_else(|| {
            catalog.iter().find(|m| {
                m.id.rsplit('/').next().map(|t| t.eq_ignore_ascii_case(&lower)) == Some(true)
            })
        });
    hit.cloned()
}
...
pub fn normalize_probe_list(raw: &[Value]) -> Vec<ProviderModelMeta> {
    ...
    for item in raw {
        ...
        let str_list =
            |value: Option<&Value>| -> Option<Vec<String>> {
                value
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("tide-or-catalog-{name}-{}", std::process::id()));
    ...
    fn normalize_maps_rich_and_bare_entries_and_sorts() {
        ...
        assert_eq!(
            rich.reasoning.as_ref().and_then(|r| r.supported_efforts.clone()),
            Some(vec!["low".to_owned(), "high".to_owned()])
        );
        assert_eq!(rich.supported_parameters, Some(vec!["tools".to_owned()]), "non-strings dropped");
        assert_eq!(rich.input_modalities, Some(vec!["text".into(), "image".into()]));
    ...
    fn enrich_preserves_provider_id_and_skips_rich_entries() {
        ...
        assert_eq!(enriched[0].context_length, Some(200_000), "metadata copied from tail match");
        assert_eq!(enriched[1].context_length, Some(8_192), "rich entry untouched");
        assert!(enriched[2].context_length.is_none(), "no match stays bare");
        reset_for_tests();
    }

    #[tokio::test]
    async fn bootstrap_reads_fresh_cache_without_network() {
        let _guard = test_state_guard();
        reset_for_tests();
        ...
        bootstrap_with(&dir, "http://127.0.0.1:1/unreachable").await;
    ...
    /// Canned HTTP server (the tide-tools http.rs test pattern): answers
    ...
    #[tokio::test]
    async fn stale_cache_triggers_fetch_and_write() {
        let _guard = test_state_guard();
        reset_for_tests();
        ...
        bootstrap_with(&dir, &format!("{base}/models")).await;
        assert_eq!(catalog().len(), 1);
