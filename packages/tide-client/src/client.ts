...
export class TideRpcError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "TideRpcError";
  }
}
...
/** Browser-safe client for Tide's versioned JSON-over-WebSocket protocol. */
export class TideClient {
  ...
  constructor(options: TideClientOptions) {
  ...
  connect(): Promise<void> {
    ...
    if (this.state === "connecting") {
      return Promise.reject(new Error("Tide client is already connecting"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.addEventListener("message", (event) => {
        ...
        try {
          ...
          failHandshake(new Error("Tide daemon sent invalid JSON"));
          return;
        }
        ...
        if (!handshakeSettled) {
          ...
          failHandshake(new Error("Tide daemon sent an invalid handshake response"));
      ...
      socket.addEventListener("error", () => {
        failHandshake(new Error("Tide daemon connection failed"));
      });
      socket.addEventListener("close", () => {
        if (generation !== this.connectionGeneration) return;
        failHandshake(new Error("Tide daemon disconnected during handshake"));
        this.markDisconnected(new Error("Tide daemon disconnected"));
      });
    });
  }
  ...
  request(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new Error("timed out waiting for Tide daemon"));
  ...
  disconnect(): void {
    this.rejectConnect?.(new Error("Tide client disconnected"));
    ++this.connectionGeneration;
    const socket = this.socket;
    this.socket = undefined;
    this.markDisconnected(new Error("Tide client disconnected"));
  ...
  private requireSocket(): WebSocketLike {
    if (this.state !== "connected" || !this.socket || this.socket.readyState !== OPEN) {
      throw new Error("Tide daemon is disconnected");
    }
    return this.socket;
  }
  ...
  private handleMessage(message: ServerMessage): void {
    if (message.type === "response") {
      ...
      else pending.reject(new TideRpcError(message.outcome.error.message));
      return;
    }
