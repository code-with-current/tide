...
import { TideClient, TideRpcError, daemonUrl, type WebSocketLike } from "./client";
...
async function connect(client: TideClient, sockets: FakeSocket[]): Promise<FakeSocket> {
...
describe("TideClient", () => {
  ...
  test("surfaces daemon errors", async () => {
    ...
    await expect(response).rejects.toBeInstanceOf(TideRpcError);
  });
  ...
  test("disconnect rejects an in-flight handshake and permits reconnecting", async () => {
    ...
    const firstConnection = client.connect();
    client.disconnect();
    await expect(firstConnection).rejects.toThrow("Tide client disconnected");
  ...
  test("disconnected requests reject instead of throwing synchronously", async () => {
    ...
    await expect(request).rejects.toThrow("Tide daemon is disconnected");
  });
  ...
  test("disconnected notifications reject instead of throwing synchronously", async () => {
    ...
    await expect(notification).rejects.toThrow("Tide daemon is disconnected");
  });
...
test("daemonUrl pins the versioned endpoint", () => {
  ...
  expect(daemonUrl("wss://tide.example.test")).toBe("wss://tide.example.test/v1");
});
