...
private final class ComputerUseStatusItem {
    ...
    private func createItemIfNeeded() {
        ...
        item.length = 54
        item.button?.toolTip = "Tide Computer Use"
        self.item = item
    }
    ...
    private func updateMenu() {
        ...
        let menu = NSMenu()
        let title = NSMenuItem(title: "Tide Computer Use", action: nil, keyEquivalent: "")
...
private final class SoftwareCursorPanel {
    static let shared = SoftwareCursorPanel()
...
struct Response: Encodable {
    ...
    var imageUrl: String?
    let summary: String?
    /// The kit tool result for `call` requests — content items plus the
    /// upstream isError flag, consumed by Tide's native Computer Use tools.
    var result: KitToolResult? = nil
...
/// One item of a kit tool result — a text chunk or a base64 PNG capture.
struct KitContentItem: Encodable {
    let type: String
    let text: String?
    let data: String?
    let mimeType: String?
...
/// The `call` tool result verbatim from the Open Computer Use kit.
struct KitToolResult: Encodable {
    let tool: String
    let content: [KitContentItem]
    let isError: Bool
}

/// Lossy JSON value so `call` arguments stay schema-loose the way the kit
/// dispatcher's `[String: Any]` expects (decoding order matters: Bool must
/// precede numbers because JSONDecoder bridges numeric 0/1 to Bool).
...
enum HelperError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .missingPermission(let permission): "\(helperDisplayName) needs \(permission) access. Open Tide Settings > Computer Use to grant it."
        case .unauthorizedClient(let reason): "Computer Use rejected a request that did not come from a trusted Tide app: \(reason)"
        ...
        case .targetBlocked: "Tide does not allow computer control of that app."
...
struct TideComputerUse {
                ...
                while let line = readLine(from: .standardInput) {
                    ...
                    DispatchQueue.main.async {
                        ...
                        case "cursor":
                            ...
                            SoftwareCursorPanel.shared.show(
                                windowID: windowID,
                                processID: processID,
                                windowLayer: windowLayer,
    ...
    static func handle(_ request: Request) async throws -> Response {
        ...
        case "call":
            return try await runKitCall(request)
        default:
            throw HelperError.invalidRequest("Unknown operation: \(request.operation)")
        }
    }

    // ── Open Computer Use kit (Tide's internal Computer Use tools) ────────
    //
    // The model-facing tool surface is the vendored OpenComputerUseKit's
    // nine Codex-style tools (list_apps, get_app_state, click, drag,
    // press_key, type_text, perform_secondary_action, set_value, scroll),
    // executed in-process — no MCP transport. The daemon forwards one
    // `call` request per tool invocation; the service below persists for
    // the whole helper session so element indexes and snapshots stay valid
    // across calls, exactly like the upstream MCP runtime.
    private static let kitService = ComputerUseService()
    private static let kitDispatcher = ComputerUseToolDispatcher(service: kitService)

    // get_app_state captures the screen; the action tools drive the AX
    // system (the kit re-preflights per action); list_apps needs neither.
    private static let kitCaptureTools: Set<String> = ["get_app_state"]
    private static let kitActionTools: Set<String> = [
        "click", "drag", "press_key", "type_text",
        "perform_secondary_action", "set_value", "scroll",
    ]

    private static func runKitCall(_ request: Request) async throws -> Response {
        guard let tool = request.tool, !tool.isEmpty else {
            throw HelperError.invalidRequest("call requires a tool name")
        }
        if kitCaptureTools.contains(tool) {
            guard CGPreflightScreenCaptureAccess() else {
                throw HelperError.missingPermission("Screen Recording")
            }
        }
        if kitActionTools.contains(tool) {
            guard AXIsProcessTrusted() else {
                throw HelperError.missingPermission("Accessibility")
            }
        }
        let arguments = (request.arguments ?? [:]).mapValues(\.value)
        // Tide policy: never drive blocked apps regardless of the kit's own
        // denylist, and fail on an unresolvable app before any capture.
        if let query = arguments["app"] as? String {
            let resolved = try AppDiscovery.resolve(query)
            guard !isBlocked(bundleId: resolved.bundleIdentifier ?? "") else {
                throw HelperError.targetBlocked
            }
        ...
        BridgeProcessRegistration.shared.activate()
        let result = try kitDispatcher.callTool(name: tool, arguments: arguments)
        let content = result.content.map { item -> KitContentItem in
        ...
        var response = Response(
            ...
            imageUrl: nil,
            summary: nil,
            result: KitToolResult(tool: tool, content: content, isError: result.isError)
        )
        // Picture-in-Picture compat: when the call captured a window, expose
        // the target + screenshot the same shape the legacy `use` op used so
        // the daemon preview hook and the app overlay keep working.
        if let imageItem = content.first(where: { $0.type == "image" }),
           let dataBase64 = imageItem.data,
           let pngData = Data(base64Encoded: dataBase64),
           let query = arguments["app"] as? String,
           let snapshot = kitService.cachedSnapshot(for: query) {
            let capture = Capture(
                dataUrl: "data:\(imageItem.mimeType ?? "image/png");base64,\(dataBase64)",
                width: pngWidth(pngData),
                height: pngHeight(pngData)
            )
            let capturedTarget = Target(
                windowId: snapshot.targetWindowID ?? 0,
                bundleId: snapshot.app.bundleIdentifier ?? "",
                teamId: signingTeamId(pid: snapshot.app.pid),
                appName: snapshot.app.name,
                windowTitle: snapshot.windowTitle ?? "Window",
                width: UInt32(max(capture.width, 1)),
                height: UInt32(max(capture.height, 1))
            )
            ComputerUsePreviewStore.publish(target: capturedTarget, capture: capture)
            response.target = capturedTarget
            response.imageUrl = capture.dataUrl
        }
        return response
    }

    private static func pngWidth(_ data: Data) -> Int {
        NSBitmapImageRep(data: data)?.pixelsWide ?? 0
    }

    private static func pngHeight(_ data: Data) -> Int {
        NSBitmapImageRep(data: data)?.pixelsHigh ?? 0
    }
            ...
            do {
                ...
                case "initialize":
                    result = [
                        ...
                        "serverInfo": ["name": "Tide Computer Use", "version": "1.0.0"],
                    ]
                case "tools/list":
    ...
    static func serveMCPBridge() async throws {
        ...
        if let processDirectory = ProcessInfo.processInfo.environment["TIDE_COMPUTER_USE_PROCESS_DIRECTORY"],
    ...
    private static func callMCPTool(name: String, arguments: [String: Any]) async throws -> [String: Any] {
        // The MCP face (the Pi extension transport) rides the same Open
        // Computer Use kit dispatcher as the native `call` op — one input
        // implementation for every surface.
        if name != "list_apps", let query = arguments["app"] as? String {
            let resolved = try AppDiscovery.resolve(query)
            guard !isBlocked(bundleId: resolved.bundleIdentifier ?? "") else {
                throw HelperError.targetBlocked
            }
        }
        let result = try kitDispatcher.callTool(name: name, arguments: arguments)
        let text = result.primaryText ?? ""
        if result.isError {
            return mcpResult(text: text)
        }
        let imageItem = result.content.first { ($0.dictionary["type"] as? String) == "image" }
        if let data = imageItem?.dictionary["data"] as? String,
           let mime = imageItem?.dictionary["mimeType"] as? String {
            let dataUrl = "data:\(mime);base64,\(data)"
            return mcpResult(
                text: text,
                imageURL: dataUrl,
                structured: ["text": text, "screenshot": dataUrl]
            )
        }
        return mcpResult(text: text)
    }
...
private func readExactly(_ count: Int, from input: FileHandle) throws -> Data? {
    ...
    while data.count < count {
        ...
        if chunk.isEmpty {
            ...
            throw HelperError.ipc("the Tide connection closed mid-message")
        }
        data.append(chunk)
    }
    return data
...
private final class UnixListener {
    ...
    init() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("tide-computer-use", isDirectory: true)
...
private func connectedChannel(at path: String) throws -> (FileHandle, pid_t) {
    ...
    do {
        ...
        guard getsockopt(descriptor, SOL_LOCAL, LOCAL_PEERPID, &peerPID, &peerPIDSize) == 0 else {
            throw HelperError.ipc("could not identify the Tide process")
        }
...
private final class WindowCaptureSession {
    ...
    init(
        ...
        outputQueue = DispatchQueue(label: "codes.tide.computer-use.capture.\(window.windowID)")
...
private func capture(
    ...
    // The helper targets macOS 14+ (the ScreenCaptureKit capture API the
    // whole flow below is built on), so there is no pre-14 fallback path.
    let configuration = SCStreamConfiguration()
    configuration.sourceRect = sourceRect
    configuration.width = size.width
    configuration.height = size.height
...
private func post(_ event: CGEvent, to processID: pid_t) {
    ...
    // input to a global event tap. PID-targeted delivery is what lets Tide
