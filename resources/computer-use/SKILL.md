---
name: tide-computer-use
description: Control local Mac apps through Tide Computer Use for tasks that require reading or operating app UI. Prefer purpose-built connectors, APIs, or CLIs when available.
---

## Native Computer Use tools

The primary surface: nine native tools executed by Tide's Computer Use helper
(`list_apps`, `get_app_state`, `click`, `drag`, `press_key`, `type_text`,
`perform_secondary_action`, `set_value`, `scroll`). Prefer these over the
`js`/`sky` bridge below.

Core workflow:

1. `list_apps` first when you don't know the exact app name or bundle id.
2. `get_app_state` for the target app BEFORE any interaction — it returns the
   window screenshot plus the accessibility tree with numbered `element_index`
   values. Call it again after any large UI change; it must be called once per
   turn before acting.
3. Prefer element-targeted actions (`element_index` from the LATEST
   `get_app_state`); never guess indexes across turns. For long text use
   `text_limit: 1000` or `"max"`; for deep UI raise `max_tree_nodes` /
   `max_tree_depth`.
4. Coordinates in `click`/`drag` are screenshot pixel coordinates from the
   most recent `get_app_state` capture. Use them only when the tree exposes no
   safer target.
5. Prefer semantic actions: `set_value` for editable fields,
   `perform_secondary_action` for exposed AX actions, `press_key`
   (Return/Tab/arrows) for keyboard-driven flows. `click` `click_method`
   defaults to `auto`, which uses the non-intrusive SkyLight background path.

Operating rules:

- This is the user's real desktop session — every action runs visibly. Confirm
  externally visible effects (send, delete, purchase, approve, upload) with the
  user first.
- Never inspect password managers, credential prompts, or private apps unless
  the user explicitly asked for that exact task.
- If an action reports a stale `element_index`, re-run `get_app_state` and
  re-resolve; do not retry the stale index.
- Blocked apps (terminal, System Settings, password managers) fail with a
  policy error — report it and stop; do not route around it.

## tide_js_repl + Tide Computer Use

* Use the `js` tool from `tide_js_repl` for all Computer Use actions.
* Do not use other technologies besides `tide_js_repl` for computer interactions, unless specifically requested by the user (e.g. AppleScript, `osascript`, JXA, System Events, CGEvent synthesis).
...
Tide exposes the native Computer Use runtime directly to QuickJS, but initializes `sky` lazily. Do not import `@oai/sky` or call the raw Computer Use helper.

Run this once per fresh `tide_js_repl` session:
...
* It is usually not necessary to pause between performing an action and getting the updated app state. Tide automatically waits up to about one second after a recent action before capturing the new state.

## Reading screenshots

Screenshot URLs are in `screenshot.url`, and Tide returns them as base64-encoded `data:` URLs. To read a screenshot:
...
## Tide target restrictions

Tide Computer Use cannot control Tide itself, Codex, ChatGPT, Sky, password managers, System Settings, login or security prompts, Keychain Access, or terminal apps. Do not try to bypass these restrictions.
