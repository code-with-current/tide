...
public final class ComputerUseService {
    ...
    /// Tide adapter hook: the most recent snapshot captured for `query`
    /// (app name or bundle id), or nil when it has not been captured yet.
    /// Lets the host adapter publish its Picture-in-Picture preview from
    /// the snapshot the service already took instead of recapturing.
    public func cachedSnapshot(for query: String) -> AppSnapshot? {
        snapshotsByApp[query.lowercased()]
    }
