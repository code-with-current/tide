...
Thanks for helping improve Tide. Bug reports, focused fixes, tests, and
...
`website/public/install.sh` (served at `https://tide.codes/install.sh`) is what
...
side beneath one versioned directory — the layout Tide needs to find its
daemon — and the installer is built from
[`resources/windows/tide.iss`](resources/windows/tide.iss), so Inno Setup 6.3
