...
fn worker() -> &'static Sender<Job> {
    WORKER.get_or_init(|| {
        ...
        std::thread::Builder::new()
            .name("tools-http".into())
            .spawn(move || {
                let rt = tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("tools http runtime");
            ...
            .expect("spawn tools-http thread");
        tx
    })
}
...
pub fn get(url: &str, headers: &[(&str, &str)], timeout: Duration) -> Result<HttpReply, HttpError> {
...
mod tests {
    ...
    fn get_returns_status_and_body() {
        let base = mock_server("HTTP/1.1 200 OK\r\nContent-Length: 5\r\n\r\nhello");
        let reply = get(
            &format!("{base}/x"),
            &[("User-Agent", "test")],
