...
//! files (upstream tide `src/lib/prompts/agents/*.md`, HTML-comment
//! frontmatter + body), vendored into `src/prompts/agents/` and
//! include_str!'d from there so the catalog can never drift from what
//! the TS bundle shipped: adding a `.md` there (re-vendored from tide)
//! and listing it in [`SOURCES`] adds an agent here (the TS "add a file
//! + rebuild" flow).
//!
...
const SOURCES: &[(&str, &str)] = &[
    ...
    (
        ...
        include_str!("prompts/agents/codebase-orchestrator.md"),
    ),
    (
        "computer-use",
        include_str!("prompts/agents/computer-use.md"),
    ),
    (
        "commit-writer",
        include_str!("prompts/agents/commit-writer.md"),
    ),
    ...
    ("pr-creator", include_str!("prompts/agents/pr-creator.md")),
    (
        "security-reviewer",
        include_str!("prompts/agents/security-reviewer.md"),
    ),
    ("simplifier", include_str!("prompts/agents/simplifier.md")),
    (
        "web-research",
        include_str!("prompts/agents/web-research.md"),
    ),
];
...
mod tests {
    ...
    fn frontmatter_fields_parse() {
        ...
        assert!(reviewer
            .system_prompt
            .starts_with("You are a code-review specialist"));
        assert!(
            !reviewer.system_prompt.contains("whenToUse"),
    ...
    fn every_agent_is_tool_enabled_with_a_prompt() {
        for a in builtin_agents() {
            ...
            assert!(
                ...
                "{}: live catalog is all multi-step",
                a.name
            );
    ...
    fn risk_tiers_rank_the_tool_subsets() {
        ...
        assert_eq!(
            agent_risk_tier(agent("general-purpose")),
            RiskTier::Destructive,
            "bash in subset"
        );
        assert_eq!(agent_risk_tier(agent("simplifier")), RiskTier::Write);
        assert_eq!(
            agent_risk_tier(agent("commit-writer")),
            RiskTier::Destructive,
            "git tool"
        );
    }
