//! tools — Tide's built-in agent tools plus the permission gate that
//! guards them. Engine-agnostic by design: nothing here imports rig or
//! engine; the orchestrator (app crate) adapts [`ToolSpec`] into the
...
pub use agents::{
    agent_names, agent_risk_tier, builtin_agents, can_dispatch_to, effective_child_tools,
...
pub use tools::{
    ...
    BrowserSetViewportTool, BrowserTypeTool, ClickTool, CompactTool, DirectoryTreeTool,
    DispatchAgentTool, DragTool, EditFileTool, ExitPlanModeTool, GetAppStateTool, GitRepoTool,
    GitTool, GlobTool, GrepTool, InitTool, JobKillTool, JobListTool, JobOutputTool, KillShellTool,
    ListAgentsTool, ListAppsTool, ListDirTool, LoadSkillTool, MemoryTool, MultiEditTool,
    NotebookEditTool, PerformSecondaryActionTool, PressKeyTool, ReadFileTool, ReadMediaFileTool,
    RememberTool, ScrollTool, SendMessageTool, SetValueTool, SlashCommandTool, TodoWriteTool,
    TypeTextTool, WebFetchTool, WebSearchTool, WriteFileTool,
};

/// A tool offered to the model — shape mirrors the engine's `ToolSpec`
/// (field-for-field) and the entries in
/// `engine/fixtures/schemas/tools.json`. Defined locally so this crate
/// stays independent of engine/rig; the orchestrator converts trivially.
...
pub struct ToolContext {
    ...
    pub abort: AbortFlag,
    /// The session's tokio runtime handle, wired by the orchestrator for
    /// tools that must spawn async work (the jobs registry's bounded
    /// reapers). `None` in tests and tool-only binaries — every consumer
    /// falls back to plain threads when it is absent.
...
pub trait Tool: Send + Sync {
    ...
    fn execute(&self, ctx: &ToolContext, args: serde_json::Value)
        -> Result<ToolOutcome, ToolError>;
}

/// Whether one tool's calls may overlap each other in a step. Port of
/// deepseek-harness's `isConcurrencySafe` classification as a per-tool
/// constant: reads with no shared mutable state commute; everything else —
/// shells, git, file writes, the shared todo store, dispatch and follow-up
/// parking — stays exclusive. Read-only risk tier alone does NOT imply
/// safety: `bash_output` reads live shell state and must stay exclusive.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ToolConcurrency {
    /// Runs alone: a barrier before and after within its step.
    Exclusive,
    /// May overlap other Parallel calls in one bounded pool.
    Parallel,
}

/// The per-tool concurrency classification. Everything unlisted — including
/// every unknown tool — is [`ToolConcurrency::Exclusive`].
pub fn concurrency_for(name: &str) -> ToolConcurrency {
    match name {
        ...
        // workspace writes and git collide and fail closed as outcomes.
        "dispatch_agent" => ToolConcurrency::Parallel,
...
mod tests {
    ...
    fn core_registry_has_the_core_tools_with_meta() {
        ...
        for t in &tools {
            ...
            assert!(
                spec.parameters["required"].is_array() || spec.parameters.get("required").is_none()
            );
        ...
        // The native job tools: reads are read-tier, job_kill is write-tier.
        assert_eq!(tools[11].risk_tier(), RiskTier::ReadOnly);
        assert_eq!(tools[12].risk_tier(), RiskTier::ReadOnly);
        assert_eq!(tools[13].risk_tier(), RiskTier::Write);
        assert_eq!(tools[14].risk_tier(), RiskTier::ReadOnly);
        assert_eq!(tools[15].risk_tier(), RiskTier::Write);
        assert_eq!(tools[16].risk_tier(), RiskTier::Destructive);
        assert_eq!(tools[17].risk_tier(), RiskTier::ReadOnly);
        // Web tools: read-tier per TS toolMeta.
        assert_eq!(tools[18].risk_tier(), RiskTier::ReadOnly);
        assert_eq!(tools[19].risk_tier(), RiskTier::ReadOnly);
        // dispatch_agent: read-tier (the target agent's effective
        // tier is gated separately at dispatch time).
        assert_eq!(tools[20].risk_tier(), RiskTier::ReadOnly);
        // Messaging pair: pure registry routing, read-tier.
        assert_eq!(tools[21].risk_tier(), RiskTier::ReadOnly);
        assert_eq!(tools[22].risk_tier(), RiskTier::ReadOnly);
        // Batch: all read-tier per TS toolMeta.
        for t in &tools[23..=27] {
        ...
        for t in &tools[24..=26] {
            assert_eq!(t.risk_tier(), RiskTier::ReadOnly);
        }
        // Computer Use: list_apps and get_app_state observe (ReadOnly);
        // the action tools act on the user's session (Write).
        for (index, expected_tier) in [
            (32, RiskTier::ReadOnly),
            (33, RiskTier::ReadOnly),
            (34, RiskTier::Write),
            (35, RiskTier::Write),
            (36, RiskTier::Write),
            (37, RiskTier::Write),
    ...
    #[test]
    fn concurrency_classification_keeps_stateful_reads_exclusive() {
        for name in [
            "read_file",
            "list_dir",
            "directory_tree",
            ...
            assert_eq!(concurrency_for(name), ToolConcurrency::Parallel, "{name}");
        }
        // Stateful or mutating tools stay exclusive — shells own registry
        // state, git owns repo state, writes touch the fs, todo_write owns
        // the shared store, follow-up owns the turn flow, and the messaging
        // pair runs as fast solo registry operations.
        for name in [
            "bash",
            "bash_output",
            "kill_shell",
            // The job tools read live registry state and mutate jobs —
            // exclusive like the shells they supersede.
            "job_output",
            "job_list",
            "job_kill",
            "git",
            ...
            assert_eq!(concurrency_for(name), ToolConcurrency::Exclusive, "{name}");
        }
        // Read-only risk tier alone does not imply parallel safety:
        // bash_output reads live shell state.
        assert_eq!(permission::risk_tier_for("bash_output"), RiskTier::ReadOnly);
        assert_eq!(concurrency_for("bash_output"), ToolConcurrency::Exclusive);
        // Unknown tools default exclusive.
        assert_eq!(
            concurrency_for("totally_new_tool"),
