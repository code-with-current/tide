...
pub fn primary_arg(tool_name: &str, args: &Value) -> Option<String> {
    ...
    match tool_name {
        ...
        "edit_file" | "multi_edit" | "write_file" | "notebook_edit" | "read_file" | "list_dir"
        | "glob" | "grep" => get_str("path").or_else(|| get_str("pattern")),
        _ => None,
    }
}
...
mod tests {
    ...
    fn primary_arg_resolution() {
        assert_eq!(
            primary_arg("bash", &json!({"command": "ls"})),
            Some("ls".into())
        );
        assert_eq!(
    ...
    fn evaluate_rules_deny_wins_over_allow() {
        ...
        assert_eq!(
            evaluate_rules(&rules, "grep", &json!({"pattern": "x"})),
            None
        );
    }
