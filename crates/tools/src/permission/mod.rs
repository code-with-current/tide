...
pub fn risk_tier_for(tool_name: &str) -> RiskTier {
    match tool_name {
        ...
        // Computer Use: the native tools are individually tiered —
...
/// the Computer Use tools included — are name-keyed like the TS sidecar.
    ...
    pub fn check(&self, mode: AutonomyMode, tool_name: &str, args: &Value) -> Decision {
        if evaluate_rules(&self.rules, tool_name, args) == Some(RuleOutcome::Deny) {
            return Decision::Deny {
                reason: "Denied by permission rule (.agent/settings.json or session).".to_string(),
            };
        }

        let tier = risk_tier_for_call(tool_name, args);
        let allow_by_rule =
            evaluate_rules(&self.rules, tool_name, args) == Some(RuleOutcome::Allow);
...
mod tests {
    ...
    fn mcp_bridged_tools_are_read_tier_in_every_mode() {
        ...
        assert_eq!(
            risk_tier_for("mcp__context7__resolve-library-id"),
            RiskTier::ReadOnly
        );
    ...
    fn mode_tool_matrix_via_gate() {
        ...
        for mode in [
            ...
            assert_eq!(
                g.check(mode, "read_file", &read),
                Decision::Allow,
                "{mode:?}"
            );
            assert_eq!(
                g.check(mode, "grep", &json!({"pattern": "x"})),
                Decision::Allow
            );
        }
        // write tools: blocked-ish in plan (Ask with escalation reason), ask in ask, auto in edit/full.
        assert!(matches!(
            g.check(AutonomyMode::Plan, "write_file", &write),
            Decision::Ask { .. }
        ));
        assert!(matches!(
            g.check(AutonomyMode::Ask, "write_file", &write),
            Decision::Ask { .. }
        ));
        assert_eq!(
            g.check(AutonomyMode::Edit, "edit_file", &write),
            Decision::Allow
        );
        assert_eq!(
            g.check(AutonomyMode::FullAccess, "write_file", &write),
            Decision::Allow
        );
        // bash (destructive): ask everywhere except full.
        assert!(matches!(
            g.check(AutonomyMode::Ask, "bash", &cmd),
            Decision::Ask { .. }
        ));
        assert!(matches!(
            g.check(AutonomyMode::Edit, "bash", &cmd),
            Decision::Ask { .. }
        ));
        assert_eq!(
            g.check(AutonomyMode::FullAccess, "bash", &cmd),
            Decision::Allow
        );
    }
    ...
    fn deny_rule_blocks_even_in_full_mode() {
        ...
        let g = gate(rules);
        let Decision::Deny { reason } = g.check(
            AutonomyMode::FullAccess,
            "bash",
            &json!({"command": "curl --version -X"}),
        ) else {
            panic!("expected deny");
        };
        ...
        assert_eq!(
            g.check(AutonomyMode::FullAccess, "bash", &json!({"command": "ls"})),
            Decision::Allow
        );
    }
    ...
    fn git_tier_refines_per_subcommand() {
        ...
        assert_eq!(
            risk_tier_for_call("git", &json!({"args": ["push"]})),
            Destructive
        );
        ...
        assert_eq!(
            g.check(
                AutonomyMode::Plan,
                "git_repo",
                &json!({"op": "info", "repo": "https://github.com/o/r"})
            ),
            Decision::Allow
        );
    }
    ...
    fn mode_serialization() {
        assert_eq!(
            serde_json::to_value(AutonomyMode::FullAccess).unwrap(),
            json!("full")
        );
        assert_eq!(
            serde_json::to_value(AutonomyMode::Plan).unwrap(),
            json!("plan")
        );
        assert_eq!(
