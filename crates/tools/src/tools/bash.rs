...
//! for silent commands, and turn-abort support (foreground only — design
//! decision 4: a background job is never tied to the spawning turn's
//! abort flag). `background:true` starts a job in the
//! [`JobRegistry`](crate::jobs) instead — the call returns a job id
//! immediately, the settlement wake notifies the model in-session when the
//! job finishes, and output is collected with job_output / the job is
//! stopped early with job_kill (the process half of the job is
//! [`spawn_bash_job`]). The permission
//! gate (destructive tier) runs BEFORE execute in the orchestrator.
...
fn blocked_reason(command: &str) -> Option<&'static str> {
    ...
    if ci.iter().any(|re| re.is_match(command)) {
        return Some(
            "Refused: command matches a blocked pattern (catastrophic / irreversible operation).",
        );
    }
    None
}
...
pub(crate) fn run_bash(
    ...
    if background {
        // Background mode: start a job in the process-global registry and
        ...
        match crate::jobs::global_job_registry().start(crate::jobs::JobStart {
            ...
            Ok(key) => {
                ...
                ToolOutcome::executed(format!(
                    "started background job {id}. Poll output with job_output(job_id: \"{id}\"); stop it with job_kill(job_id: \"{id}\")."
                ))
            ...
            Err(e) => ToolOutcome::failed(format!("Could not start background job: {e}")),
        }
    } else {
...
fn run_foreground(
    ...
    let out_reader = child
        .stdout
        .take()
        .map(|p| spawn_reader(p, MAX_OUTPUT, start));
    ...
    let early_deadline = start
        + Duration::from_millis(if long_runner(trimmed) {
            120_000
        } else {
            60_000
        });
...
fn no_output_yet(out: &Option<StreamReader>, err: &Option<StreamReader>) -> bool {
    let out_ms = out
        .as_ref()
        .map(|r| r.last_output_ms.load(Ordering::SeqCst));
    let err_ms = err
        .as_ref()
        .map(|r| r.last_output_ms.load(Ordering::SeqCst));
        ...
        ToolSpec {
            ...
            parameters: json!({
                ...
                "properties": {
                    ...
                    "background": {
                        "type": "boolean",
                        "description": "If true, run as a background job and return the job id immediately. The job keeps running across turns; you are notified in-session when it finishes, then collect its output with job_output (or stop it early with job_kill). Do not poll.",
                        "default": false
                    }
                },
                "required": ["command"]
...
mod tests {
    ...
    /// Foreground calls never touch the job registry; the background test
    /// uses its own session id (the registry is process-global).
    const S: &str = "bash-tests";

    #[test]
    fn rejects_empty_command() {
        let tmp = tempfile::tempdir().unwrap();
        let out = run_bash("   ", tmp.path(), 1000, false, &no_abort(), S);
    ...
    fn refuses_blocked_patterns() {
        ...
        for cmd in [
            ...
            let out = run_bash(cmd, tmp.path(), 1000, false, &no_abort(), S);
    ...
    fn allows_reasonable_scoped_commands() {
        ...
        let out = run_bash("rm -rf subdir", tmp.path(), 5000, false, &no_abort(), S);
        assert_ne!(out.status, crate::OutcomeStatus::Rejected);
        // 'sudo' as a substring of another word is fine (\b anchors).
        let out = run_bash("echo desudo", tmp.path(), 5000, false, &no_abort(), S);
        assert_ne!(out.status, crate::OutcomeStatus::Rejected);
    }
    ...
    fn executes_safe_command_and_reports_success() {
        let tmp = tempfile::tempdir().unwrap();
        let out = run_bash("echo hello-tide", tmp.path(), 10_000, false, &no_abort(), S);
    ...
    fn reports_failure_with_nonzero_exit() {
        let tmp = tempfile::tempdir().unwrap();
        let out = run_bash("exit 7", tmp.path(), 10_000, false, &no_abort(), S);
        assert_eq!(out.status, crate::OutcomeStatus::Failed);
    ...
    fn timeout_kills_and_reports() {
        let tmp = tempfile::tempdir().unwrap();
        let out = run_bash("sleep 5", tmp.path(), 300, false, &no_abort(), S);
    ...
    fn abort_flag_kills_and_reports() {
        ...
        let handle =
            std::thread::spawn(move || run_bash("sleep 5", &root, 10_000, false, &abort2, S));
    ...
    fn output_line_cap_trims() {
        let tmp = tempfile::tempdir().unwrap();
        let out = run_bash("seq 1 3000", tmp.path(), 10_000, false, &no_abort(), S);
        assert_eq!(out.status, crate::OutcomeStatus::Executed);
    ...
    #[test]
    fn background_returns_job_id_immediately() {
        let tmp = tempfile::tempdir().unwrap();
        let session = "bash-bg-test";
        let out = run_bash(
            "echo bg-echo; sleep 10",
            tmp.path(),
            30_000,
            true,
            &no_abort(),
        ...
        assert_eq!(out.status, crate::OutcomeStatus::Executed);
        assert!(out.output.starts_with("started background job bash-"));
        assert!(out.output.contains("job_output(job_id:"));
        assert!(out.output.contains("job_kill(job_id:"));
        ...
        // The job really runs in the registry: poll its output, then kill
        // it and watch the settlement land as `Stopped`.
    ...
    fn background_blocked_command_still_rejected() {
        let tmp = tempfile::tempdir().unwrap();
        let out = run_bash("sudo rm -rf /", tmp.path(), 1000, true, &no_abort(), S);
    ...
    fn runs_in_workspace_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let out = run_bash("pwd", tmp.path(), 10_000, false, &no_abort(), S);
