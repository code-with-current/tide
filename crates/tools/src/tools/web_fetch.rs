...
pub(crate) fn run_web_fetch(url: &str) -> ToolOutcome {
    ...
    if !reply.is_ok() {
        ...
        return ToolOutcome::failed(
            format!("HTTP {} {}", reply.status, reason)
                .trim_end()
                .to_string(),
        );
    }
...
fn looks_like_html(body: &str) -> bool {
    ...
    RE.get_or_init(|| Regex::new(r"(?i)</?(?:html|body|div|p|a)\b").unwrap())
        .is_match(body)
}
...
pub(crate) fn strip_html(html: &str) -> String {
    ...
    let block_close = BLOCK_CLOSE
        .get_or_init(|| Regex::new(r"(?i)</(?:p|div|li|h[1-6]|tr|br|article|section)>").unwrap());
...
<a href=\"/x\">A link</a>\n<br>After break\n</body></html>",
    ...
    fn caps_response_at_64k_chars() {
        ...
        let small_base = mock_server(Arc::new(|_p: &str| {
            response("200 OK", "text/plain", "tiny")
        }));
