...
fn repair_split_args(
    question: &mut String,
    options: &mut Option<Vec<FollowupOption>>,
    multiple: &mut bool,
) {
...
mod tests {
    ...
    fn more_than_four_options_fail() {
        ...
        assert_eq!(
            out.output,
            "Too many options (5). Max 4 \u{2014} narrow it down."
        );
    }
    ...
    fn mis_split_json_question_gets_repaired() {
        ...
        let balanced =
            "{\"question\": \"Q2\", \"options\": [{\"label\": \"B\"}], \"multiple\": true}";
