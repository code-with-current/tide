...
use git2::{
    BranchType, DiffFormat, DiffOptions, IndexEntry, Repository, Sort, Status, StatusOptions,
};
use serde_json::json;
...
pub(crate) fn diff_patch(diff: &git2::Diff<'_>, max: usize) -> String {
    ...
    let mut cb =
        |_d: git2::DiffDelta, _h: Option<git2::DiffHunk<'_>>, line: git2::DiffLine<'_>| -> bool {
            if out.len() < max {
                match line.origin() {
    ...
    for oid in walk.take(limit) {
        ...
        if commit.parent_count() == 0 {
            ...
            for i in 0..commit.parent_count() {
                ...
                if let Ok(d) = repo.diff_tree_to_tree(
                    parent_tree.as_ref(),
                    this_tree.as_ref(),
                    Some(&mut opts),
                ) {
...
fn cmd_log(repo: &Repository, args: &[String], ws: &Path) -> ToolOutcome {
    ...
    while i < args.len() {
        ...
        if after_dashdash {
            ...
            return ToolOutcome::failed(format!(
                "log: unsupported flag {a} (supported: -n N, --oneline, ref, -- paths)"
            ));
...
pub(crate) fn show_commit_patch(
    repo: &Repository,
...
fn cmd_branch(repo: &Repository, flags: &[String]) -> ToolOutcome {
    ...
    let unexpected = flags.iter().find(|f| {
        !matches!(
            f.as_str(),
            "-a" | "--all" | "-r" | "--remotes" | "-v" | "--list"
        )
    });
        ...
        if let Ok(branches) = repo.branches(Some(ty)) {
            for b in branches.flatten() {
                ...
                if let Some(name) = branch.name().ok().flatten() {
                    ...
                    let marker = if ty == BranchType::Local
                        && Some(name.trim_start_matches(prefix)) == current.as_deref()
                    {
                        "*"
                    } else {
                        " "
    ...
    ToolOutcome::executed(if out.is_empty() {
...
fn cmd_add(repo: &Repository, args: &[String], ws: &Path) -> ToolOutcome {
    if args
        .iter()
        .any(|a| a == "." || a == "-A" || a == "--all" || a == ".." || a.starts_with("../"))
    {
    ...
    if let Some(f) = unexpected {
        return ToolOutcome::failed(format!(
            "add: unsupported flag {f} — stage specific files by name"
        ));
    }
    if args.is_empty() {
...
fn env_identity_pair() -> Option<(String, String)> {
    let keys = ["GIT_COMMITTER_NAME", "GIT_AUTHOR_NAME"];
    for k in keys {
        if let Ok(v) = std::env::var(k) {
            if !v.trim().is_empty() {
                let peer = if k.ends_with("NAME") {
                    k.replace("NAME", "EMAIL")
                } else {
                    k.replace("EMAIL", "NAME")
                };
...
/// The identity a commit here would use: repo config first, then the
/// GIT_COMMITTER/GIT_AUTHOR env fallback the TS tool inherited.
fn identity_pair(repo: &Repository) -> Option<(String, String)> {
...
/// Tide's commit-attribution decision from the data-dir config. Read per
/// commit (not cached) so a settings change applies to the next commit
/// without a process restart; absent/unreadable config → no attribution,
/// exactly like the panel path's read failure.
fn tide_attribution() -> Option<store::config::CommitAttribution> {
    store::config::current_attribution()
}
...
fn cmd_commit(repo: &Repository, args: &[String]) -> ToolOutcome {
    ...
    let Some((user_name, user_email)) = identity_pair(repo) else {
        return ToolOutcome::failed(
            "commit: no user.name/user.email configured (git config) and no GIT_AUTHOR_NAME/GIT_AUTHOR_EMAIL set",
        );
    };
    let committer = match git2::Signature::now(&user_name, &user_email) {
        Ok(s) => s,
        Err(e) => return ToolOutcome::failed(format!("commit: {e}")),
    };
    // Same attribution contract as the panel's gitCommit: Co-author mode
    // trails Tide; Author mode makes Tide the author and trails the user's
    // identity.
    let attribution = tide_attribution();
    let (author, message) = match attribution.as_ref() {
        Some(attribution) => {
            ...
            match attribution.author_override() {
                Some((name, email)) => match git2::Signature::now(name, email) {
                    Ok(author) => (author, message),
                    Err(e) => return ToolOutcome::failed(format!("commit: {e}")),
                },
    ...
    let parent = match repo.head() {
        Ok(h) => h
            .resolve()
            .ok()
            .and_then(|r| r.target().to_owned())
            .and_then(|t| repo.find_commit(t).ok()),
        Err(_) => None,
    };
    let parents: Vec<&git2::Commit<'_>> = parent.iter().collect();
    let oid = match repo.commit(Some("HEAD"), &author, &committer, &message, &tree, &parents) {
        Ok(o) => o,
        Err(e) => return ToolOutcome::failed(format!("commit: {e}")),
    };
    let branch = repo
            ...
            .and_then(|t| t.get_path(Path::new(&rel)).ok())
        {
            ...
            None => {
                if index.get_path(Path::new(&rel), 0).is_some() {
                    ...
                    return ToolOutcome::failed(format!(
                        "{cmd}: pathspec '{a}' did not match any files"
                    ));
                }
            }
        }
...
mod tests {
    ...
    // cmd_commit reads the data-dir config for attribution — pin TIDE_DATA_DIR
    // to a scratch dir so tests never see the developer's real ~/.tide, and
    // serialize (env is process-global across parallel test threads).
    static TIDE_DIR_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    fn git(ws: &Path, v: &[&str]) -> ToolOutcome {
        let _guard = TIDE_DIR_LOCK.lock().unwrap();
        let scratch = tempfile::tempdir().unwrap();
        std::env::set_var(store::paths::DATA_DIR_ENV, scratch.path());
        let out = run_git(&args(v), ws);
        std::env::remove_var(store::paths::DATA_DIR_ENV);
        out
    }

    /// Like `git` but with an attribution-bearing config in the data dir.
    fn git_with_config(ws: &Path, v: &[&str], config_json: &str) -> ToolOutcome {
        let _guard = TIDE_DIR_LOCK.lock().unwrap();
        let scratch = tempfile::tempdir().unwrap();
        std::fs::write(scratch.path().join("config.json"), config_json).unwrap();
        std::env::set_var(store::paths::DATA_DIR_ENV, scratch.path());
        let out = run_git(&args(v), ws);
        std::env::remove_var(store::paths::DATA_DIR_ENV);
        out
    ...
    fn status_reports_clean_tree_after_commit() {
        ...
        assert!(
            out.output.contains("nothing to commit, working tree clean"),
            "{}",
            out.output
        );
    }
    ...
    fn log_and_show_work() {
        ...
        assert!(
            out.output.contains("Author: Tester <tester@example.com>"),
            "{}",
            out.output
        );
    ...
    #[test]
    fn commit_attribution_co_author_and_author_modes() {
        let (tmp, repo) = seed_repo();
        ...
        // The mode must be named: the default (unset) attribution mode is
        // "author" — the same default the TS settings read applies — so a
        // config without gitAttributionMode would not exercise co-author
        // semantics.
        let co_author_cfg = r#"{"generalSettings":{"gitCoAuthored":true,"gitAttributionMode":"co-author","gitCoAuthorName":"Tide","gitCoAuthorEmail":"314188112+tide-codes@users.noreply.github.com"}}"#;
        let out = git_with_config(tmp.path(), &["commit", "-m", "add c"], co_author_cfg);
        assert_eq!(out.status, crate::OutcomeStatus::Executed, "{}", out.output);
        let head = repo.head().unwrap().peel_to_commit().unwrap();
        assert_eq!(head.author().name().unwrap(), "Agent");
        assert_eq!(
            head.message().unwrap(),
        ...
        let out = git_with_config(tmp.path(), &["commit", "-m", "add d"], author_cfg);
        assert_eq!(out.status, crate::OutcomeStatus::Executed, "{}", out.output);
        let head = repo.head().unwrap().peel_to_commit().unwrap();
        assert_eq!(head.author().name().unwrap(), "Tide");
        assert_eq!(head.committer().name().unwrap(), "Agent");
        assert_eq!(
            head.message().unwrap(),
            "add d\n\nCo-authored-by: Agent <agent@tide.local>"
        );

        // Attribution off → message untouched.
        let off_cfg = r#"{"generalSettings":{"gitCoAuthored":false}}"#;
        std::fs::write(tmp.path().join("e.txt"), "e\n").unwrap();
        git(tmp.path(), &["add", "e.txt"]);
        let out = git_with_config(tmp.path(), &["commit", "-m", "add e"], off_cfg);
        assert_eq!(out.status, crate::OutcomeStatus::Executed, "{}", out.output);
        let head = repo.head().unwrap().peel_to_commit().unwrap();
        assert_eq!(head.message().unwrap(), "add e");
    }
    ...
    fn add_commit_and_unstage_roundtrip() {
        ...
        assert!(
            out.output.contains("Refused: stage specific files by name"),
            "{}",
            out.output
        );
    ...
    fn subcommands_outside_native_set_fail_without_executing() {
        ...
        for v in [&["push"][..], &["rebase", "main"][..], &["stash"][..]] {
            ...
            assert!(
                out.output.contains("git2-native tool does not implement"),
                "{v:?}"
            );
        }
    ...
    fn paths_escaping_the_workspace_are_rejected() {
        ...
        assert_eq!(out.status, crate::OutcomeStatus::Failed);
        assert!(
            out.output.contains("outside the workspace root"),
            "{}",
            out.output
        );
        let _ = repo;
    ...
    fn non_repo_root_reports_fatal() {
        ...
        assert_eq!(out.status, crate::OutcomeStatus::Failed);
        assert!(
            out.output.contains("not a git repository"),
            "{}",
            out.output
        );
    }
    ...
    fn trait_execute_parses_args_array() {
        ...
        let out = tool
            .execute(
                ...
                serde_json::json!({"args": ["log", "--oneline"]}),
            )
            .unwrap();
