...
pub(crate) fn run_glob(
    ...
    let root = if rel_path.is_empty() {
        ...
        match resolve_and_follow_symlinks_roots(workspace_root, extra_read_roots, rel_path) {
            Ok(root) => root,
...
mod tests {
    ...
    fn double_star_spans_depths() {
        ...
        assert_eq!(
            paths,
            vec!["src/components/b.tsx", "src/components/ui/c.tsx"]
        );
    ...
    fn ignores_common_dirs() {
        ...
        assert!(
            paths.is_empty(),
            "node_modules/dist must be ignored: {paths:?}"
        );
