...
pub fn start(
    ...
    let child = Command::new("sh")
        ...
        // Same environment as the bash tool: login-shell capture plus
        // version-manager PATH entries, so `npx`/`bun` resolve when the
        // daemon was GUI-launched with a stripped PATH.
        .env_clear()
        .envs(tools::tools::proc::tool_env())
        .stdin(Stdio::null())
