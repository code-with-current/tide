...
use crate::model::{
    ActivityKind, DriverEvent, PermissionOption, UsageBreakdown, UserInputQuestion,
};
...
mod tests {
    use super::*;
    use crate::model::{
        ThreadGoal, ThreadGoalStatus, UsageBreakdown, UserInputOption, UserInputQuestion,
    };

    #[test]
    fn usage_updates_round_trip_and_old_payloads_decode_without_breakdown() {
        let wire = event_to_wire(DriverEvent::UsageUpdated {
        ...
        .unwrap();
        assert_eq!(wire.kind, "usageUpdated");
        assert_eq!(wire.payload["breakdown"]["cacheRead"], 11_000);
        ...
        assert_eq!(step.cache_read, 11_000);
        assert_eq!(step.calls, 1);

        // A payload from before the field existed must decode with None,
        // which reads as "not measured" rather than zero.
        let usage: UsageWire =
            serde_json::from_value(json!({ "contextTokens": 5, "contextWindow": null })).unwrap();
        assert_eq!(usage.context_tokens, Some(5));
