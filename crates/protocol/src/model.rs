...
impl ProviderKind {
    pub const ALL: [Self; 1] = [Self::Tide];
...
impl ProviderResumeCursor {
    pub fn from_session_id(provider: ProviderKind, id: String) -> Self {
        match provider {
            ProviderKind::Tide => Self::Tide { session_id: id },
        }
    }

    pub fn provider(&self) -> ProviderKind {
        match self {
            Self::Tide { .. } => ProviderKind::Tide,
        }
    }

    pub fn native_id(&self) -> &str {
        match self {
            Self::Tide { session_id } => session_id,
        }
    }
}
...
pub struct ProviderModel {
    ...
    /// Tide-enriched metadata: "$X / $Y per Mtok" and image input support,
    /// rendered on picker rows exactly like tide's model selector.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ProviderModel {
    ...
    pub fn price_label(mut self, price_label: impl Into<String>) -> Self {
        let price_label = price_label.into();
        if !price_label.is_empty() {
            self.price_label = Some(price_label);
        }
        self
    }
...
///
/// Drafts may carry [`Self::NewWorktree`] until their first prompt. Tide then
...
pub struct QueuedMessage {
    pub id: Uuid,
    pub content: String,
    /// The text typed before Tide appended provider-facing attachment
...
pub struct SessionUsageTotals {
    ...
    pub ttft_steps: u64,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub last_step: Option<UsageBreakdown>,
}

impl SessionUsageTotals {
    pub fn apply_step(&mut self, step: &UsageBreakdown) {
        ...
        self.tool_ms += step.tool_ms.unwrap_or(0);
        if let Some(ttft) = step.ttft_ms {
            self.ttft_ms_total += ttft;
            self.ttft_steps += 1;
        }
...
pub struct AgentSession {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub usage_totals: Option<SessionUsageTotals>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub enum ActivityKind {
    ...
    Search,
    Plan,
    /// The driver compacted the model's context (summary + tail replaced
    /// the shadowed range). A marker for the transcript and inspector —
    /// the user's transcript itself is never truncated.
    Compact,
    Tool,
}
...
impl ActivityKind {
    ...
    pub fn from_tool_name(name: &str) -> Self {
        ...
        if matches!(
            ...
            Self::Plan
        } else if compact == "compact" || compact == "compactcontext" {
            Self::Compact
        } else if matches!(
            compact.as_str(),
            "bash"
...
pub enum DriverEvent {
    ...
    /// fresh Harness session may resolve its deployment default when Tide did
    ...
    UsageUpdated {
        ...
        /// This step's delta breakdown. Old payloads decode with `None`,
        /// which reads as "not measured" rather than zero.
...
pub struct BackgroundWorkItem {
    ...
    /// The task a dispatched sub-agent was launched with — the prompt header
    /// the agents-panel timeline renders above the block stream. Items
    /// persisted before the field existed deserialize it as `None`.
    #[serde(default)]
    pub task: Option<String>,
    /// The sub-agent's streamed block timeline — reasoning, tool runs,
    /// narration — replaced wholesale by each
    /// [`BackgroundWorkEvent::SubagentBlocks`] event. The child's FINAL
    /// message pops into `output` (the Result card) at settlement, so it
    /// renders once and never also appears here. Empty for non-agent work
    /// and for items persisted before blocks existed, where the plain
...
pub enum SubagentBlock {
    ...
    /// A message DELIVERED to this child — from `main` or a sibling agent —
    /// as an annotation on its timeline, distinct from the child's own
    /// narration. `from` is the sender's label ("main" or an agent name).
    Message {
        from: String,
        text: String,
    },
...
pub enum BackgroundWorkEvent {
    ...
    /// A sub-agent's whole block timeline, replaced wholesale — lists are
    /// small, so the full snapshot keeps the event shape dumb and the
    /// reducer's apply trivial.
    SubagentBlocks {
        key: BackgroundWorkKey,
...
pub struct ActivityItem {
    ...
    /// Structured sub-agent result for a dispatch activity: what the
    /// `ToolDisplay::Agent` payload carries, attached by the driver so the
    /// transcript renders a dedicated card instead of raw output text.
    /// Old sessions deserialize without it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent: Option<ActivityAgent>,
    /// The compaction record on a Compact activity — the same numbers the
    /// inspector's Context Window section summarizes. Old sessions
...
mod tests {
    ...
    fn background_work_snapshots_have_serializable_named_items() {
        ...
        let BackgroundWorkEvent::ReconcileLive { items } = serde_json::from_value(json).unwrap()
        else {
    ...
    #[test]
    fn subagent_block_events_carry_the_whole_timeline() {
        let key = BackgroundWorkKey::new(BackgroundWorkKind::Subagent, "call-1");
        let event = BackgroundWorkEvent::SubagentBlocks {
            key: key.clone(),
            blocks: vec![
                SubagentBlock::Reasoning {
                    text: "where does the stream land?".to_owned(),
                    streaming: false,
                },
                SubagentBlock::Tool {
                    id: "t1".to_owned(),
        ...
        assert_eq!(json["type"], "subagentBlocks");
        assert_eq!(json["key"]["providerId"], "call-1");
        assert_eq!(json["blocks"][0]["kind"], "reasoning");
        assert_eq!(json["blocks"][1]["kind"], "tool");
        assert_eq!(json["blocks"][1]["status"], "done");
        assert_eq!(json["blocks"][1]["durationMs"], 1_240);
        assert_eq!(json["blocks"][2]["kind"], "text");
        let restored: BackgroundWorkEvent = serde_json::from_value(json).unwrap();
        let BackgroundWorkEvent::SubagentBlocks {
            key: restored_key,
            blocks,
        } = restored
        else {
    ...
    #[test]
    fn persisted_items_without_subagent_fields_still_load() {
        // The shape before `task`/`subagentBlocks` existed: every field the
        // old writer serialized, and nothing else, must deserialize.
        let json = serde_json::json!({
            ...
            "canStop": false,
            "controlId": null,
            "originActivityId": "call-7",
            "role": null,
            "model": null,
            "parentId": null,
            "status": "completed",
        });
        let item: BackgroundWorkItem = serde_json::from_value(json).unwrap();
        assert_eq!(item.task, None);
        assert!(item.subagent_blocks.is_empty());
        assert_eq!(item.output.as_deref(), Some("the old text log"));
    }

    #[test]
    fn attachment_messages_keep_transport_and_visible_content_separate() {
        let project = Project::from_path(PathBuf::from("/tmp/tide"));
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("tide-blob:ab/reference.png".to_owned()),
        };
    ...
    fn activity_targets_are_normalized_when_events_arrive() {
        let cases = [
            (
                ActivityKind::FileRead,
                serde_json::json!({"input": {"file_path": "/tmp/tide/src/app.rs"}}),
                "/tmp/tide/src/app.rs",
            ),
            (
                ActivityKind::FileSearch,
            ...
            (
                ActivityKind::FileList,
                serde_json::json!({"arguments": {"directory": "/tmp/tide/src"}}),
                "/tmp/tide/src",
            ),
            (
            ...
            (
                ActivityKind::Search,
                serde_json::json!({"action": {"queries": ["Tide GPUI"]}}),
                "Tide GPUI",
            ),
            (
                ActivityKind::FileRead,
                serde_json::json!("/tmp/tide/README.md"),
                "/tmp/tide/README.md",
            ),
        ];
    ...
    #[test]
    fn file_edit_metadata_is_normalized_for_every_argument_shape() {
        let cases = [
            (
                "diff-array",
                serde_json::json!([{
        ...
        for (shape, arguments, path, additions, deletions) in cases {
            let activity = ActivityItem::new(
                Some(format!("{shape}-edit")),
                ActivityKind::FileChange,
                "edit",
                None,
                false,
            ...
            .with_arguments(Some(arguments.to_string()));
            assert_eq!(activity.file_changes.len(), 1, "{shape}");
            let change = &activity.file_changes[0];
            assert_eq!(change.path, path, "{shape}");
            assert_eq!(change.additions, Some(additions), "{shape}");
            assert_eq!(change.deletions, Some(deletions), "{shape}");
            ...
            let diff = change.diff.as_deref().unwrap_or_else(|| {
                panic!("{shape} edit should carry a diff body");
            });
            ...
            assert_eq!(rendered_additions, additions, "{shape}");
            assert_eq!(rendered_deletions, deletions, "{shape}");
        }
    }
    ...
    fn projectless_projects_use_projects_root_and_recognize_legacy_paths() {
        ...
        let root = home.join(".tide");
        // The hardcoded no-project location is the home directory itself.
        let hardcoded = Project::from_path(home.clone());
    ...
    fn model_selection_keeps_started_sessions_on_their_provider() {
        ...
        assert!(session.can_choose_model(ProviderKind::Tide));
    ...
    fn provider_ids_are_stable() {
        assert_eq!(ProviderKind::Tide.id(), "tide");
        assert_eq!(ProviderKind::Tide.display_name(), "Tide");
        assert_eq!(ProviderKind::Tide.command(), "tide");
    }

    #[test]
    fn the_tide_provider_supports_native_conversation_actions() {
        assert!(ProviderKind::Tide.supports_conversation_fork());
        assert!(ProviderKind::Tide.supports_conversation_rollback());
    }

    #[test]
    fn the_tide_provider_catalog_is_discovered() {
        assert!(ProviderKind::Tide.supports_model_discovery());
    }
    ...
    fn response_fork_is_a_distinct_idle_session_through_the_selected_turn() {
        ...
        assert!(matches!(
            ...
            Some(ProviderResumeCursor::Tide { ref session_id }) if session_id == "forked-session"
        ));
    }
    ...
    fn legacy_file_edit_details_are_promoted_to_arguments_and_metadata() {
        ...
        assert_eq!(activities[0].file_changes[0].path, "/tmp/tide/README.md");
    ...
    fn legacy_file_tools_are_reclassified_and_gain_cached_targets() {
        ...
        cached.display_target = Some("/tmp/tide/src/persisted.rs".into());
        session.transcript_blocks.push(TranscriptBlock {
            after_message: 0,
            turn_id: None,
            activities: vec![
                ActivityItem::new(
                    ...
                    ActivityKind::Search,
                    "read",
                    Some(r#"{"filePath":"/tmp/tide/src/model.rs"}"#.into()),
                    true,
                ),
                ActivityItem::new(
                    None,
        ...
        assert_eq!(
            ...
            Some("/tmp/tide/src/persisted.rs")
        );
        assert!(
            activities[..2]
                .iter()
