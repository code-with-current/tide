...
pub enum SessionsV2Error {
    ...
    /// Truncate target message absent from the session (stale timeline).
    MessageNotFound {
        session_id: String,
        message_id: String,
    },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidPartData {
                ...
                f,
                "part {part_id} of message {message_id} has non-JSON data"
            ),
            ...
            Self::MessageNotFound {
                ...
                write!(f, "message {message_id} not found in session {session_id}")
            }
        }
    }
}
...
impl SessionsV2 {
    ...
    pub fn message_payloads(
        ...
        let placeholders = message_ids
            ...
            .join(", ");
        let sql = format!(
            "SELECT message_id, payload FROM message_payload WHERE message_id IN ({placeholders})"
        );
        let mut stmt = self.conn.prepare(&sql)?;
    ...
    /// Full `data` for one part of one session — the on-demand escape hatch
    /// for `output`/`display` strings the wire derivations bound. None when
    /// the part does not exist in this session.
    pub fn part_data(&self, session_id: &str, part_id: &str) -> Result<Option<Value>> {
        let mut stmt = self
            .conn
            .prepare("SELECT data FROM part WHERE id = ?1 AND session_id = ?2")?;
        let mut rows = stmt.query(rusqlite::params![part_id, session_id])?;
        match rows.next()? {
            Some(row) => {
                let data: String = row.get(0)?;
                Ok(serde_json::from_str(&data).map(Some).map_err(|source| {
    ...
    fn parts_of(&self, message_id: &str) -> Result<Vec<SessionPartV2>> {
        ...
        for row in rows {
            ...
            let data =
                serde_json::from_str(&data).map_err(|source| SessionsV2Error::InvalidPartData {
                    part_id: id.clone(),
                    message_id: message_id.to_owned(),
                    source,
                })?;
            parts.push(SessionPartV2 {
                id,
                seq,
                kind,
                data,
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("store-sessions-v2-{name}-{}", std::process::id()));
    ...
    fn list_meta_matches_wire_shape() {
        ...
        let page = store
            .list_sessions("/ws/alpha", SessionListOptsV2::default())
            .unwrap();
        let one = page.sessions.iter().find(|s| s.id == "s-one").unwrap();
        assert_eq!(
    ...
    fn messages_window_cursors_walk_back() {
        ...
        let second = store
            ...
            .unwrap();
        assert_eq!(
            message_ids(&second.messages),
            ["msg-02", "msg-03", "msg-04"]
        );
    ...
    fn headers_match_legacy_filters_order_and_counts() {
        ...
        assert!(store
            .list_session_headers("/ws/missing", "ws_x")
            .unwrap()
            .is_empty());
    }
    ...
    fn headers_wire_shape_is_camel_case_with_iso_timestamps() {
        ...
        insert_session_full(
            ...
            SeedSession {
                ...
                title: "Iso",
                parent_id: None,
                model_id: Some("model-x"),
                archived_at: None,
                time_created: 951_782_400_000,
                time_updated: 1_759_000_000_123,
            },
        );
        let store = open_at(&db);
    ...
    fn archived_headers_list_archived_only_in_first_archive_order() {
        ...
        insert_session_full(
            ...
            SeedSession {
                ...
                title: "OtherWorkspace",
                parent_id: None,
                model_id: None,
                archived_at: Some(9_999),
                time_created: 1,
                time_updated: 1,
            },
        );
        ...
        let store = open_at(&db);
        let archived = store
            .list_archived_headers("/ws/alpha", "ws_alpha")
            .unwrap();
        ...
        assert!(store
            .list_archived_headers("/ws/missing", "ws_x")
            .unwrap()
            .is_empty());
    }
    ...
    fn dispatch_headers_list_active_children_newest_first() {
        ...
        assert!(store
            .list_dispatch_headers("s-one", "ws_1")
            .unwrap()
            .is_empty());
    }
    ...
    fn side_table_reads_tolerate_dbs_without_the_tables() {
        ...
        writer
            .set_session_settings("s-one", Some("plan"), Some("high"), 1)
            .unwrap();
        writer
            .set_session_worktree("s-one", Some(&serde_json::json!({ "branch": "b" })), 2)
            .unwrap();
        drop(writer);
        let store = open_at(&db);
        assert_eq!(
    ...
    fn iso_from_unix_ms_matches_js_to_iso_string() {
        ...
        assert_eq!(
            iso_from_unix_ms(1_759_000_000_123),
        ...
        assert_eq!(
            iso_from_unix_ms(1_709_164_800_000),
    ...
    #[test]
    #[ignore = "touches the real ~/.tide — run explicitly: cargo test -p store -- --ignored --nocapture"]
    fn live_real_db_returns_ok() {
        ...
        println!(
            "sessions-v2: {total} sessions across {} workspaces",
            by_workspace.len()
        );
