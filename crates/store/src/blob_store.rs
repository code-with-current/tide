...
//! the transcript keeps only a `tide-blob:` reference.
...
pub fn is_blob_reference(value: &str) -> bool {
    protocol::blob::is_reference(value)
}
...
impl BlobStore {
    ...
    /// Resolves a `tide-blob:` reference against this daemon store's root.
...
mod tests {
    ...
    fn temporary_root() -> PathBuf {
        std::env::temp_dir().join(format!("tide-blobs-{}", uuid::Uuid::new_v4()))
    }
    ...
    fn traversal_references_are_rejected() {
        let store = BlobStore::new(temporary_root());
        assert!(store.path_for("tide-blob:../../etc/passwd").is_none());
