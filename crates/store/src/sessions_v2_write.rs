...
/// Outcome of [`SessionsV2Writer::truncate_after`] — what an edit removed
/// below the edited message.
...
impl SessionsV2Writer {
    ...
    pub fn delete_session(&self, id: &str) -> Result<()> {
        ...
        self.conn.execute(
            "DELETE FROM session_worktree WHERE session_id = ?1",
            params![id],
        )?;
        self.conn.execute(
    ...
    pub fn unarchive_session(&self, id: &str) -> Result<()> {
        self.conn.execute(
            "UPDATE session SET archived_at = NULL WHERE id = ?1",
            params![id],
        )?;
        Ok(())
    }
    ...
    pub fn archive_workspace_sessions(
        &self,
        workspace_path: &str,
        now_ms: i64,
        main_only: bool,
    ...
    /// Role of a stored message (`user`/`assistant`); None when absent.
    /// Edit-and-resend validates its target is a user message before
    /// truncating anything.
    pub fn message_role(&self, message_id: &str) -> Option<String> {
        self.conn
            .query_row(
    ...
    /// Edit-and-resend support: remove every row strictly after
    /// `message_id` — the reader's `ORDER BY id` canonical order — in one
    /// transaction: downstream messages, their parts, their frozen
    /// payloads, and every event past the last event of a kept message
    /// (the seq floor, so turn-level rows with no message_id go too). The
    /// session's live floor is dropped so the next live consumer
    /// re-anchors instead of skipping to ghost seqs. The edited message's
    /// own rows are untouched; rewriting its text is the caller's job
    /// (`update_part_data`).
    pub fn truncate_after(&mut self, session_id: &str, message_id: &str) -> Result<TruncateStats> {
        let tx = self.conn.unchecked_transaction()?;
        let found: i64 = tx.query_row(
            "SELECT COUNT(1) FROM message WHERE session_id = ?1 AND id = ?2",
            params![session_id, message_id],
            |row| row.get(0),
        )?;
        if found == 0 {
            return Err(SessionsV2Error::MessageNotFound {
        ...
        let floor: i64 = tx.query_row(
            "SELECT COALESCE(MAX(e.seq), 0) FROM event e \
             JOIN message m ON m.id = e.message_id AND m.session_id = e.session_id \
             WHERE e.session_id = ?1 AND m.id <= ?2",
            params![session_id, message_id],
            |row| row.get(0),
        )?;
        ...
        let parts = tx.execute(
            "DELETE FROM part WHERE session_id = ?1 \
             AND message_id IN (SELECT id FROM message WHERE session_id = ?1 AND id > ?2)",
            params![session_id, message_id],
        )?;
        let has_payload_table: i64 = tx.query_row(
            "SELECT COUNT(1) FROM sqlite_master WHERE type = 'table' AND name = 'message_payload'",
            [],
            |row| row.get(0),
        )?;
        if has_payload_table > 0 {
            tx.execute(
                "DELETE FROM message_payload WHERE message_id IN \
                 (SELECT id FROM message WHERE session_id = ?1 AND id > ?2)",
                params![session_id, message_id],
            )?;
        }
        ...
        tx.commit()?;
        self.live_seq.remove(session_id);
        Ok(TruncateStats {
            messages,
            parts,
            events,
                ...
                SinkEventType::MessageEnd => {
                    if let Some(message_id) = event.message_id.as_deref() {
                        ...
                        if let Some(usage) = event.data.as_ref().and_then(|d| d.get("usage")) {
                            let usage: SinkUsage =
                                serde_json::from_value(usage.clone()).map_err(|e| {
                                    SessionsV2Error::MalformedEvent {
...
fn commit_part_tx(
    ...
    let kind = body.get("kind").and_then(Value::as_str).ok_or_else(|| {
        SessionsV2Error::MalformedEvent {
...
mod tests {
    ...
    fn temp_dir(name: &str) -> Dir {
        let path =
            std::env::temp_dir().join(format!("store-v2-write-{name}-{}", std::process::id()));
    ...
    /// Seed a two-turn transcript: u1 → a1 (2 parts, events) → u2 → a2,
    /// returning the four message ids (ascending store order).
    fn seed_two_turn_transcript(writer: &mut SessionsV2Writer, sid: &str) -> [String; 4] {
        writer
            .create_session(
                CreateSessionInput {
                    id: sid,
            ...
            .unwrap();
        let mut ids: [String; 4] = std::array::from_fn(|_| String::new());
        let mut now = T0;
        for (i, role) in ["user", "assistant", "user", "assistant"]
            .into_iter()
            .enumerate()
        {
            let (id, slot) = (format!("m_{:020}_{i}", now), now);
            ids[i] = id.clone();
            writer
                .insert_message(
                    InsertMessageInput {
            ...
            writer
                .insert_part(
                    InsertPartInput {
                        ...
                        seq: 0,
                        kind: "text",
                        data: &json!({ "text": format!("msg {i}") }),
                    },
                    slot,
                )
                .unwrap();
            writer
                .insert_event(&text_commit(sid, &id, &pid, &format!("msg {i}"), 0), slot)
                .unwrap();
            if role == "assistant" {
                writer.insert_event(&turn_end(sid, &id), slot).unwrap();
            }
            now += 10;
        }
        ids
    ...
    #[test]
    fn truncate_after_drops_downstream_rows_and_resets_live_floor() {
        let (dir, mut writer) = writer_at("truncate");
        let sid = "s_trunc";
        let ids = seed_two_turn_transcript(&mut writer, sid);
        writer.mark_live(sid, 7);

        let stats = writer.truncate_after(sid, &ids[2]).unwrap();
        // Everything strictly after u2: a2's message + part, and a2's
        // text/turn.end events.
        assert_eq!(
            stats,
            TruncateStats {
                messages: 1,
        ...
        assert_eq!(
            writer.live_floor(sid),
            None,
            "live floor dropped for re-anchor"
        );

        let reader = reader_at(&dir);
        let page = reader.session_messages(sid, Default::default()).unwrap();
        let remaining: Vec<String> = page.messages.iter().map(|m| m.id.clone()).collect();
        assert_eq!(
            remaining,
            vec![ids[0].clone(), ids[1].clone(), ids[2].clone()]
        );
        // Kept events: both turns' u1/a1 rows plus u2's own text event —
        // a2's rows are gone.
        assert_eq!(event_rows(&writer, sid).len(), 4);

        // Idempotent: nothing downstream left.
        let again = writer.truncate_after(sid, &ids[2]).unwrap();
        assert_eq!(
            again,
            TruncateStats {
                messages: 0,
    ...
    #[test]
    fn truncate_after_unknown_message_errors_without_touching_rows() {
        let (dir, mut writer) = writer_at("truncate-missing");
        let sid = "s_missing";
        seed_two_turn_transcript(&mut writer, sid);
        let err = writer.truncate_after(sid, "m_nope").unwrap_err();
        assert!(matches!(err, SessionsV2Error::MessageNotFound { .. }));
        let reader = reader_at(&dir);
        assert_eq!(
            reader
                .session_messages(sid, Default::default())
                .unwrap()
                .messages
                .len(),
            4
    ...
    #[test]
    fn truncate_after_first_message_empties_the_session() {
        let (_dir, mut writer) = writer_at("truncate-all");
        let sid = "s_all";
        let ids = seed_two_turn_transcript(&mut writer, sid);
        let stats = writer.truncate_after(sid, &ids[0]).unwrap();
        assert_eq!(
            stats,
            TruncateStats {
                messages: 3,
    ...
    fn id_formats_match_the_ts_scheme() {
        ...
        assert!(
            part.starts_with('p') && part[1..].starts_with('_'),
            "{part}"
        );
    ...
    fn standalone_prune_and_replay_limit() {
        ...
        assert_eq!(
            event_rows(&writer, "s_d"),
            vec![(again, "turn.end".to_owned())]
        );
    }
    ...
    fn update_part_data_rewrites_and_touches() {
        ...
        assert_eq!(
            writer.update_part_data("ghost", &updated, 10_200).unwrap(),
            0
        );
        ...
        let time_updated: i64 = writer
            .conn
            .query_row("SELECT time_updated FROM part WHERE id = 'p_1'", [], |r| {
                r.get(0)
            })
            .unwrap();
    ...
    fn update_archive_delete_session_lifecycle() {
        ...
        writer
            ...
            .unwrap();
        writer
            .insert_event(&turn_end("s_l", "m_1"), 30_100)
            .unwrap();
        ...
        assert_eq!(
            event_rows(&writer, "s_l").len(),
            1,
            "events orphaned, like the TS"
        );
    }
    ...
    fn session_settings_patch_is_set_or_keep() {
        ...
        writer
            .set_session_settings("s_1", Some("plan"), None, T0 + 1)
            .unwrap();
        assert_eq!(
        ...
        writer
            .set_session_settings("s_1", None, Some("high"), T0 + 2)
            .unwrap();
        assert_eq!(
            writer.session_settings("s_1"),
    ...
    fn worktree_round_trips_and_clears() {
        ...
        writer
            .set_session_worktree("s_1", Some(&wt), T0 + 1)
            .unwrap();
    ...
    fn archive_state_probe_unarchive_and_cascade() {
        ...
        assert_eq!(
            writer
                .archive_workspace_sessions("/ws", T0 + 5, true)
                .unwrap(),
            1
        );
        ...
        writer
            .archive_workspace_sessions("/ws", T0 + 6, false)
            .unwrap();
        ...
        writer
            .set_session_todos("s_main", &json!([{ "content": "x" }]), T0 + 7)
            .unwrap();
        writer
            .set_session_settings("s_main", Some("ask"), Some("low"), T0 + 8)
            .unwrap();
    ...
    fn clear_all_wipes_every_table() {
        ...
        writer
            .set_session_settings("s_1", Some("ask"), None, T0 + 4)
            .unwrap();
