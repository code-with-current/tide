...
//! worktree beneath `~/.tide/worktrees`, then every workspace consumer uses
...
pub use protocol::git::CreatedWorktree;
...
pub fn create(
    ...
    let root = dirs::home_dir()
        .ok_or_else(|| anyhow!("could not locate the home directory for ~/.tide/worktrees"))?
        .join(".tide/worktrees");
    create_in(
        project_path,
        &root,
...
fn create_in(
    ...
    let branch = format!("tide/{fallback}");
...
mod tests {
    ...
    fn creates_isolated_worktrees_from_the_default_branch() {
        ...
        .unwrap();
        assert_eq!(first.branch, "tide/build-a-project-selector");
        assert_eq!(
        ...
        .unwrap();
        assert_eq!(second.branch, "tide/build-a-project-selector-2");
