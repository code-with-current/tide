...
fn build_catalog(raw: &HashMap<String, RawCatalogEntry>) -> HashMap<String, CatalogEntry> {
    raw.iter()
        .map(|(k, v)| (k.clone(), normalize_entry(k, v)))
        .collect()
}
...
pub(crate) fn refresh_from(data_dir: &Path, url: &str) -> bool {
...
/// Refresh when the loaded catalog is past the 7-day interval; deduped per
/// process session. Returns true when a fresh catalog was swapped in.
pub fn refresh_if_stale(data_dir: &Path) -> bool {
    if !claim_refresh_slot() {
        return false;
    }
    load(data_dir); // ensure is_stale sees a catalog even if refresh fails
    if !is_stale() {
        return false;
    }
    refresh_from(data_dir, CATALOG_URL)
}

/// Boot-time init: load, and when stale pull a fresh catalog. Never fails —
/// a failed fetch keeps the loaded (or bundled) catalog.
pub fn init(data_dir: &Path) {
    load(data_dir);
    if is_stale() && claim_refresh_slot() {
        let _ = refresh_from(data_dir, CATALOG_URL);
    }
}
            ...
            match entry {
                ...
                Some(entry) => {
                    ...
                    ModelMeta {
                        ...
                        pricing: if entry.input_cost_per_token != 0.0
...
fn fallback_meta(context_window: u64) -> ModelMeta {
    ModelMeta {
        ...
        max_input_tokens: if context_window != 0 {
            context_window
        } else {
            FALLBACK_CONTEXT
        },
...
fn enrich_stored_model(model: &mut StoredModel) -> bool {
    ...
    model.reasoning.get_or_insert(meta.supports_reasoning);
    extra
...
/// Rewrite every stored provider model that lacks a catalogId — the
/// `enrichExistingModels` port. Runs after the enrichment catalogs finish
/// loading/refreshing, so rows saved against a stale or missing catalog
/// (a model newer than the bundled baseline) get their pricing / context /
/// reasoning filled in after the fact. Idempotent: models that already carry
/// a catalogId, and models the catalog still can't resolve, are untouched —
/// an unresolved row keeps its `None` and a later pass can fill it in.
pub fn enrich_existing_models() {
    ...
    for provider in &mut config.providers {
        for model in &mut provider.models {
            changed |= enrich_stored_model(model);
        }
    }
    if changed {
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("tide-model-catalog-{name}-{}", std::process::id()));
    ...
    fn iso_round_trip_through_the_bundled_timestamp() {
        ...
        assert!(
            parse_iso_ms("2026-08-13T04:41:57Z").is_some(),
            "no-millis form"
        );
    }
    ...
    fn load_reads_bundled_baseline_and_resolves_claude() {
        ...
        assert_eq!(
            meta.resolved_catalog_id.as_deref(),
    ...
    fn match_semantics_exact_suffix_and_ambiguous() {
        ...
        install_catalog(
            &[
                ("anthropic/claude-sonnet-4-5", RawCatalogEntry::default()),
                (
        ...
        assert_eq!(
            match_model_to_catalog("no-such-model").state,
            MatchState::None_
        );
    }
    ...
    fn resolve_falls_back_conservatively_without_catalog_or_match() {
        ...
        install_catalog(
            &[("x/known", RawCatalogEntry::default())],
            "2099-01-01T00:00:00.000Z",
        );
    ...
    fn flatten_keeps_slim_fields_and_drops_descriptions() {
        ...
        assert_eq!(
            normalized.cache_read_input_token_cost,
            Some(0.3 / 1_000_000.0)
        );
    }
    ...
    fn enrichment_is_one_time_and_fills_missing_fields() {
        ...
        assert_eq!(
            model.extra["inputCostPerToken"],
            serde_json::json!(3.0 / 1_000_000.0)
        );
        // User-set values are never clobbered.
        model
            .extra
            .insert("max_completion_tokens".into(), Value::from(1024u64));
        assert!(!enrich_stored_model(&mut model), "catalogId set → skip");
        assert_eq!(model.extra["max_completion_tokens"], Value::from(1024u64));
    }

    /// The post-refresh pass heals rows saved during a stale-baseline window
    /// (a model newer than the bundled snapshot probes as "none" and lands
    /// in the config bare) and is idempotent once healed.
    #[test]
    fn enrich_existing_models_fills_saved_rows_in_place() {
        let _env = crate::TIDE_DIR_TEST_LOCK
            .lock()
            .unwrap_or_else(|poison| poison.into_inner());
        let _guard = test_state_guard();
        reset_for_tests();
        let dir = temp_dir("enrich-pass");
        // Edition 2024: env mutation is process-global; TIDE_DIR_TEST_LOCK
        // serializes it against the other env-touching tests.
        ...
        let config = store::config::Config {
            providers: vec![store::config::StoredProvider {
                id: "p_1".into(),
                name: "z.ai".into(),
        ...
        store::config::save(&store::paths::config_path(), &config).unwrap();

        enrich_existing_models();

        let healed = store::config::load(&store::paths::config_path()).unwrap();
        let model = &healed.providers[0].models[0];
        assert_eq!(model.catalog_id.as_deref(), Some("z-ai/glm-5.3"));
        assert_eq!(model.context_window, 1_310_720);
        assert_eq!(model.reasoning, Some(true));
        assert_eq!(
            model.extra["inputCostPerToken"],
            serde_json::json!(1.4 / 1_000_000.0)
        );

        // Idempotent: the healed row keeps its catalogId and is not rewritten.
        enrich_existing_models();
        let again = store::config::load(&store::paths::config_path()).unwrap();
        assert_eq!(
            again.providers[0].models[0].catalog_id.as_deref(),
            Some("z-ai/glm-5.3")
        );
        assert_eq!(again.providers[0].models[0].context_window, 1_310_720);
    ...
    #[test]
    fn refresh_fetches_caches_and_dedupes_per_session() {
        let _guard = test_state_guard();
        reset_for_tests();
        let dir = temp_dir("refresh");
        let url = models_dev_server(150);

        assert!(claim_refresh_slot());
        assert!(refresh_from(&dir, &url));
        ...
        let cached: CatalogFile =
            serde_json::from_str(&std::fs::read_to_string(dir.join(CACHE_FILENAME)).unwrap())
                .unwrap();
    ...
    #[test]
    fn refresh_rejects_tiny_payloads_and_keeps_the_loaded_catalog() {
        let _guard = test_state_guard();
        reset_for_tests();
        let dir = temp_dir("refresh-tiny");
        let url = models_dev_server(3);
        assert!(
            !refresh_from(&dir, &url),
            "sanity gate aborts under 100 models"
        );
        assert!(
            std::fs::read_to_string(dir.join(CACHE_FILENAME)).is_err(),
            "no cache written for a rejected payload"
        );
        assert!(!refresh_from(&dir, "http://127.0.0.1:1/api.json"));
        std::fs::remove_dir_all(&dir).unwrap();
        reset_for_tests();
    }
}
