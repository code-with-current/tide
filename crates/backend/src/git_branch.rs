...
pub use protocol::git::{BranchEntry, BranchSnapshot};
...
/// Checks out a commit sha as a detached HEAD — tide's History-tab
/// "Checkout (detached)".
pub fn checkout_detached(cwd: &Path, sha: &str) -> anyhow::Result<BranchSnapshot> {
    let output = crate::command_env::plain_command("git")
        .args(["switch", "--detach", "--"])
        .arg(sha)
        .current_dir(cwd)
...
pub fn create_and_checkout(cwd: &Path, branch: &str) -> anyhow::Result<BranchSnapshot> {
    create_and_checkout_from(cwd, branch, None)
}

/// Creates `branch` at `start_point` (a sha or any revision) and checks it
/// out — tide's "Branch from here…". A `None` start point is plain
/// `git switch -c <branch>` from HEAD.
pub fn create_and_checkout_from(
    cwd: &Path,
    branch: &str,
    start_point: Option<&str>,
) -> anyhow::Result<BranchSnapshot> {
    ...
    // No `--` before the name: `git switch -c -- <name>` makes git read the
    // name as a start-point reference; the check-ref-format gate above has
    // already rejected option-shaped names.
    let mut command = crate::command_env::plain_command("git");
    command.args(["switch", "-c"]).arg(branch);
    if let Some(start_point) = start_point.map(str::trim).filter(|s| !s.is_empty()) {
        command.arg("--").arg(start_point);
    }
    let output = command
        .current_dir(cwd)
        .output()
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("tide-branch-test-{}", Uuid::new_v4()));
