...
pub fn checkpoint_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/tide/session-{session_id}-turn-{turn_count}")
}

pub fn turn_start_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/tide/session-{session_id}-turn-start-{turn_count}")
}

pub fn turn_diff_base_ref(session_id: Uuid, turn_count: usize) -> String {
    format!("refs/tide/session-{session_id}-turn-diff-{turn_count}")
}
...
pub fn capture_turn(cwd: &Path, session_id: Uuid, turn_count: usize) -> anyhow::Result<Checkpoint> {
    ...
    let end_commit =
        capture_worktree_commit_from(cwd, end_head.as_deref(), "Tide worktree snapshot", &[])?;
...
pub fn capture_worktree_commit(cwd: &Path) -> anyhow::Result<String> {
    ...
    let head = resolve_ref(cwd, "HEAD");
    capture_worktree_commit_from(cwd, head.as_deref(), "Tide worktree snapshot", &[])
}
...
fn capture_worktree_commit_from(
    ...
    let temporary_index = common_dir.join(format!("tide-checkpoint-index-{}", Uuid::new_v4()));
...
fn virtual_branch_start(
    ...
    commit_tree(cwd, tree, "Tide turn diff base", &[])
}

fn empty_tree_commit(cwd: &Path) -> anyhow::Result<String> {
    commit_tree(cwd, EMPTY_TREE, "Tide empty turn diff base", &[])
}
    ...
    S: AsRef<OsStr>,
{
    ...
    if identity {
        command
            .env("GIT_AUTHOR_NAME", "Tide")
            .env("GIT_AUTHOR_EMAIL", "tide@localhost")
            .env("GIT_COMMITTER_NAME", "Tide")
            .env("GIT_COMMITTER_EMAIL", "tide@localhost");
    }
...
mod tests {
    ...
    fn diverged_repository() -> PathBuf {
        let directory = std::env::temp_dir().join(format!("tide-checkpoints-{}", Uuid::new_v4()));
        fs::create_dir_all(&directory).unwrap();
        git_ok(&directory, &["init", "--quiet", "--initial-branch=main"]);
        git_ok(&directory, &["config", "user.name", "Tide Test"]);
        git_ok(&directory, &["config", "user.email", "tide@example.com"]);
