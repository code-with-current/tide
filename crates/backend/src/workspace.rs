...
pub fn execute(operation: WorkspaceOperation) -> anyhow::Result<WorkspaceResult> {
    Ok(match operation {
        ...
        WorkspaceOperation::DiscoverSlashCommands { project_root, .. } => {
            WorkspaceResult::SlashCommands {
                commands: crate::composer_complete::discover_slash_commands(
                    protocol::model::ProviderKind::Tide,
                    &project_root,
                ),
            }
        }
        ...
        WorkspaceOperation::CheckoutBranch {
            ...
            snapshot: match (create, start_point.as_deref()) {
                (true, Some(start_point)) => {
                    crate::git_branch::create_and_checkout_from(&cwd, &branch, Some(start_point))?
                }
                (true, None) => crate::git_branch::create_and_checkout(&cwd, &branch)?,
                (false, Some(sha)) => crate::git_branch::checkout_detached(&cwd, sha)?,
                (false, None) => crate::git_branch::checkout(&cwd, &branch)?,
            },
        },
        ...
        WorkspaceOperation::GenerateSessionTitle {
            ...
            first_message,
            invocation,
        } => WorkspaceResult::GitText {
            text: crate::git_commit::generate_title(&cwd, &first_message, &invocation)?,
        },
        ...
        WorkspaceOperation::InspectGitStatus { cwd } => WorkspaceResult::GitStatus {
            changes: crate::git_panel::status(&cwd),
        },
        WorkspaceOperation::GitFileDiff {
            cwd,
            path,
            staged,
            context_lines,
        } => WorkspaceResult::GitDiff {
            hunks: crate::git_panel::file_diff(&cwd, &path, staged, context_lines),
        },
        WorkspaceOperation::GitLog { cwd, limit } => WorkspaceResult::GitLog {
            commits: crate::git_panel::log(&cwd, limit),
        },
        WorkspaceOperation::GitCommitFiles { cwd, sha } => WorkspaceResult::GitStatus {
            changes: crate::git_panel::commit_files(&cwd, &sha),
        },
        WorkspaceOperation::GitCommitFileDiff { cwd, sha, path } => WorkspaceResult::GitDiff {
            hunks: crate::git_panel::commit_file_diff(&cwd, &sha, &path),
        },
        WorkspaceOperation::GitCommitMessage { cwd, sha } => WorkspaceResult::GitText {
            text: crate::git_panel::commit_message(&cwd, &sha),
        },
        WorkspaceOperation::GitBulk { cwd, op, message } => WorkspaceResult::GitOp {
            result: crate::git_panel::bulk(&cwd, &op, message.as_deref()),
        },
        WorkspaceOperation::GitStashList { cwd } => WorkspaceResult::GitStashes {
            stashes: crate::git_panel::stash_list(&cwd),
        },
        WorkspaceOperation::GitStageFile { cwd, path, stage } => WorkspaceResult::GitOp {
            result: crate::git_panel::stage_file(&cwd, &path, stage),
        },
        ...
        WorkspaceOperation::GitAmend { cwd, message } => WorkspaceResult::GitCommitDone {
            result: crate::git_panel::amend(&cwd, message.as_deref()),
        },
        WorkspaceOperation::GitRevert { cwd, sha } => WorkspaceResult::GitRevertDone {
            result: crate::git_panel::revert(&cwd, &sha),
        },
        WorkspaceOperation::GitAheadBehind { cwd } => WorkspaceResult::GitAheadBehind {
            ahead_behind: crate::git_panel::ahead_behind(&cwd),
        },
        WorkspaceOperation::GitBranchInfo { cwd } => WorkspaceResult::GitBranchInfoDone {
            info: crate::git_panel::branch_info(&cwd),
        },
        WorkspaceOperation::GitRecentBranches { cwd } => WorkspaceResult::GitBranches {
            branches: crate::git_panel::recent_branches(&cwd),
        },
        WorkspaceOperation::GitMerge { cwd, name } => WorkspaceResult::GitMergeDone {
            result: crate::git_panel::merge(&cwd, &name),
        },
        WorkspaceOperation::GitConflictFiles { cwd } => WorkspaceResult::GitConflicts {
            conflicts: crate::git_panel::conflict_files(&cwd),
        },
        WorkspaceOperation::GitResolveFile { cwd, path, side } => WorkspaceResult::GitOp {
            result: crate::git_panel::resolve_file(&cwd, &path, &side),
        },
...
mod tests {
    ...
    fn repository() -> PathBuf {
        let root = std::env::temp_dir().join(format!("tide-workspace-{}", Uuid::new_v4()));
    ...
    fn directory_browser_lists_an_arbitrary_daemon_directory() {
        let directory =
            std::env::temp_dir().join(format!("tide-directory-browser-{}", Uuid::new_v4()));
    ...
    fn file_reader_keeps_the_complete_disk_content() {
        let root = std::env::temp_dir().join(format!("tide-editor-file-{}", Uuid::new_v4()));
        fs::create_dir_all(&root).unwrap();
        let content = "line\n".repeat(10_000);
