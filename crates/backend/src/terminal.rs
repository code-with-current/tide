    ...
    impl DaemonTerminal {
        pub fn open(
            ...
            let reader = std::thread::Builder::new()
                .name("tide-daemon-terminal-output".into())
                .spawn(move || {
    ...
    impl Drop for DaemonTerminal {
        fn drop(&mut self) {
            ...
            if let Some(reader) = self.reader.take() {
                // A shell descendant that outlives the shell (prompt daemons
                // like gitstatusd inherit the slave fd) can keep the master
                // read from ever waking, and joining then would hang whoever
                // drops the terminal — the daemon's request handler — far
                // past any client timeout. Give the reader a short grace
                // period and detach it when it stays wedged: it owns its
                // Arcs and exits on its own once the pty finally closes.
