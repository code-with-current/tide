...
//!
//! Dotfile setups symlink one directory everywhere. The catalog therefore
...
pub fn user_skill_locations() -> Vec<SkillLocation> {
    ...
    // Every tool keeps its own tree, but the skills in them are one shared
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!("tide-skills-{tag}-{}", std::process::id()));
    ...
    fn copies_across_roots_group_into_one_entry() {
        ...
        let tide_root = temp_root("group-tide");
        // The installer-drops-it-everywhere layout: same skill, several roots.
        for root in [&shared_root, &tide_root] {
            write_skill(
                root,
                "agents-sdk",
        ...
        let locations = vec![
            ...
            user_location(SkillSource::Provider(ProviderKind::Tide), &tide_root),
        ];
        let catalog = scan_skills(&locations);
    ...
    fn scan_orders_user_before_projects_and_counts_cross_scope_duplicates() {
        ...
        let locations = vec![
            ...
            SkillLocation {
                source: SkillSource::Provider(ProviderKind::Tide),
                scope: SkillScope::Project,
                root: project_root.clone(),
                project: Some("tide".into()),
            },
        ];
        let catalog = scan_skills(&locations);
        ...
        assert_eq!(
            names,
            vec![("review", None), ("zeta", None), ("review", Some("tide"))]
        );
    ...
    #[test]
    fn shared_roots_are_listed_for_user_and_project_scopes() {
    ...
    #[test]
    fn same_name_skills_across_user_roots_group_into_one_entry() {
        let agents = temp_root("pool-agents");
        let claude = temp_root("pool-claude");
        write_skill(&agents, "review", "---\nname: review\n---\nX");
        write_skill(
            &claude,
            "review",
            "---\nname: review\ndescription: Re-reads diffs\n---\nY",
        );

        let locations = vec![
            user_location(SkillSource::Shared, &agents),
            user_location(SkillSource::Shared, &claude),
        ];
        let catalog = scan_skills(&locations);
        assert_eq!(catalog.skills.len(), 1, "one row, not one per root");
        ...
        let skill = &catalog.skills[0];
        assert_eq!(skill.installs.len(), 2);
        assert_eq!(skill.description, "Re-reads diffs");
        // All user roots are one shared pool, so the label says it once.
        assert_eq!(skill.sources_label(), "Shared");
        assert_eq!(skill.duplicates, 0, "grouped copies are not duplicates");
