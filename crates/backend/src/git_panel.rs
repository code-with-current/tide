...
fn status_entries(repo: &Repository) -> Result<Vec<PanelFileChange>, git2::Error> {
    ...
    // vs HEAD. Untracked files ride along with their content counted, and an
    // unborn HEAD diffs against the empty tree so every file still reads.
...
fn single_file_diff<'r>(
    ...
    let diff = if staged {
        ...
        // Untracked files must diff too — a file nothing tracks yet (an
        // unborn HEAD, or one the tools just created) otherwise reads as a
        // blank +0/-0 viewer.
        opts.include_untracked(true)
            .show_untracked_content(true)
            .recurse_untracked_dirs(true);
...
mod tests {
    ...
    fn status_covers_staged_unstaged_untracked_deleted_with_counts() {
        ...
        assert!(!c.staged);
        // Untracked content counts now: the file's lines read as additions.
    ...
    #[test]
    fn untracked_files_count_and_diff_like_additions() {
        let root = seeded_repo("untracked");
        fs::write(root.join("tracked.txt"), "one\n").unwrap();
        commit_all(&root, "seed");
        fs::write(root.join("tool-made.txt"), "a\nb\nc\n").unwrap();

        // The status row counts the untracked file's lines instead of 0/0…
        let changes = status(&root);
        let tool = changes
            .iter()
            .find(|change| change.path == "tool-made.txt")
            .unwrap();
        assert_eq!((tool.additions, tool.deletions), (3, 0));

        // …and its unstaged diff is the whole file as additions.
        let hunks = file_diff(&root, "tool-made.txt", false, 3);
        let kinds: Vec<&str> = hunks
            .iter()
            .flat_map(|hunk| hunk.lines.iter().map(|line| line.kind.as_str()))
            .collect();
        assert_eq!(kinds, vec!["add", "add", "add"]);

        fs::remove_dir_all(root.parent().unwrap()).unwrap();
    }

    #[test]
    fn unborn_head_still_counts_and_diffs_the_workdir() {
        let root = seeded_repo("unborn");
        ...
        assert_eq!(hunks.iter().flat_map(|hunk| hunk.lines.iter()).count(), 2);
