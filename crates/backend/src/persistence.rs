...
//! `~/.tide/app.json` for Release, daemon preferences in
//! `~/.tide/settings.json`, and binary payloads in [`crate::blob_store`].
//! Of the configuration documents, only the desktop file is written here;
//! daemon settings cross the RPC boundary and are persisted by `tide-daemon`.
//!
...
/// at `~/.tide/app.json` without exposing app-managed state or daemon-owned
/// provider policy.
...
impl PersistedState {
    ...
    pub fn empty() -> Self {
        Self {
            ...
            last_provider: ProviderKind::Tide,
            last_model: None,
    ...
    pub fn fresh(cwd: PathBuf) -> Self {
        ...
        let session = AgentSession::new(project.id, ProviderKind::Tide);
        Self {
...
pub struct StateStore {
    ...
    /// Release uses the explicit cross-client Tide configuration directory.
...
impl StateStore {
    ...
    /// Local database owner used inside `tide-daemon`. It never reads or
...
mod tests {
    ...
    fn temporary_directory() -> PathBuf {
        std::env::temp_dir().join(format!("tide-state-{}", Uuid::new_v4()))
    }
    ...
    fn new_session_drafts_follow_the_project_across_runtime_session_ids() {
        ...
        let first_runtime_session = AgentSession::new(project_id, ProviderKind::Tide);
        let relaunched_runtime_session = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    fn existing_session_drafts_are_isolated_by_session_id() {
        let project_id = Uuid::new_v4();
        let mut first = AgentSession::new(project_id, ProviderKind::Tide);
        let mut second = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    fn sent_attachment_presentation_round_trips_with_message_rows() {
        ...
        let attachment = MessageAttachment {
            ...
            is_dir: false,
            is_image: true,
            blob_reference: Some("tide-blob:abcdef.png".to_owned()),
        };
    ...
    fn default_path_is_build_specific() {
        ...
        {
            assert_eq!(directory, Some(std::ffi::OsStr::new("Tide")));
            let configuration_directory = dirs::home_dir()
                .unwrap_or_else(std::env::temp_dir)
                .join(".tide");
            assert_eq!(
    ...
    fn state_round_trips() {
        ...
        // The inspector's Context Window detail: occupancy plus the
        // cumulative totals. Both must survive the skeleton → hydration
        // handoff, which copies fields by hand onto the list row.
        state.sessions[0].context_usage = Some(crate::model::ContextUsage {
            tokens: 128_000,
            window: Some(200_000),
        });
        state.sessions[0].usage_totals = Some(crate::model::SessionUsageTotals {
            input_tokens: 1_500,
            output_tokens: 350,
            cache_read: 11_000,
            cache_write: 250,
        ...
        assert_eq!(
            restored.sessions[0].context_usage,
            state.sessions[0].context_usage
        );
        assert_eq!(
            restored.sessions[0].usage_totals,
            state.sessions[0].usage_totals
        );
        assert_eq!(
            restored.model_traits_for(ProviderKind::Tide, "gpt-5.6-luna"),
    ...
    fn unchanged_sessions_are_not_rewritten() {
        ...
        state.sessions[0].finish_active_turn(crate::model::TurnStatus::Completed);
        let quiet = {
            let mut session = state.new_session(state.projects[0].id, ProviderKind::Tide);
    ...
    fn message_search_reads_skeleton_history_and_prefers_user_matches() {
        ...
        let mut assistant_match = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    fn deleting_a_session_removes_its_messages() {
        ...
        state.sessions[0].push_message(MessageRole::User, "keep me");
        state.sessions[0].finish_active_turn(crate::model::TurnStatus::Completed);
        let mut extra = state.new_session(state.projects[0].id, ProviderKind::Tide);
    ...
    fn last_reply_at_is_derived_for_sessions_stored_without_it() {
        ...
        let mut fresh = AgentSession::new(Uuid::new_v4(), ProviderKind::Tide);
    ...
    fn sessions_can_be_listed_without_deserializing_transcripts() {
        ...
        state.sessions[0].begin_turn("First");
        state.sessions[0].finish_active_turn(crate::model::TurnStatus::Completed);
        let mut second = state.new_session(state.projects[0].id, ProviderKind::Tide);
    ...
    fn quitting_on_a_draft_relaunches_to_the_new_session_page() {
        ...
        state.sessions[0].begin_turn("Persist this session");
        state.sessions[0].finish_active_turn(crate::model::TurnStatus::Completed);
        let draft = state.new_session(state.projects[0].id, ProviderKind::Tide);
    ...
    fn sessions_without_transcript_blocks_remain_compatible() {
        let session = AgentSession::new(Uuid::new_v4(), ProviderKind::Tide);
    ...
    fn selected_model_and_traits_are_used_for_new_sessions() {
        let mut state = PersistedState::fresh(PathBuf::from("/tmp/project"));
        state.last_provider = ProviderKind::Tide;
        state.last_model = Some("tide/model-a".into());
        state.last_reasoning_effort = Some("high".into());
        state.last_service_tier = Some("fast".into());

        let remembered = state.new_session(state.projects[0].id, ProviderKind::Tide);

        assert_eq!(remembered.model.as_deref(), Some("tide/model-a"));
    ...
    fn model_traits_are_remembered_by_provider_and_model() {
        ...
        assert_eq!(
            state.model_traits_for(ProviderKind::Tide, "tide/model-b"),
            (None, None, None),
            "a different model starts from its own defaults"
        );
        assert_eq!(
            state.model_traits_for(ProviderKind::Tide, "tide/model-a"),
