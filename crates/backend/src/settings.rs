...
pub use protocol::settings::DaemonSettings;
use uuid::Uuid;
        ...
        let (mut settings, write_current) = match fs::read(&path) {
            Ok(bytes) => match serde_json::from_slice(&bytes) {
                ...
                Err(error) => {
                    ...
                    eprintln!(
                        "Tide daemon moved invalid settings to {}: {error}",
                        backup.display()
                    );
...
mod tests {
    ...
    fn legacy_combined_settings_keep_only_daemon_fields() {
        let path = std::env::temp_dir().join(format!("tide-settings-{}.json", Uuid::new_v4()));
        fs::write(
            &path,
    ...
    fn imports_daemon_fields_from_a_debug_combined_settings_file() {
        let directory = std::env::temp_dir().join(format!("tide-settings-{}", Uuid::new_v4()));
        let path = directory.join("home/settings.json");
        let legacy = directory.join("checkout/temp/settings.json");
        fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        fs::write(&legacy, r#"{"theme":"dark","future":42}"#).unwrap();
