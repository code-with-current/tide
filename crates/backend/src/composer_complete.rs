...
//! text, command discovery, fuzzy filtering — so the popup itself can stay a
//! pure view. Discovery walks directories, which means it runs on the
//! background executor only; the render path reads the cached results and
//! nothing else.
...
pub use protocol::composer::{CommandScope, FileEntry, SlashCommand};
...
/// through `merge_reported_commands`.
pub fn discover_slash_commands(_provider: ProviderKind, project_root: &Path) -> Vec<SlashCommand> {
    // The parameter matches the wire operation; the catalog itself no longer
    // varies by provider.
    assemble_slash_commands(project_root)
}

fn assemble_slash_commands(project_root: &Path) -> Vec<SlashCommand> {
    ...
    scan_command_files(
        &project_root.join(".tide/commands"),
    ...
    if let Some(home) = home.as_deref() {
        scan_command_files(
            &home.join(".config/tide/commands"),
            CommandScope::User,
            true,
            &mut commands,
        );
...
/// templates expand. Passthrough commands, skills, and unknown `/words` stay
/// untouched.
...
mod tests {
    ...
    fn submission_expands_template_commands_only() {
        ...
        assert_eq!(
            resolved_submission(ProviderKind::Tide, "/fix the tests", &commands).as_deref(),
            Some("Fix the tests carefully.")
        );
        assert_eq!(
            resolved_submission(ProviderKind::Tide, "/compact", &commands),
            None
        );
        assert_eq!(
            resolved_submission(ProviderKind::Tide, "/unknown thing", &commands),
            None
        );
        assert_eq!(
            resolved_submission(ProviderKind::Tide, "plain prompt", &commands),
            None
        );
        // Skills are passthrough: the engine's own machinery resolves them.
        assert_eq!(
            resolved_submission(
                ProviderKind::Tide,
    ...
    fn shared_skills_are_listed_raw() {
        let root = std::env::temp_dir().join(format!("tide-skills-{}", std::process::id()));
        ...
        .unwrap();
        let commands = assemble_slash_commands(&root);
        let skill = commands
            .iter()
            .find(|command| command.name == "deploy-runbook")
            .expect("the shared skill is listed");
        ...
        assert_eq!(
            resolved_submission(ProviderKind::Tide, "/deploy-runbook staging", &commands),
            None
        );
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn symlinked_skills_and_command_dirs_are_discovered() {
        let root = std::env::temp_dir().join(format!("tide-symlink-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn derived_directories_join_the_file_index() {
        let root = std::env::temp_dir().join(format!("tide-files-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
