...
use crate::store::{ChunkRow, EmbeddingPlan, RagStore};
...
/// re-embeds chunks whose contentHash changed. The intended plan (from
/// the resolved embedder + configured chunking) must match an existing
/// index's recorded plan — a mismatch blocks ingest (rebuild required)
...
pub fn ingest_workspace(
    ...
    let rag_store = if db_path.is_file() {
        ...
        RagStore::open_at_with_plan(&db_path, plan)
    ...
    // searches (and pre-v3 dbs backfill their plan from it).
...
mod tests {
    ...
    fn ingest_resets_index_on_plan_change() {
        ...
        std::fs::write(ws.join("lib.rs"), "pub fn alpha() {}\n").unwrap();
        let data = dir.path().join("data");
        let embedder = crate::store::FakeEmbedder { dim: 384 };
        let plan = |embedder_id: &str, chunk_size: Option<u64>| EmbeddingPlan {
            embedder_id: embedder_id.into(),
            dims: 384,
            chunk_size,
            chunk_overlap: None,
            created_at: crate::unix_ms_now(),
        };
        fn inputs<'a>(
            data: &'a std::path::Path,
            ws: &'a std::path::Path,
        ) -> WorkspaceIngestInputs<'a> {
        ...
        // First run under the default plan succeeds and records it.
        ingest_workspace(
            inputs(&data, &ws),
            &embedder,
