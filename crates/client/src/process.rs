...
impl DaemonProcess {
    ...
    fn spawn_configured(
        ...
        let app_executable = std::env::current_exe().context("could not locate Tide executable")?;
        ...
        let stdout = child
            .stdout
            .take()
            .context("Tide daemon did not expose its readiness stream")?;
        let (ready_tx, ready_rx) = mpsc::sync_channel(1);
        std::thread::Builder::new()
            .name("tide-daemon-ready".into())
            .spawn(move || {
                ...
                let result = BufReader::new(stdout)
                    ...
                    .and_then(|bytes| {
                        if bytes == 0 {
                            bail!("Tide daemon exited before becoming ready")
                        }
                        serde_json::from_str::<DaemonReady>(&line).map_err(anyhow::Error::from)
                    });
                let _ = ready_tx.send(result);
            })
            .context("could not start Tide daemon readiness reader")?;
        let ready = match ready_rx.recv_timeout(START_TIMEOUT) {
            ...
            Err(error) => {
                let _ = child.kill();
                let _ = child.wait();
                bail!("timed out waiting for Tide daemon: {error}");
            }
        };
...
fn desktop_client_address(address: &str) -> anyhow::Result<String> {
    let address = address
        ...
        .with_context(|| format!("Tide daemon returned an invalid address {address:?}"))?;
...
struct SupervisorInner {
    ...
    /// True for the in-process backend the desktop serves itself: same
...
impl DaemonSupervisor {
    ...
    pub fn spawn_configured(
        ...
        std::thread::Builder::new()
            .name("tide-daemon-supervisor".into())
            .spawn(move || monitor_daemon(weak_inner, Some(initial_stamp), watch_for_rebuilds))
            .context("could not start Tide daemon supervisor")?;
        Ok(supervisor)
    }
    ...
    /// Connect to the backend this process serves itself. Identical to
    /// [`Self::connect`], but marked local: folder pickers, reveal-in-Finder,
    /// and skills operate on the same machine as the backend.
    pub fn connect_local(address: &str, token: String) -> anyhow::Result<Self> {
        let supervisor = Self::connect(address, token)?;
        supervisor
            .inner
            .in_process
    ...
    pub fn connect(address: &str, token: String) -> anyhow::Result<Self> {
        ...
        let weak_inner = Arc::downgrade(&supervisor.inner);
        std::thread::Builder::new()
            .name("tide-remote-daemon-supervisor".into())
            .spawn(move || monitor_daemon(weak_inner, None, false))
            .context("could not start remote Tide daemon supervisor")?;
        Ok(supervisor)
    }
    ...
    fn from_target(
        ...
        std::thread::Builder::new()
            .name("tide-daemon-settings".into())
            .spawn(move || persist_settings(weak_inner, settings_update_rx))
            .context("could not start Tide daemon settings writer")?;
        Ok(Self { inner })
    }
    ...
    pub fn reconfigure(&self, exposure: DaemonExposureSettings) -> anyhow::Result<()> {
        let exposure = exposure.validate()?;
        if self.inner.in_process.load(Ordering::SeqCst) {
            return self.reconfigure_in_process(exposure);
        }
        let executable = self
            .inner
            .executable
            .as_ref()
            .context("the connected daemon is managed outside Tide Desktop")?
            .clone();
    ...
    pub fn update_settings(&self, settings: DaemonSettings) -> anyhow::Result<()> {
        *self.inner.settings.lock() = settings.clone();
        ...
        self.inner
            .settings_updates
            .send(settings)
            .map_err(|_| anyhow::anyhow!("Tide daemon settings writer is closed"))
    }
}
...
fn monitor_daemon(
    ...
    loop {
        ...
        match replace_local_daemon(&inner, executable, &exposure) {
            Ok(()) => {}
            Err(error) => {
                eprintln!("could not restart rebuilt Tide daemon: {error:#}");
                continue;
            }
        }
...
fn replace_local_daemon(
    ...
    let previous = {
        ...
        match &*target {
            DaemonTarget::Remote { .. } => {
                bail!("the connected daemon is managed outside Tide Desktop")
            }
...
fn read_settings(client: &DaemonClient) -> anyhow::Result<DaemonSettings> {
    match client.request(Uuid::nil(), Uuid::nil(), Command::GetSettings)? {
        ...
        _ => bail!("Tide daemon returned an invalid settings response"),
    }
}
    ...
    while let Ok(mut settings) = updates.recv() {
        ...
        loop {
            ...
            match result {
                ...
                Ok(_) => {
                    eprintln!("Tide daemon returned an invalid settings update response");
                }
                Err(error) => {
                    eprintln!("could not persist Tide daemon settings: {error:#}");
                }
            }
            drop(inner);
            std::thread::sleep(REBUILD_POLL_INTERVAL);
    ...
    fn browser_origins_are_exact_and_deduplicated() {
        assert_eq!(
            parse_allowed_origins(
                "https://app.tide.test, http://localhost:3001, https://app.tide.test"
            )
            .unwrap(),
            ["https://app.tide.test", "http://localhost:3001"]
        );
        assert!(parse_allowed_origins("https://app.tide.test/path").is_err());
        assert!(parse_allowed_origins("ws://app.tide.test").is_err());
    }
