...
use protocol::model::ReportedCommand;
...
/// Parse the submitted text as the tide-native `/goal` command, which Tide
/// bridges to `thread/goal/*`. `None` when it is not one — other text, or a
/// project/user command that deliberately owns `/goal` (resolution precedence
/// stands).
pub fn parse_goal_submission(prompt: &str, commands: &[SlashCommand]) -> Option<GoalCommand> {
    ...
    let goal_is_builtin = commands.iter().any(|command| {
        command.name == "goal"
...
/// Resolve composer text into the exact prompt sent to the tide engine.
///
/// Template commands expand to their body; other text passes through.
pub fn resolved_submission(prompt: &str, commands: &[SlashCommand]) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn command_composer_text_keeps_the_slash() {
        assert_eq!(
    ...
    fn goal_submissions_parse_into_their_intent() {
        ...
        let parse = |prompt: &str| parse_goal_submission(prompt, commands);
    ...
    #[test]
    fn goal_command_respects_command_overrides() {
        ...
        assert_eq!(
            parse_goal_submission("/goal", std::slice::from_ref(&project)),
            None
        );
    }
}
