...
use protocol::computer_use::ComputerAppGrant;
use protocol::i18n::AppLanguage;
use protocol::identity::DATA_DIRECTORY_NAME;
use protocol::model::{AgentSession, FavoriteModel, Project, ProviderKind, RuntimeMode};
use protocol::theme::ThemePreference;
...
impl ComposerDraftStore {
    ...
    pub fn load(&self) -> io::Result<ComposerDrafts> {
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Tide daemon returned an invalid composer-drafts response",
            )),
        }
    }
    ...
    pub fn save(&self, drafts: ComposerDrafts, generation: u64) -> io::Result<()> {
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Tide daemon returned an invalid composer-drafts save response",
            )),
        }
    }
}
    ...
    fn migrate_loaded(&mut self) {
        for session in &mut self.sessions {
            let checkpoint_totals_current = session.turns.iter().all(|turn| {
                turn.checkpoint
                    .as_ref()
                    .is_none_or(protocol::model::Checkpoint::totals_are_current)
            });
            let before = (
                session.turns.len(),
...
/// Persist app-owned launch settings (e.g. a freshly generated Remote
/// Control relay path) so subsequent boots reuse them.
...
impl StateStore {
    ...
    pub fn session_message_search(
            ...
            .map_err(to_io_error)?
        {
            ...
            _ => Err(io::Error::other(
                "Tide daemon returned an invalid message-search response",
            )),
        }
    }
    ...
    pub fn load(&self) -> io::Result<PersistedState> {
            ...
            .map_err(to_io_error)?
        {
            ResponsePayload::TaskState {
                ...
                protocol::projectless::set_workspace_root(projectless_root);
                (projects, sessions, default_cwd)
            }
            _ => {
                return Err(io::Error::other(
                    "Tide daemon returned an invalid task-state response",
                ));
            }
        };
...
pub fn hydrate_session(
        ...
        .map_err(to_io_error)?
    {
        ...
        _ => Err(io::Error::other(
            "Tide daemon returned an invalid session-hydration response",
        )),
    }
}
...
mod tests {
    ...
    fn new_tasks_inherit_the_remembered_access_mode() {
        ...
        let session = state.new_session(state.projects[0].id, ProviderKind::Tide);
...
pub fn read_remote_reference(
    ...
    let command = if protocol::blob::is_reference(reference) {
        Command::ReadBlob {
            reference: reference.to_owned(),
        }
    } else if reference.starts_with(protocol::attachments::ATTACHMENT_SCHEME) {
