...
use crate::wire::MAX_WIRE_MESSAGE_BYTES;
use crate::wire::{
...
use protocol::model::{AgentSession, Project, ProviderKind, SessionStatus};
    ...
    fn dispatch_runtime(
        ...
        loop {
            ...
            let worker = std::thread::Builder::new()
                .name(format!("tide-daemon-runtime-{session_id}"))
                .spawn(move || {
                    run_runtime_mailbox(
                        session_id,
                        mailbox_id,
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                std::thread::Builder::new()
                    .name("tide-daemon-connection".into())
                    .spawn(move || {
                        let _connection_permit = connection_permit;
                        if let Err(error) =
                            handle_connection(stream, &token, dispatcher, hub, shutdown, &options)
                        {
                            eprintln!("tide-daemon connection ended: {error:#}");
                        }
                    })
                    .context("could not start Tide daemon connection thread")?;
            }
    ...
    'connection: while !shutdown.load(Ordering::Acquire) {
        ...
        match socket.read() {
            Ok(Message::Text(text)) => match serde_json::from_str(text.as_ref()) {
                ...
                Err(error) => {
                    eprintln!("tide-daemon ignored invalid message: {error}");
                }
            },
            ...
            Err(error) => return Err(error).context("Tide daemon WebSocket failed"),
        }
    }
...
mod tests {
    use super::*;
    use client::{DaemonClient, DaemonSupervisor};
    use crossbeam_channel::{RecvTimeoutError, bounded};
    use protocol::model::{AgentSession, ProviderKind};
    use protocol::{DaemonSettings, WireDriverStartOptions};
    use serde_json::json;
        ...
        fn handle(&self, request: Request, _events: EventSink) -> anyhow::Result<ResponsePayload> {
            match request.command {
                ...
                Command::LoadTaskState => Ok(ResponsePayload::TaskState {
                    ...
                    projectless_root: Some(PathBuf::from("/tmp/.tide/projects")),
                }),
                _ => Ok(ResponsePayload::Ack),
            }
        }
    ...
    fn websocket_task_state_changes_reach_another_client() {
        ...
        let second = AgentSession::new(Uuid::new_v4(), ProviderKind::Tide);
    ...
    fn browser_origins_are_denied_unless_explicitly_allowed() {
        let request = HandshakeRequest::builder()
            .uri("/v1")
            .header(ORIGIN, "https://app.tide.test")
            .body(())
            .unwrap();
        ...
        let allowed = HashSet::from(["https://app.tide.test".to_owned()]);
    ...
    struct BlockingSourcesBackend {
        scan_started: Sender<()>,
        release_scan: Receiver<()>,
    }

    impl Backend for BlockingSourcesBackend {
        fn handle(&self, request: Request, _: EventSink) -> anyhow::Result<ResponsePayload> {
            if matches!(request.command, Command::SourcesList) {
                self.scan_started.send(()).unwrap();
                self.release_scan.recv().unwrap();
            }
            Ok(ResponsePayload::Ack)
        }
    }
    ...
    fn slow_background_command_does_not_block_session_hydration() {
        ...
        let (scan_started, scan_started_rx) = bounded(1);
        let (release_scan, release_scan_rx) = bounded(1);
        let backend: Arc<dyn Backend> = Arc::new(BlockingSourcesBackend {
            scan_started,
            release_scan: release_scan_rx,
        });
        let hub = Arc::new(Hub::default());
        ...
        assert!(matches!(
            response_rx.recv_timeout(Duration::from_secs(1)).unwrap(),
            ServerMessage::Response { request_id, .. } if request_id == scan_id
        ));
    }
