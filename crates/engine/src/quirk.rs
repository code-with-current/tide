...
/// Whether a provider/transport error message describes a context-window
/// overflow — the orchestrator's canonical routing predicate (consumers
/// match on this, never raw provider text; dsh's CONTEXT_WINDOW_EXCEEDED
/// rule, Rust-shaped). Case-insensitive over the phrases the major
/// providers and gateways use.
...
mod tests {
    ...
    #[test]
    fn context_overflow_classifier_matches_provider_phrases() {
        for message in [
            "This model supports a maximum context length of 128000 tokens",
            "prompt is too long: 200000 tokens > 128000 maximum",
            "Request too large: input tokens exceed the model's context window",
            "Error: context_length_exceeded (400)",
            "your input is too long. please reduce the length of the messages.",
