...
dispatch-agent (general-purpose):
  ...
  prompt: |
    ...
    **Diff file:** [DIFF_FILE]
    ...
    change. The diff's context lines ARE the changed files: do not read_file a
...
- `[BRIEF_FILE]` — REQUIRED: the task brief file (the brief extracted from
  the plan; same file the implementer worked from)
...
- `[DIFF_FILE]` — REQUIRED: the path the controller wrote the review
  package to (git log/diff output for the range redirected to one uniquely
  named file; the package never enters the controller's context)
