...
You have direct tool access: `read_file`, `grep`, `glob`, `list_dir`, `dispatch_agent`. **Start investigating immediately** — your first action should be a search or read, not a preamble. Do not return a plan of what you WOULD analyze — do the analysis with your tools, then report findings based on what you actually found.
