...
const TASK_NOTIFICATION_TAG_PREFIX: &str = "tide-task:";
...
enum ModelPickerTab {
    Favorites,
    /// One rail entry per configured tide provider (its config id).
    TideProvider(SharedString),
}
...
enum SettingsPage {
    General,
    /// Tide's own providers screen — credentials, models, the add wizard.
    Tide,
...
impl SettingsPage {
    /// Computer Use's helper only exists on macOS, so other platforms keep the
    /// page out of the Settings sidebar and the command palette. Keeping this
    /// decision on the page itself makes both use the same gate.
...
enum RightPanelSurface {
    ...
    /// The session's dispatched sub-agents, one row each — tide's agents
    /// tab. Single-instance like Files/Diff: opening it again reuses the
    /// tab and re-renders the live list.
    Agents,
    Files,
    /// The session's git panel: working-tree changes and history, tide's
    /// git tab. Single-instance like Files: opening it again reuses the
    /// tab and refreshes.
    Git,
    File(String),
}
...
/// Tide's runtime map yet. Its event receiver safely buffers early events.
...
/// into [`Tide`] under its own view identity.
///
...
struct TidePane {
    ...
    content: fn(&mut Tide, &mut Window, &mut Context<Tide>) -> AnyElement,
}
...
impl TidePane {
    ...
    fn bind(&mut self, tide: &Entity<Tide>, cx: &mut Context<Self>) {
        self.tide = Some(tide.downgrade());
        cx.observe(tide, |_, tide, cx| {
            ...
            if !tide.read(cx).panels_sliding() {
                cx.notify();
            }
        })
        .detach();
...
impl Render for TidePane {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let Some(tide) = self.tide.as_ref().and_then(WeakEntity::upgrade) else {
            return gpui::div().into_any_element();
        };
        let content = self.content;
        tide.update(cx, |tide, cx| content(tide, window, cx))
    }
}
...
pub struct Tide {
    ...
    /// legacy pane keeps its existing test coverage.
    pub(crate) timeline_v2: bool,
    /// View state owned by the v2 pane alone; never shared with the legacy
    /// transcript's state.
    ...
    /// The composer's background-jobs indicator popup (stage 6): `hovered`
    /// pins the popup while the pointer is inside the indicator or the
    /// popup itself; `pinned` is the keyboard toggle (enter/space to open,
    ...
    /// Layout width the inspector column consumed in the last frame — card
    ...
    /// Unified diff rows for the Git panel's sub-views, virtualized so a
    /// large generated patch stays proportional to what is on screen.
    git_panel_diff_list_state: ListState,
    git_panel_diff_scrollbar: Rc<ScrollbarState>,
    /// Selection spans and visible glyph geometry for the Git panel's diff
    /// sub-views. Kept separate from the transcript because both surfaces
    /// paint at once.
    git_panel_diff_selection: TranscriptSelection,
    ...
    /// lane-graph painter's scroll source.
    git_panel_history_list_state: ListState,
    git_panel_history_scrollbar: Rc<ScrollbarState>,
    ...
    /// URLs a browser tab must navigate to on its first render. Browser views
    /// are created by the surface's renderer (they need the `Window`), so a
    ...
    /// Same sharing for mermaid cards: the renderer only knows a diagram was
    /// asked for, this callback knows the browser surface to render it in.
    markdown_mermaid_handler: md::render::MermaidHandler,
    /// The inline twin of that sharing: the renderer's mermaid host asks this
    /// closure for the element that renders a diagram in place, and this side
    /// shows the cached PNG fetched from the hosted renderer in
    /// [`mermaid_images`].
    markdown_mermaid_host: md::render::MermaidHost,
    ...
    /// Cached islands of the root view; see [`TidePane`].
    sidebar_pane: Entity<TidePane>,
    transcript_pane: Entity<TidePane>,
    right_panel_pane: Entity<TidePane>,
    inspector_pane: Entity<TidePane>,
...
fn migrate_legacy_projectless_projects(
    ...
    for index in legacy_indices {
        ...
        let response =
            workspace.request(client::WorkspaceOperation::MigrateProjectlessWorkspace { path });
        let cwd = match response {
            Ok(client::WorkspaceResult::ProjectlessWorkspace { cwd }) => cwd,
            Ok(_) => {
                return (
                    changed,
                    Some(anyhow::anyhow!(
...
impl Tide {
    ...
    pub fn new(
        ...
        window.set_rem_size(px(client::persistence::sanitized_ui_font_size(
            state.ui_font_size,
        )));
        ...
        let sidebar_pane = TidePane::new(Tide::sidebar_pane_content, cx);
        let transcript_pane = TidePane::new(Tide::transcript_pane_content, cx);
        let right_panel_pane = TidePane::new(Tide::right_panel_pane_content, cx);
        let inspector_pane = TidePane::new(Tide::inspector_pane_content, cx);
        let workspace_client = client::WorkspaceClient::new(daemon.client());
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            std::thread::Builder::new()
                .name("tide-computer-permission-probe".into())
                .spawn(move || {
                    let result = match daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        client::Command::ProbeComputerPermissions { prompt: false },
                    ) {
        ...
        let model_picker_tab = ModelPickerTab::Favorites;
        ...
        let entity = cx.new(|cx| {
            ...
            cx.subscribe(
                ...
                |this: &mut Self, _, event: &ChatComposerEvent, cx| match event {
                    ...
                    // Clipboard images and Finder file copies are attachment
                    // payloads, not text paths. The field owns representation
                    // priority; Tide owns durable staging and composer/session
                    // state.
                    ChatComposerEvent::MediaPasted(entries) => {
                        this.stage_pasted_attachments(entries.clone(), cx);
                    }
                },
            )
            .detach();

            // Input-drop tracing: records every keystroke that survives
            // dispatch unhandled (the ones the platform IME should turn into
            // text). Debug builds always trace; release builds under
            // `TIDE_INPUT_TRACE=1` — see `input::trace_path`.
            cx.observe_keystrokes(
                |_: &mut Self, event: &KeystrokeEvent, _, _: &mut Context<Self>| {
                    if !crate::input::input_trace_enabled() {
                        return;
                    }
                    crate::input::debug_log_input(
                        "keystroke_unhandled",
                        "",
                        &format!(
                            "key={:?} char={:?} mods={:?} action={:?}",
                            event.keystroke.key,
            ...
            let markdown_link_handler: md::render::LinkHandler = {
                ...
                Rc::new(move |target, _, cx| {
                    let handled = tide
                        .update(cx, |tide, cx| tide.open_transcript_link(target, cx))
                        .unwrap_or(false);
                    if !handled {
                        cx.open_url(target);
                    }
            ...
            // The mermaid twin of the link handler: the card's full-size
            // button hands over the diagram source, and the app owns the
            // panel surface it renders in. No fallback — without an app
            // entity there is no browser to open.
            let markdown_mermaid_handler: md::render::MermaidHandler = {
                let tide = cx.entity().downgrade();
                Rc::new(move |source, _, cx| {
                    let _ = tide.update(cx, |tide, cx| tide.open_mermaid_diagram(source, cx));
                })
            };

            // The inline mermaid host: every rendered diagram block asks for
            // the element that shows it in place. The hosted mermaid.ink
            // renderer's PNG lands in the app's cache folder and the card
            // shows that image; until the fetch completes — or for as long
            // as it keeps missing — the host declines and the engine's
            // source preview stands in.
            let markdown_mermaid_host: md::render::MermaidHost = {
                let tide = cx.entity().downgrade();
                Rc::new(move |source, window, cx| {
                    mermaid_images::host_element(&tide, source, window, cx)
                })
            };

            // Tests construct without a controlled environment, so the pane
            // flag pins to the legacy transcript there; everywhere else the
            // env override beats the debug-on / release-off default.
            let timeline_v2 = if cfg!(test) {
                false
            } else {
                timeline_v2_enabled(
                    std::env::var("TIDE_TIMELINE_V2").ok().as_deref(),
            ...
            Self {
                ...
                rag_settings: crate::app::rag_settings::RagSettingsPanel::new(),
                ...
                git_panel_diff_list_state: ListState::new(0, ListAlignment::Top, px(512.0)),
                git_panel_diff_scrollbar: ScrollbarState::new(),
                git_panel_diff_selection: TranscriptSelection::default(),
                git_panel_changes_rows: RefCell::new(Vec::new()),
                git_panel_changes_list_state: ListState::new(0, ListAlignment::Top, px(180.0))
                ...
                markdown_mermaid_handler,
                markdown_mermaid_host,
        ...
        entity.update(cx, |this, cx| {
            ...
            // Load the tide provider catalog after resolving the user's
            // login-shell environment off-thread, so the model picker and the
            // Providers page open onto data instead of an empty rail.
            this.tide_load_providers();
            // The Git settings screen renders from the cached snapshot, so it
            // is only as slow to open as the first fetch.
            if !this.git_settings.loaded {
                this.git_load_snapshot();
            }
