...
fn attach_changed_files(session: &mut AgentSession, files: Vec<CheckpointFile>) {
    ...
    turn.checkpoint = Some(Checkpoint {
        ...
        git_ref: format!("refs/tide/test-turn-{}", turn.turn_count),
...
fn remote_task_catalog_adds_web_tasks_without_replacing_hydrated_detail() {
    ...
    let mut local = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    let mut web_task = AgentSession::new(project_id, ProviderKind::Tide).list_projection();
...
#[test]
fn whitespace_mentions_are_quoted_into_one_token() {
    // A path with spaces must survive as one @mention token for the
    // provider-side scanner — one quoted span, not one word per space.
    let mentions = vec!["/tmp/shot 2.png".to_owned()];
    assert_eq!(
...
fn task_notification_tags_route_to_the_corresponding_task() {
    ...
    assert_eq!(task_id_from_notification_tag("tide-task:not-a-uuid"), None);
...
fn conversation_navigation_preview_uses_each_prompt_and_latest_response() {
    let project_id = Uuid::new_v4();
    let mut session = AgentSession::new(project_id, ProviderKind::Tide);
...
fn stream_parts_keep_targeting_the_running_session_after_selection_changes() {
    ...
    let mut running = AgentSession::new(project_id, ProviderKind::Tide);
    running.begin_turn("background task");
    running.status = SessionStatus::Working;
    let running_id = running.id;
    let visible = AgentSession::new(project_id, ProviderKind::Tide);
...
fn idle_reaping_releases_finished_sessions_but_never_a_running_turn() {
    ...
    let idle = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    let mut working = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    let mut waiting = AgentSession::new(project_id, ProviderKind::Tide);
    waiting.begin_turn("needs approval");
    waiting.status = SessionStatus::Waiting;
    assert!(!session_is_reapable(Some(&waiting), stale, false));

    let mut failed = AgentSession::new(project_id, ProviderKind::Tide);
...
fn the_row_fingerprint_moves_whenever_the_fold_does() {
    let mut base = AgentSession::new(Uuid::new_v4(), ProviderKind::Tide);
...
fn time_label_wakes_land_exactly_on_label_boundaries() {
    ...
    let mut fresher = AgentSession::new(Uuid::new_v4(), ProviderKind::Tide);
    ...
    let mut busy = AgentSession::new(Uuid::new_v4(), ProviderKind::Tide);
    busy.begin_turn("go");
...
fn settings_search_filters_pages_for_arrow_cycling() {
    ...
    // Each page stays reachable through one of its own keywords.
    assert_eq!(pages("delete"), vec![SettingsPage::Skills]);
    assert_eq!(pages("chart"), vec![SettingsPage::Usage]);
...
fn picker_rows_filter_by_tab_and_search_the_tide_catalog() {
    ...
    let model = |id: &str, name: &str, sub: &str| ProviderModel {
        id: id.to_owned(),
        name: name.to_owned(),
        sub_provider: Some(sub.to_owned()),
        is_default: false,
        reasoning_efforts: vec![ProviderModelOption::new("high", "HIGH")],
        default_reasoning_effort: None,
        service_tiers: Vec::new(),
        default_service_tier: None,
        context_windows: Vec::new(),
        default_context_window: Some("200000".to_owned()),
        price_label: None,
        vision: false,
    };
    let tide_models = [
        model("openrouter/claude-sonnet", "Claude Sonnet", "OpenRouter"),
        model("zai/glm-5", "GLM-5", "z.ai"),
    ];
    let favorites = [FavoriteModel {
        provider: ProviderKind::Tide,
        model: "zai/glm-5".into(),
    }];

    // A provider tab lists only that sub-provider's rows...
    let models = visible_picker_models(
        &tide_models,
        &favorites,
        ModelPickerTab::TideProvider("zai".into()),
        "",
    );
    assert_eq!(models.len(), 1);
    assert_eq!(models[0].1.id, "zai/glm-5");

    // ...favorites lists only starred rows...
    let models = visible_picker_models(&tide_models, &favorites, ModelPickerTab::Favorites, "");
    assert_eq!(models.len(), 1);
    assert_eq!(models[0].1.id, "zai/glm-5");

    // ...and search spans every configured provider.
    let models = visible_picker_models(
        &tide_models,
        &favorites,
        ModelPickerTab::TideProvider("openrouter".into()),
        "glm",
    );
    assert_eq!(models.len(), 1);
    assert_eq!(models[0].1.id, "zai/glm-5");
...
fn tab_cycle_walks_favorites_then_configured_tide_providers_in_order() {
    ...
    let rows = [row("anthropic"), row("openai")];

    // Favorites leads, then one tab per configured provider, in config order.
    assert_eq!(
        visible_picker_tabs(&rows),
        vec![
            ModelPickerTab::Favorites,
            ModelPickerTab::TideProvider("anthropic".into()),
            ModelPickerTab::TideProvider("openai".into()),
        ]
    );
}
...
fn model_picker_subtitle_deduplicates_the_provider_name() {
    ...
    // is the whole line.
    assert_eq!(
        model_picker_subtitle(ProviderKind::Tide, Some("OpenRouter"), None),
        "OpenRouter"
    );
    assert_eq!(
        model_picker_subtitle(ProviderKind::Tide, None, None),
        "Tide"
    );

    // A resolved row carries provider · context · price, tide's own selector.
    let model = ProviderModel {
        id: "openrouter/claude-sonnet".to_owned(),
        name: "Claude Sonnet".to_owned(),
        sub_provider: Some("OpenRouter".to_owned()),
        is_default: false,
        reasoning_efforts: vec![ProviderModelOption::new("high", "HIGH")],
        default_reasoning_effort: None,
        service_tiers: Vec::new(),
        ...
        context_windows: Vec::new(),
        default_context_window: Some("200000".to_owned()),
        price_label: Some("$3 / $15 per Mtok".to_owned()),
        vision: false,
    };
    assert_eq!(
