...
impl Tide {
    ...
    pub(super) fn open_settings_page(&mut self, page: SettingsPage, cx: &mut Context<Self>) {
        ...
        if page == SettingsPage::Git {
            // Stale-while-revalidate: the page renders the cached snapshot
            // immediately (warmed at startup), while a fresh one lands via
            // the ops drain. The loading placeholder only ever shows before
            // the first snapshot exists.
            self.git_load_snapshot();
        }
        if page == SettingsPage::Knowledge
            && let Some(project_id) = self.state.selected_project
        {
            self.rag_refresh(&project_id.to_string());
        }
        cx.notify();
    }
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .items_center()
                            .gap(px(8.0))
                            .child(div().size(px(9.0)).rounded_full().bg(color))
                            .child(
                                div()
                                    .flex_1()
                                    .min_w_0()
...
mod tests {
    ...
    fn project_paths_only_shorten_the_home_prefix() {
        ...
        assert_eq!(
            usage_project_path(Path::new("/Users/developer/dev/tide"), Some(home)),
            "~/dev/tide"
        );
        assert_eq!(usage_project_path(home, Some(home)), "~");
        assert_eq!(
            usage_project_path(Path::new("/Users/developer-2/tide"), Some(home)),
            "/Users/developer-2/tide"
        );
        assert_eq!(
            usage_project_path(Path::new("/Volumes/work/tide"), Some(home)),
            "/Volumes/work/tide"
        );
    }
