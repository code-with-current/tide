...
impl Tide {
    ...
    pub(super) fn open_commit_dialog(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(workspace) = self.selected_session().and_then(|session| {
        ...
        // The background-model override (Settings → General → Background
        // Tasks → Commit Message) wins when set; Tide generates through the
        ...
        // Like Tide's other deferred surfaces, the modal joins the dispatch
        ...
        cx.spawn(async move |tide, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    match workspace_client.request(client::WorkspaceOperation::InspectCommit {
                        cwd: workspace.clone(),
                    }) {
                        Ok(client::WorkspaceResult::CommitSnapshot { snapshot }) => Ok(snapshot),
            ...
            let _ = tide.update(cx, |tide, cx| {
                let Some(dialog) = tide.commit_dialog.as_mut().filter(|dialog| dialog.id == id)
                else {
                    return;
                };
            ...
            let result = cx
                ...
                .spawn(async move {
                    match workspace_client.request(
                        client::WorkspaceOperation::GenerateCommitMessage {
                ...
                .await;
            let _ = tide.update(cx, |tide, cx| {
                let current = tide.commit_operation.as_ref().is_some_and(|operation| {
                    operation.id == id
                ...
                match result {
                    Ok(message) => {
                        if let Some(operation) = tide.commit_operation.as_mut() {
                            operation.pending = CommitPending::Git(action);
                        }
                        if let Some(dialog) =
                            tide.commit_dialog.as_mut().filter(|dialog| dialog.id == id)
                        {
                            dialog
                                .message
                                .update(cx, |input, cx| input.set_content(message.clone(), cx));
                        }
                        tide.spawn_git_action(
                            id,
                            action,
                            workspace,
                            message,
            ...
            let result = cx
                ...
                .spawn(async move {
                    let operation = match action {
                        CommitAction::Commit => client::WorkspaceOperation::Commit {
                            cwd: operation_workspace.clone(),
                            message,
                            include_unstaged,
                            push: false,
                        },
                        CommitAction::CommitAndPush => client::WorkspaceOperation::Commit {
                            cwd: operation_workspace.clone(),
                            message,
                            include_unstaged,
                            push: true,
                        },
                        CommitAction::Push => client::WorkspaceOperation::Push {
                            cwd: operation_workspace.clone(),
                        },
                    };
                    ...
                    let snapshot = result.as_ref().err().and_then(|_| {
                        match workspace_client.request(client::WorkspaceOperation::InspectCommit {
                            ...
                            Ok(client::WorkspaceResult::CommitSnapshot { snapshot }) => {
                                Some(snapshot)
                            }
                            _ => None,
                        }
                ...
                .await;
            let focus = tide.update(cx, |tide, cx| {
                let current = tide.commit_operation.as_ref().is_some_and(|operation| {
                    operation.id == id
                        && operation.workspace == workspace
                        && operation.pending == CommitPending::Git(action)
                ...
                let focus = match result {
                    Ok(()) => {
                        let dialog_was_open = tide
                            .commit_dialog
                            .as_ref()
                            .is_some_and(|dialog| dialog.id == id);
                        if dialog_was_open {
                            tide.commit_dialog = None;
                        }
                        tide.show_success_toast(match action {
                        ...
                        dialog_was_open.then(|| tide.composer_focus(cx))
                    }
                    Err(error) => {
                        if let Some(dialog) =
                            tide.commit_dialog.as_mut().filter(|dialog| dialog.id == id)
    ...
    pub(super) fn render_commit_dialog(&mut self, cx: &mut Context<Self>) -> Option<AnyElement> {
            ...
            div()
                ...
                .when(include_enabled, |row| {
                    row.on_click(move |_, _, cx| {
                        let _ = click_weak.update(cx, |tide, cx| tide.toggle_include_unstaged(cx));
                    })
                    .on_key_down(move |event: &KeyDownEvent, _, cx| {
                        ...
                        {
                            let _ =
                                key_weak.update(cx, |tide, cx| tide.toggle_include_unstaged(cx));
                            cx.stop_propagation();
                        }
                    })
                })
        ...
        let card = div()
            ...
            .on_action(cx.listener(|tide, _: &ConfirmCommitDialog, window, cx| {
                tide.request_commit_action(CommitAction::Commit, window, cx)
            }))
            .on_action(cx.listener(|tide, _: &DismissCommitDialog, window, cx| {
                tide.close_commit_dialog(window, cx)
            }))
            .tab_group()
            .tab_stop(false)
            .w_full()
        ...
        let layer = div()
            ...
            .on_mouse_down(
                MouseButton::Left,
                cx.listener(|tide, _, window, cx| tide.close_commit_dialog(window, cx)),
            )
            .child(card);
    ...
    div()
        ...
        .when(enabled, |row| {
            row.on_click(move |_, window, cx| {
                let _ = click_weak.update(cx, |tide, cx| {
                    tide.request_commit_action(action, window, cx)
                });
            })
            .on_key_down(move |event: &KeyDownEvent, window, cx| {
                if !event.keystroke.modifiers.modified()
                    && matches!(event.keystroke.key.as_str(), "enter" | "space")
                {
                    let _ = key_weak.update(cx, |tide, cx| {
                        tide.request_commit_action(action, window, cx)
                    });
                    cx.stop_propagation();
                }
