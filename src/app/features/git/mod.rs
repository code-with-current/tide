...
use self::git_history::HistoryGraph;
...
impl Tide {
    ...
    pub(in crate::app) fn git_panel_visible(&self) -> bool {
        self.right_panel_visible
            && self
                .active_right_panel_surface()
                .is_some_and(|surface| matches!(surface, crate::app::RightPanelSurface::Git))
    }
    ...
    pub(in crate::app) fn select_git_panel_tab(
        &mut self,
        tab: GitPanelTab,
        cx: &mut Context<Self>,
    ) {
    ...
    #[allow(dead_code)]
    pub(in crate::app) fn git_panel_op_done(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn start_git_panel_timer(&mut self, cx: &mut Context<Self>) {
            ...
            loop {
                ...
                let tick = tide.update(cx, |tide, cx| {
                    ...
                    if !tide
                        ...
                        .iter()
                        .any(|surface| matches!(surface, crate::app::RightPanelSurface::Git))
                    {
                        return None;
                    }
    ...
    pub(in crate::app) fn reset_git_panel_for_workspace(&mut self) {
    ...
    pub(in crate::app) fn refresh_git_panel(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |tide, cx| {
            ...
            tide.update(cx, |tide, cx| {
                ...
                // The timer re-reads the repository every few seconds; an
                // identical pass must leave the panel untouched — overwriting
                // the queries and resetting the list states would scroll the
                // file and history lists back to the top on every tick.
                let mut changed = false;
                if let Some(changes) = status {
                    if !matches!(&tide.git_panel.status, Query::Ready(old) if **old == changes) {
                        tide.git_panel.status = Query::Ready(Arc::new(changes));
                        changed = true;
                    }
                }
                if let Some(conflicts) = conflicts {
                    if !matches!(&tide.git_panel.conflicts, Query::Ready(old) if **old == conflicts)
                    {
                ...
                if let Some(info) = branch_info {
                    if !matches!(&tide.git_panel.branch_info, Query::Ready(old) if **old == info) {
                        tide.git_panel.branch_info = Query::Ready(Arc::new(info));
                        changed = true;
                    }
                }
                if tide.git_panel.ahead_behind != ahead_behind {
                    tide.git_panel.ahead_behind = ahead_behind;
                    changed = true;
                }
                if let Some(stashes) = stashes {
                    if !matches!(&tide.git_panel.stashes, Query::Ready(old) if **old == stashes) {
                        tide.git_panel.stashes = Query::Ready(Arc::new(stashes));
                        changed = true;
                    }
                }
                if let Some(identity) = identity {
                    if !matches!(&tide.git_panel.current_identity, Query::Ready(old) if **old == identity)
                    {
                ...
                if let Some(worktrees) = worktrees {
                    if !matches!(&tide.git_panel.worktrees, Query::Ready(old) if **old == worktrees)
                    {
                ...
                if tide.git_panel.trailer != trailer {
                    tide.git_panel.trailer = trailer;
                    changed = true;
                }
                if let Some(commits) = log {
                    if !matches!(&tide.git_panel.log, Query::Ready(old) if **old == commits) {
    ...
    pub(in crate::app) fn sync_git_panel_changes_rows(&mut self) {
    ...
    pub(in crate::app) fn toggle_git_panel_section(
        &mut self,
        section: GitFileSection,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn toggle_git_panel_directory(
        &mut self,
        key: String,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn set_git_panel_tree_mode(
        &mut self,
        tree_mode: bool,
        cx: &mut Context<Self>,
    ) {
    ...
    /// dropped.
    pub(in crate::app) fn run_git_panel_op(
        &mut self,
        label: &'static str,
    ...
    pub(in crate::app) fn toggle_worktree_removal(
        &mut self,
        path: PathBuf,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn remove_worktree(
        &mut self,
        entry: &PanelWorktree,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn worktree_session(&self, path: &Path) -> Option<&AgentSession> {
    ...
    pub(in crate::app) fn worktree_session_is_busy(&self, path: &Path) -> bool {
    ...
    pub(in crate::app) fn open_git_panel_file_diff(
        &mut self,
        path: String,
        staged: bool,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn close_git_panel_file_diff(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn expand_selected_file_diff_context(
        &mut self,
    ...
    pub(in crate::app) fn open_last_turn_review(
        &mut self,
        ...
        self.open_right_panel_surface(crate::app::RightPanelSurface::Git, cx);
        if !reuse {
    ...
    pub(in crate::app) fn close_last_turn_review(&mut self, cx: &mut Context<Self>) {
        self.git_panel.last_turn_review = None;
        cx.notify();
    }
    ...
    pub(in crate::app) fn refresh_last_turn_review(&mut self, cx: &mut Context<Self>) {
        let Some(session_id) = self.state.selected_session else {
            return;
        };
        let Some(cwd) = self.selected_workspace_path().map(Path::to_path_buf) else {
    ...
    pub(in crate::app) fn expand_last_turn_review_gap(
        &mut self,
        line_index: usize,
    ...
    pub(in crate::app) fn ensure_git_panel_commit_draft(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn git_panel_commit_draft(&self) -> Option<&CommitDraft> {
    ...
    pub(in crate::app) fn set_git_panel_identity(&mut self, profile_id: String) {
    ...
    pub(in crate::app) fn commit_invocation(&self) -> Option<crate::git_commit::AgentInvocation> {
    ...
    /// else the session's own provider. `None` means a CLI provider has no
    /// installed binary — the "agent unavailable" case.
    pub(in crate::app) fn background_invocation(
        &self,
    ...
    pub(in crate::app) fn generate_git_panel_commit_message(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn confirm_git_panel_commit(&mut self, cx: &mut Context<Self>) {
        let has_conflicts =
    ...
    pub(in crate::app) fn open_git_commit_detail(&mut self, sha: String, cx: &mut Context<Self>) {
        let Some(session_id) = self.state.selected_session else {
            return;
        };
        let Some(cwd) = self.selected_workspace_path().map(Path::to_path_buf) else {
    ...
    pub(in crate::app) fn close_git_commit_detail(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn select_git_commit(&mut self, sha: String, cx: &mut Context<Self>) {
        let known =
    ...
    pub(in crate::app) fn toggle_git_commit_file_diff(
        &mut self,
        path: String,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn set_history_action_stage(
        &mut self,
        stage: self::HistoryActionStage,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        if stage == self::HistoryActionStage::Branch && action.branch_input.is_none() {
    ...
    pub(in crate::app) fn close_history_row_action(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn submit_history_branch(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn run_git_history_checkout(
        &mut self,
        create: bool,
        branch: String,
    ...
    pub(in crate::app) fn run_git_panel_revert(&mut self, sha: String, cx: &mut Context<Self>) {
        let Some(session_id) = self.state.selected_session else {
            return;
        };
        let Some(cwd) = self.selected_workspace_path().map(Path::to_path_buf) else {
    ...
    pub(in crate::app) fn pop_git_panel_stash(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn run_git_panel_remote(
        &mut self,
        label: &'static str,
        fetch: bool,
        rebase: bool,
    ...
    pub(in crate::app) fn run_git_panel_push(&mut self, cx: &mut Context<Self>) {
        let Some(session_id) = self.state.selected_session else {
            return;
        };
        let Some(cwd) = self.selected_workspace_path().map(Path::to_path_buf) else {
...
pub(in crate::app) enum CommitModelSource {
...
pub(in crate::app) fn resolve_commit_model_source(
    reference: Option<&protocol::git_settings::ModelRefWire>,
