...
impl Tide {
    ...
    pub(in crate::app) fn toggle_disclosure(&mut self, id: &str, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn timeline_v2_begin_edit(
        &mut self,
    ...
    pub(in crate::app) fn timeline_v2_cancel_edit(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn timeline_v2_disarm_confirm(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn timeline_v2_send_edit(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
...
pub(in crate::app) fn message_index_row(
    state: &TranscriptV2,
    message_index: usize,
) -> Option<usize> {
    state
...
pub(in crate::app) fn render_list(
    tide: &mut Tide,
...
pub(in crate::app) fn render_jump_button(
    tide: &Tide,
    cx: &mut Context<Tide>,
) -> Option<AnyElement> {
...
pub(in crate::app) fn render_pending_question(
    tide: &Tide,
    cx: &mut Context<Tide>,
) -> Option<AnyElement> {
...
pub(in crate::app) fn render_pending_permission(
    tide: &Tide,
    cx: &mut Context<Tide>,
) -> Option<AnyElement> {
    // The switch flash owns the pane's tail too: no card over gray blocks.
    ...
    let attachments = message
        ...
        .map(|(index, attachment)| {
            ...
            let open_preview = image.as_ref().map(|image| {
                ...
                })) as crate::app::features::transcript::timeline_v2::parts::user_bubble::UserBubbleAction
            });
