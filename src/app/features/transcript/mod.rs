//! The transcript feature: message and activity presentation shared by the
//! legacy transcript view and timeline_v2.
use crate::app::CONTENT_MAX_WIDTH;
use crate::app::STREAM_REMEASURE_TAIL_ROWS;
use crate::app::Tide;
use crate::app::features::transcript::navigation_rail::TranscriptNavigationTurn;
...
use std::rc::Rc;
use uuid::Uuid;
pub(in crate::app) mod activity_diff;
pub(in crate::app) mod inspector;
pub(in crate::app) mod navigation_rail;
pub(in crate::app) mod timeline_v2;
pub(in crate::app) mod transcript_search;
pub(in crate::app) mod transcript_view;

pub(in crate::app) mod components;

use self::navigation_rail::{NavigationTurnOpening, navigation_turns};
...
impl Tide {
    ...
    pub(in crate::app) fn selected_transcript_row_kinds(&self) -> Vec<TranscriptRowKind> {
    ...
    pub(in crate::app) fn remeasure_transcript_rows(&self, range: Range<usize>) {
        if range.is_empty() {
            return;
        }
    ...
    pub(in crate::app) fn splice_transcript_rows(&self, splice: Option<(Range<usize>, usize)>) {
...
/// Resolve the legacy pane's rail turn openings — each user message's row from
/// the cached row kinds — then build the turns through the shared
/// [`navigation_rail::navigation_turns`].
pub(in crate::app) fn transcript_navigation_turns(
    ...
    for (turn_index, message_index) in user_message_indexes.iter().copied().enumerate() {
        if session.messages.get(message_index).is_none() {
            continue;
        }
        let row_index = loop {
    ...
    navigation_turns(session, &openings)
}
