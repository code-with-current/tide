use super::navigation_rail::{
    ConversationNavigationRailSnapshot, active_navigation_turn_index, should_show_navigation_rail,
};
use crate::app::ActivityScrollViewport;
use crate::app::FOLLOWUP_TURN_TOP_GAP;
...
use super::assistant_response_footer_index;
use super::format_working_elapsed;
use super::message_starts_followup_turn;
use super::response_row_turn_id;
use super::should_show_scroll_to_bottom;
use super::transcript_rests_at_tail;
use crate::CopySelection;
use crate::app::ACTIVITY_IMAGE_HEIGHT;
use crate::app::ACTIVITY_IMAGE_WIDTH;
use crate::app::AssistantMessageAction;
use crate::app::CONTENT_MAX_WIDTH;
use crate::app::RightPanelSurface;
use crate::app::StreamPhase;
use crate::app::Tide;
...
use crate::app::features::sessions::background_work::work_status_color;
use crate::app::features::sessions::background_work::work_status_label;
use crate::app::features::transcript::activity_diff;
use crate::app::features::transcript::message_opens_turn;
use crate::app::turn_fold_label;
use crate::md;
...
use crate::ui::menu::floating_menu;
use crate::ui::motion;
...
impl Tide {
    ...
    pub(in crate::app) fn transcript_control_focus(
        &self,
    ...
    pub(in crate::app) fn render_transcript(
        &mut self,
        window: &mut Window,
    ...
    pub(in crate::app) fn copy_selection_action(
        &mut self,
        _: &CopySelection,
        _: &mut Window,
        cx: &mut Context<Self>,
        ...
        // The review surface's text selection copy went with the Diff
        // surface; the Git panel's diff view will reintroduce it with its
        // own selection state.
    ...
    pub(in crate::app) fn selection_menu_handler(
        &self,
        cx: &mut App,
    ) -> md::render::SelectionMenuHandler {
    ...
    /// the wash.
    pub(in crate::app) fn selection_menu_card(&self, cx: &mut App) -> AnyElement {
        let handle = self.menu_handle("transcript-selection", cx);
        let selection = self.transcript_selection.clone();
...
impl Tide {
    ...
    pub(in crate::app) fn toggle_activities(
        &mut self,
        block_index: usize,
        current: bool,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn toggle_activity_item(
        &mut self,
        id: Uuid,
        current: bool,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn toggle_turn_fold(
        &mut self,
        turn_id: Uuid,
        expanded: bool,
        cx: &mut Context<Self>,
    ...
    fn prefetch_checkpoint_refs(&self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            let existing = cx
                .background_executor()
                .spawn(async move {
                    match workspace.request(client::WorkspaceOperation::SessionTurnRefs {
                        cwd: project_path,
                        session_id,
                    }) {
                        Ok(client::WorkspaceResult::TurnRefs { turn_counts }) => {
    ...
    pub(in crate::app) fn scaled_markdown_metrics(
        &self,
    ...
    pub(in crate::app) fn menu_handle(
        &self,
    ...
    pub(in crate::app) fn menu_handle_with(
        &self,
        id: impl Into<SharedString>,
        cx: &mut App,
    ...
    pub(in crate::app) fn transcript_row(
        &mut self,
        index: usize,
        window: &mut Window,
        cx: &mut Context<Self>,
                    ...
                    let view = matches!(message.role, MessageRole::User | MessageRole::Assistant)
                        .then(|| {
                            ...
                            let source = if message.role == MessageRole::User {
                                crate::app::features::transcript::components::message::user_mention_links(
    ...
    pub(in crate::app) fn toggle_changed_files(
        &mut self,
        turn_id: Uuid,
        expanded: bool,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn render_turn_fold_row(
        &self,
        turn_id: Uuid,
        theme: &Theme,
        cx: &mut Context<Self>,
    ...
    fn render_working_indicator_row(&self, theme: &Theme) -> AnyElement {
        ...
        div()
            ...
            .items_center()
            .gap(px(8.0))
            .child(crate::ui::dots::working_wave_dots(theme.text_tertiary))
            .child(
                div()
                    .text_size(sp(13.5))
                    .line_height(sp(18.0))
    ...
    pub(in crate::app) fn render_activities_row(
        &self,
                        ...
                        .when(!has_detail && reasoning.is_none(), |element| {
                            element
                                ...
                                .when(!activity.complete && !activity.failed, |element| {
                                    element.child(crate::ui::dots::pulse_dot(5.0, theme.accent))
                                })
                        })
                        .on_click(cx.listener(move |this, _, _, cx| {
                            if has_detail {
                ...
                && (!sections.is_empty() || !activity.image_urls.is_empty())
            {
                ...
                for section in sections {
                    ...
                    if let Some(label) = section_kind.label() {
                        ...
                        let copy_tide = cx.entity().downgrade();
...
fn activity_scroll_fade(
    ...
    crate::ui::scroll_fade::overlay(
        scroll,
        gpui::Axis::Vertical,
        match side {
            ActivityScrollFadeSide::Top => crate::ui::scroll_fade::ScrollFadeSide::Start,
            ActivityScrollFadeSide::Bottom => crate::ui::scroll_fade::ScrollFadeSide::End,
        },
        18.0,
        surface,
    )
...
/// Pure window arithmetic behind [`Tide::live_reasoning_window_start`]:
