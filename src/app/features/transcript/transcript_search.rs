...
pub(in crate::app) struct TranscriptSearch {
    open: bool,
...
impl Tide {
    pub(in crate::app) fn refresh_transcript_search_localized_text(
        &mut self,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn transcript_search_open(&self) -> bool {
    ...
    pub(in crate::app) fn open_transcript_search(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn close_transcript_search(
        &mut self,
        restore_focus: bool,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn reset_transcript_search_for_session(&mut self) {
    ...
    pub(in crate::app) fn navigate_transcript_search(
        &mut self,
        backwards: bool,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn transcript_search_highlights(
        &self,
        message_index: usize,
    ...
    pub(in crate::app) fn apply_pending_transcript_search_reveal(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn render_transcript_search_bar(
        &self,
...
mod tests {
    ...
    fn literal_find_is_case_insensitive_and_unicode_safe() {
        let regex = literal_find_regex("tide");
        let (matches, limited) =
            md::render::markdown_search_matches("Tide **tide** TIDE", &regex, 20);
        assert!(!limited);
        assert_eq!(
            matches
                .iter()
