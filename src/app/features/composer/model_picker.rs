...
use crate::ui::menu::SelectNextTab;
use crate::ui::menu::SelectPreviousEntry;
use crate::ui::menu::SelectPreviousTab;
...
pub(in crate::app) type ModelPickerSelect =
    Rc<dyn Fn(&mut Tide, ProviderKind, String, &mut Context<Tide>)>;

/// What the optional reset entry ("use the session's model") does: clear the
/// stored override at this call site.
pub(in crate::app) type ModelPickerClear = Rc<dyn Fn(&mut Tide, &mut Context<Tide>)>;
...
#[derive(Clone)]
pub(in crate::app) struct ModelPickerConfig {
...
impl Tide {
    ...
    pub(in crate::app) fn render_model_picker<E>(
        &self,
