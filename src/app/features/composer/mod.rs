...
use crate::app::BRANCH_PICKER_MENU_ID;
use crate::app::CONTENT_MAX_WIDTH;
use crate::app::ComposerAttachment;
use crate::app::ComposerSubmission;
use crate::app::MODEL_PICKER_MENU_ID;
use crate::app::ModelPickerTab;
use crate::app::PendingUserInput;
use crate::app::RemoteImageState;
use crate::app::SettingsPage;
use crate::app::Tide;
use crate::app::image_preview;
use crate::app::traits_choice;
use crate::computer_use::ComputerUsePhase;
use crate::computer_use::PendingComputerApproval;
...
use std::sync::Arc;
use uuid::Uuid;
pub(in crate::app) mod autocomplete;
pub(in crate::app) mod chat_composer;
pub(in crate::app) mod drafts;
pub(in crate::app) mod model_picker;

use self::model_picker::ModelPickerConfig;
...
impl Tide {
    ...
    pub(in crate::app) fn render_permission(&self, cx: &mut Context<Self>) -> Option<Div> {
    ...
    pub(in crate::app) fn render_computer_use_overlay(
        &self,
        cx: &mut Context<Self>,
    ) -> Option<Div> {
        let previews = self
    ...
    pub(in crate::app) fn tide_provider_rail_rows(
        &self,
    ...
    pub(in crate::app) fn render_model_traits_control(
        &self,
        cx: &mut Context<Self>,
    ) -> Option<AnyElement> {
        let theme = Theme::current(cx);
    ...
    pub(in crate::app) fn render_interaction_mode_control(
        &self,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        let theme = Theme::current(cx);
    ...
    /// surface.
    pub(in crate::app) fn render_goal_control(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        let session = self.selected_session()?;
        let goal = session.thread_goal.as_ref()?;
        let session_id = session.id;
        let theme = Theme::current(cx);
        let color = crate::app::goal_dialog::goal_status_color(goal.status, &theme);
        ...
        let label = crate::app::goal_dialog::goal_chip_label(goal, live_elapsed_seconds);
    ...
    pub(in crate::app) fn stage_dropped_files(
        &mut self,
        paths: &ExternalPaths,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn stage_pasted_text(&mut self, text: String, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn render_queued_messages(&self, cx: &mut Context<Self>) -> Option<Div> {
    ...
    pub(in crate::app) fn render_composer_todo(&self, cx: &mut Context<Self>) -> Option<Div> {
        let session = self.selected_session()?;
    ...
    pub(in crate::app) fn render_workspace_footer(
        &mut self,
        cx: &mut Context<Self>,
        chips_align: MenuAlign,
    ) -> Div {
...
pub(in crate::app) const MAX_PICKER_BRANCHES: usize = 5;
...
pub(in crate::app) fn latest_todo_plan(session: &AgentSession) -> Option<Vec<(TodoState, String)>> {
    session
        .transcript_blocks
        .iter()
        .rev()
...
pub(in crate::app) fn pasted_text_attachment_name(
...
/// not navigate.
pub(in crate::app) fn next_picker_highlight(
...
pub(in crate::app) fn visible_picker_tabs(
...
pub(in crate::app) fn model_picker_empty_state(
    theme: &Theme,
    focus: &FocusHandle,
...
/// own selector.
pub(in crate::app) fn model_picker_subtitle(
    ...
    let context = crate::app::tide_wizard::format_context(
        model
...
pub(in crate::app) fn visible_picker_models(
