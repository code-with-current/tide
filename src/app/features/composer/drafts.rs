...
impl Tide {
    pub(in crate::app) fn selected_composer_draft_key(
        &self,
    ...
    pub(in crate::app) fn capture_current_composer_draft(&mut self, cx: &App) -> bool {
    ...
    pub(in crate::app) fn capture_and_save_current_composer_draft(
        &mut self,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn select_project_from_composer(
        &mut self,
        project_id: Uuid,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn create_projectless_session_from_composer(
        &mut self,
        cx: &mut Context<Self>,
    ) {
        let source = self.selected_composer_draft_key();
    ...
    pub(in crate::app) fn discard_current_composer_draft(&mut self, cx: &mut Context<Self>) {
        let Some(key) = self.selected_composer_draft_key() else {
            return;
        };
    ...
    pub(in crate::app) fn remove_composer_draft(
        &mut self,
    ...
    pub(in crate::app) fn restore_selected_composer_draft(&mut self, cx: &mut Context<Self>) {
        let draft = self
    ...
    pub(in crate::app) fn schedule_composer_draft_save(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |tide, cx| {
            cx.background_executor()
                ...
                .await;
            let Some((store, drafts)) = tide
                .update(cx, |tide, cx| {
                    if tide.composer_draft_save_generation != generation {
                        return None;
                    }
                    tide.capture_current_composer_draft(cx);
                    Some((
                        tide.composer_draft_store.clone(),
                        tide.composer_drafts.clone(),
                    ))
                })
                .ok()
                .flatten()
            ...
            if let Err(error) = result {
                let _ = tide.update(cx, |tide, cx| {
                    tide.show_toast(tr!("errors.save_local_state", error = error));
                    cx.notify();
                });
            }
        })
