use crate::CancelTurn;
use crate::FocusComposer;
use crate::NavigateBack;
use crate::NavigateForward;
use crate::NewProject;
use crate::NewSession;
use crate::OpenSettings;
use crate::ToggleFpsCounter;
use crate::ToggleModelPicker;
use crate::ToggleRightPanel;
use crate::ToggleSidebar;
use crate::app::ESCAPE_STOP_CONFIRMATION_TIMEOUT;
use crate::app::EscapeStopPress;
use crate::app::EscapeStopTarget;
use crate::app::FILE_TREE_MIN_WIDTH;
use crate::app::MAIN_PANEL_MIN_WIDTH;
use crate::app::MODEL_PICKER_MENU_ID;
use crate::app::ModelPickerTab;
...
use crate::app::fitted_panel_widths;
use crate::app::persisted_window_state;
use crate::app::{
    FILE_EDITOR_MIN_WIDTH, FILE_TREE_MAX_WIDTH, MAX_CACHED_MESSAGE_SOURCE_BYTES, PanelResizeDrag,
    PendingSessionActivation, RIGHT_PANEL_MAX_WIDTH, SIDEBAR_MAX_WIDTH,
};
use crate::md;
...
impl Tide {
    ...
    pub(in crate::app) fn ensure_session_loaded(
        &mut self,
        session_id: Uuid,
        cx: &mut Context<Self>,
    ) {
        ...
        cx.spawn(async move |tide, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    match client::persistence::hydrate_session(&daemon, session_id)? {
            ...
            let _ = tide.update(cx, |tide, cx| {
                ...
                match result {
                    Ok(session) => {
                        ...
                        if pending.is_some() {
                            tide.pending_session_activation = None;
                        }
                        if replaced && let Some(pending) = pending {
                            tide.finish_session_activation(session_id, pending.transition, cx);
                        } else if tide.state.selected_session == Some(session_id) {
                            tide.reset_visible_state();
                            tide.reset_transcript_rows(tide.transcript_row_count());
                    ...
                    Err(error) => {
                        ...
                        tide.show_toast(tr!("errors.open_session", error = error));
                    }
                }
                cx.notify();
            });
    ...
    fn activate_session(&mut self, session_id: Uuid, cx: &mut Context<Self>) {
        ...
        // The inspector's Memory & RAG section reads the loaded project
        // status; a session switch to another project loads that one.
        if let Some(project_id) = self
            ...
            && self
                ...
                .status
                .as_ref()
                .is_none_or(|status| status.project_id != project_id)
        {
    ...
    pub(in crate::app) fn invalidate_workspace_queries(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn remove_session(&mut self, session_id: Uuid, cx: &mut Context<Self>) {
        ...
        if let Some(project_path) = project_path {
            ...
            cx.background_executor()
                .spawn(async move {
                    let _ = workspace.request(client::WorkspaceOperation::DeleteSessionRefs {
                        cwd: project_path,
                        session_id,
                    });
                })
    ...
    pub(in crate::app) fn new_session_action(
        &mut self,
        _: &NewSession,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn new_project_action(
        &mut self,
        _: &NewProject,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn open_settings_action(
        &mut self,
        _: &OpenSettings,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        // Warm the Usage page's report while the user is still on
    ...
    pub(in crate::app) fn toggle_sidebar_action(
        &mut self,
        _: &ToggleSidebar,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn toggle_right_panel_action(
        &mut self,
        _: &ToggleRightPanel,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn toggle_fps_counter_action(
        &mut self,
        _: &ToggleFpsCounter,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn set_sidebar_visible(&mut self, visible: bool, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn set_right_panel_visible(
        &mut self,
        visible: bool,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn persist_panel_layout(&mut self) {
    ...
    pub(in crate::app) fn capture_window_state(&mut self, window: &Window, cx: &App) {
    ...
    /// [`Tide::sidebar_rendered_width`] / [`Tide::right_panel_rendered_width`].
    pub(in crate::app) fn effective_panel_widths(&self, window: &Window) -> (f32, f32) {
        fitted_panel_widths(
    ...
    pub(in crate::app) fn begin_panel_resize(
        &mut self,
    ...
    pub(in crate::app) fn resize_panel_mouse_move(
        &mut self,
    ...
    pub(in crate::app) fn finish_panel_resize(
        &mut self,
        event: &MouseUpEvent,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn navigate_back_action(
        &mut self,
        _: &NavigateBack,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn navigate_forward_action(
        &mut self,
        _: &NavigateForward,
        _: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn navigation_mouse_down(
        &mut self,
        event: &MouseDownEvent,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn focus_composer_action(
        &mut self,
        _: &FocusComposer,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn cancel_turn_action(
        &mut self,
        _: &CancelTurn,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn reset_visible_state(&mut self) {
    ...
    pub(in crate::app) fn choose_model(
        ...
        if let Some(session) = self.selected_session_mut() {
            ...
            self.state.last_service_tier = service_tier;
            self.state.last_context_window = context_window;
            // A different sub-provider owns a different catalog; only a model
            // change can be applied in session, so re-apply the options to the
    ...
    pub(in crate::app) fn toggle_model_picker_action(
        &mut self,
        _: &ToggleModelPicker,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    /// The tide provider list refreshes on open and after edits, so tabs only
    /// ever switch between loaded lists.
    pub(in crate::app) fn select_model_picker_tab(
        &mut self,
        tab: ModelPickerTab,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn set_interaction_mode(
        &mut self,
    ...
    pub(in crate::app) fn cancel_turn(&mut self, cx: &mut Context<Self>) {
        ...
        // Stopping the tide runtime ends the turn; the daemon-side runtime is
        // dropped and the next prompt starts a fresh one from the resume
        // cursor. Live detached work still owns its own lifetime.
        let retain_runtime = self.session_has_live_detached_work(session_id)
        ...
        // A provider runtime owns its Tide JavaScript REPL and Computer Use
        ...
        // remain resident so Tide can keep observing and stopping it.
    ...
    pub(in crate::app) fn dismiss_computer_use(&mut self, window_id: u32, cx: &mut Context<Self>) {
        if let Some(runtime) = self
            .state
            .selected_session
            .and_then(|session_id| self.runtimes.get_mut(&session_id))
        ...
        cx.spawn(async move |tide, cx| {
            let result = cx
                ...
                .spawn(async move {
                    match workspace.request(
                        client::WorkspaceOperation::CreateProjectlessWorkspace { prompt: None },
                    )? {
                        client::WorkspaceResult::ProjectlessWorkspace { cwd } => Ok(cwd),
            ...
            let _ = tide.update(cx, |tide, cx| match result {
                Ok(cwd) => {
                    ...
                    let project_id = project.id;
                    tide.state.projects.push(project);
                    tide.create_session_for(project_id, tide.state.last_provider, cx);
                }
                Err(error) => {
                    tide.show_toast(tr!("errors.create_projectless_task", error = error));
                    cx.notify();
                }
            });
        })
...
pub(in crate::app) fn effective_start_model(
...
mod tests {
    ...
    fn new_task_carries_the_current_tasks_access_mode() {
        let mut current = AgentSession::new(Uuid::new_v4(), ProviderKind::Tide);
    ...
    fn new_task_navigation_reuses_a_draft_from_the_current_project() {
        ...
        let draft = AgentSession::new(project_id, ProviderKind::Tide);
        let mut started = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    fn new_task_navigation_does_not_reopen_a_started_or_removed_draft() {
        let project_id = Uuid::new_v4();
        let mut draft = AgentSession::new(project_id, ProviderKind::Tide);
        let mut navigation = SessionNavigation::default();
        navigation.remember_new_task(draft.id);
