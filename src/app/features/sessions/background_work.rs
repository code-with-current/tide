use crate::app::BACKGROUND_WORK_TICK_INTERVAL;
use crate::app::features::git::GitPanelTab;
use crate::app::features::transcript::timeline_v2::rows::turn_item::TurnFooterUsage;
use gpui::prelude::*;
...
use crate::model::BackgroundWorkItem;
use crate::model::BackgroundWorkKey;
use crate::model::BackgroundWorkKind;
use crate::model::BackgroundWorkStatus;
use crate::model::SubagentBlock;
use crate::model::SubagentRun;
use crate::model::SubagentToolStatus;
use crate::model::unix_time;
use crate::model::unix_time_millis;
use crate::theme::Theme;
use crate::theme::sp;
use crate::ui::contain_scroll;
use crate::ui::icon;
...
use crate::ui::menu::toggle_popover;
use crate::ui::motion;
...
#[derive(Default)]
pub(in crate::app) struct BackgroundWorkRegistry {
...
impl BackgroundWorkRegistry {
    ...
    pub(in crate::app) fn action_job_state(&self, job_id: &str) -> Option<(bool, Option<u16>)> {
    ...
    pub(in crate::app) fn rehydrate_subagent_runs(&mut self, runs: &[SubagentRun]) {
        for run in runs {
    ...
    pub(in crate::app) fn rehydrate_subagent_runs_without_runtime(&mut self, runs: &[SubagentRun]) {
...
pub(in crate::app) fn work_status_label(status: BackgroundWorkStatus) -> String {
    match status {
...
pub(in crate::app) fn work_status_color(status: BackgroundWorkStatus, theme: Theme) -> Hsla {
    match status {
...
pub(in crate::app) fn work_kind_icon(kind: BackgroundWorkKind) -> &'static str {
    match kind {
...
impl Tide {
    pub(in crate::app) fn background_output_refresh_delay(&self) -> Option<Duration> {
    ...
    pub(in crate::app) fn observe_foreground_command_activity(
        &mut self,
        session_id: Uuid,
    ...
    pub(in crate::app) fn handle_background_work_event(
        &mut self,
        session_id: Uuid,
    ...
    pub(in crate::app) fn mark_background_work_lost(&mut self, session_id: Uuid) {
        if let Some(registry) = self.background_work.get_mut(&session_id) {
            registry.mark_live_lost();
        }
    }

    pub(in crate::app) fn settle_foreground_work(
        &mut self,
        session_id: Uuid,
        status: BackgroundWorkStatus,
    ) {
    ...
    pub(in crate::app) fn session_has_live_background_work(&self, session_id: Uuid) -> bool {
    ...
    pub(in crate::app) fn session_has_live_detached_work(&self, session_id: Uuid) -> bool {
        self.background_work
            .get(&session_id)
            .is_some_and(BackgroundWorkRegistry::has_live_detached)
    }

    pub(in crate::app) fn background_work_counts(&self, session_id: Uuid) -> (usize, usize) {
        self.background_work
            .get(&session_id)
    ...
    pub(in crate::app) fn background_work_for_activity(
        &self,
        session_id: Uuid,
        activity_id: &str,
    ...
    pub(in crate::app) fn maybe_refresh_background_work(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn stop_background_work(
        &mut self,
        session_id: Uuid,
    ...
    pub(in crate::app) fn open_background_work_surface(
        &mut self,
        session_id: Uuid,
        key: BackgroundWorkKey,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn selected_session_agents(&self) -> Vec<BackgroundWorkItem> {
        self.state
            .selected_session
    ...
    pub(in crate::app) fn selected_session_agents_count(&self) -> usize {
        self.state
            .selected_session
            .and_then(|session_id| self.background_work.get(&session_id))
    ...
    /// somewhere useful.
    pub(in crate::app) fn open_dispatch_activity(
        &mut self,
        dispatch_id: &str,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn render_background_work_summary(
        &self,
        cx: &mut Context<Self>,
    ) -> AnyElement {
    ...
    pub(in crate::app) fn detect_open_in_apps(&self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn render_background_work_surface(
        &self,
        key: &BackgroundWorkKey,
        cx: &mut Context<Self>,
    ) -> AnyElement {
...
use crate::app::features::transcript::timeline_v2::parts::reasoning_part::ReasoningMarkdown;
use crate::app::features::transcript::timeline_v2::parts::tool_part::{
    HEADER_GAP, HEADER_ICON, HEADER_LINE_HEIGHT, HEADER_PAD,
};
use crate::app::features::transcript::timeline_v2::parts::user_bubble::{
    CLAMP_MAX_HEIGHT, clamp_needed,
};
use crate::app::features::transcript::timeline_v2::rows::activity_group::{
    GroupToggle, render_activities,
};
use crate::app::features::transcript::timeline_v2::rows::changed_files::{
    render_changed_files, summarize_changes,
};
use crate::app::features::transcript::timeline_v2::rows::turn_item::render_turn_footer;
use crate::app::features::transcript::timeline_v2::rows::working_footer::render_working_footer;
use crate::app::features::transcript::timeline_v2::{
    TranscriptActions, tools_description, tools_dim, tools_title,
};
...
impl Tide {
    ...
    pub(in crate::app) fn render_right_panel_agents(&self, cx: &mut Context<Self>) -> AnyElement {
        let theme = Theme::current(cx);
        let session_id = self.state.selected_session;
        let registry = session_id.and_then(|session_id| self.background_work.get(&session_id));
...
impl Tide {
    ...
    /// work.
    pub(in crate::app) fn composer_jobs_popup_open(&self) -> bool {
        self.composer_jobs_popup_hovered || self.composer_jobs_popup_pinned
    }

    pub(in crate::app) fn set_composer_jobs_popup_hovered(
        &mut self,
        hovered: bool,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn toggle_composer_jobs_popup_pinned(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn render_header_jobs_pill(
        &self,
        cx: &mut Context<Self>,
    ) -> impl IntoElement {
        let empty = div();
    ...
    pub(in crate::app) fn render_composer_jobs_indicator(
        &self,
        cx: &mut Context<Self>,
    ) -> impl IntoElement {
        let empty = div();
