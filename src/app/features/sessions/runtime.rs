use super::streaming::pop_stream_batch;
use crate::app::BACKGROUND_WORK_REFRESH_INTERVAL;
use crate::app::ComposerSubmission;
use crate::app::DriverStartRequest;
use crate::app::EventPumpSchedule;
use crate::app::IDLE_SESSION_SWEEP_INTERVAL;
use crate::app::MessageEdit;
use crate::app::PendingCheckpointCapture;
use crate::app::PreparedDriver;
use crate::app::PreparedSubmission;
use crate::app::RemoteTaskStateSnapshot;
use crate::app::STREAM_FRAME_INTERVAL;
use crate::app::STREAM_SAVE_INTERVAL;
use crate::app::SessionRuntime;
use crate::app::Tide;
use crate::app::TranscriptAnchor;
use crate::app::UserMessageAction;
use crate::app::driver;
use crate::app::features::composer;
use crate::app::features::composer::chat_composer::ChatComposer;
use crate::app::features::composer::chat_composer::ChatComposerEvent;
use crate::app::inspector;
use crate::app::session_is_reapable;
use crate::app::signal_event_pump;
use crate::app::stream_delta_kind;
...
pub(in crate::app) fn session_has_active_provider_turn(session: &AgentSession) -> bool {
    session.is_busy()
        && session
            .turns
            .last()
...
/// drained.
pub(in crate::app) fn merge_remote_session_catalog(
...
impl Tide {
    pub(in crate::app) fn restart_task_state_sync(&self) {
    ...
    pub(in crate::app) fn apply_remote_task_state(
        &mut self,
    ...
    pub(in crate::app) fn start_runtime_attachment(
        &mut self,
        session_id: Uuid,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn selected_project(&self) -> Option<&Project> {
        let id = self.state.selected_project?;
        self.state.projects.iter().find(|project| project.id == id)
    }

    pub(in crate::app) fn selected_session(&self) -> Option<&AgentSession> {
    ...
    pub(in crate::app) fn finish_active_turn_with_analytics(
        &mut self,
        session_id: Uuid,
        status: TurnStatus,
        outcome: crate::analytics::TurnOutcome,
    ...
    pub(in crate::app) fn workspace_path_for_session<'a>(
        &'a self,
    ...
    pub(in crate::app) fn selected_workspace_path(&self) -> Option<&std::path::Path> {
        let session = self.selected_session()?;
        self.workspace_path_for_session(session)
    }

    /// Marks the session for the next save; see `PersistedState::session_mut`.
    pub(in crate::app) fn selected_session_mut(&mut self) -> Option<&mut AgentSession> {
        let id = self.state.selected_session?;
        self.state.session_mut(id)
    }

    pub(in crate::app) fn selected_runtime(&self) -> Option<&SessionRuntime> {
    ...
    pub(in crate::app) fn tide_models_from_providers(
    ...
    pub(in crate::app) fn model_for_session<'a>(
        &'a self,
        session: &'a AgentSession,
    ) -> Option<&'a str> {
        session
    ...
    pub(in crate::app) fn model_display_name(
        &self,
    ...
    pub(in crate::app) fn model_metadata_for_session(
        &self,
    ...
    pub(in crate::app) fn selected_transcript_blocks(&self) -> &[TranscriptBlock] {
    ...
    pub(in crate::app) fn save(&mut self) {
    ...
    pub(in crate::app) fn model_route_label(&self, model: Option<&str>) -> Option<String> {
        let model = model?;
        let candidate = self
            .tide_models
            .iter()
    ...
    pub(in crate::app) fn turn_branch_state(
        &self,
        session: &AgentSession,
        turn: &AgentTurn,
    ) -> (bool, bool) {
    ...
    pub(in crate::app) fn fork_session_from_response(
        &mut self,
        session_id: Uuid,
        turn_count: usize,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn defer_restore_composer_after_fork(
        &self,
        session_id: Uuid,
        prompt: String,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn begin_message_edit(
        &mut self,
    ...
    pub(in crate::app) fn cancel_message_edit(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn submit_message_edit(&mut self, cx: &mut Context<Self>) {
        let prompt = self
            .message_edit
            .as_ref()
    ...
    pub(in crate::app) fn session_options(&self, session: &AgentSession) -> SessionOptions {
        let model = session
            .model
            .clone()
    ...
    pub(in crate::app) fn reap_idle_sessions(&mut self) {
    ...
    pub(in crate::app) fn apply_session_options(
        &mut self,
        session_id: Uuid,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn start_goal_runtime(&mut self, session_id: Uuid, cx: &mut Context<Self>) {
        if self.runtimes.contains_key(&session_id)
    ...
    pub(in crate::app) fn submit_composer_submission(
        &mut self,
        submission: ComposerSubmission,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn steer_composer_submission(
        &mut self,
        submission: ComposerSubmission,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn session_can_steer(&self, session: &AgentSession) -> bool {
    ...
    pub(in crate::app) fn collect_runtime_events(runtime: &mut SessionRuntime) {
        while let Ok(event) = runtime.events.try_recv() {
            runtime.pending_events.push_back(event);
        }
    }

    pub(in crate::app) fn drain_event_pump(&mut self, cx: &mut Context<Self>) -> EventPumpSchedule {
    ...
    pub(in crate::app) fn drain_driver_events(&mut self, cx: &mut Context<Self>) -> bool {
...
impl Tide {
    ...
    pub(in crate::app) fn tide_dispatch(&self, command: client::Command) {
    ...
    pub(in crate::app) fn tide_load_providers(&mut self) {
        self.tide_dispatch(client::Command::TideProviders);
    }

    pub(in crate::app) fn tide_open_add_wizard(
        &mut self,
    ...
    pub(in crate::app) fn tide_open_custom_wizard(
        &mut self,
    ...
    pub(in crate::app) fn tide_open_edit_wizard(
        &mut self,
        provider_id: String,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(in crate::app) fn tide_close_wizard(&mut self, cx: &mut Context<Self>) {
        self.tide.wizard = None;
        cx.notify();
    }

    /// Choose-step tile click: prefill from the preset and move to Connect.
    pub(in crate::app) fn tide_choose_preset(
        &mut self,
    ...
    pub(in crate::app) fn tide_wizard_continue_connect(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn tide_zed_sign_in(&mut self, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_mut() else {
            return;
        };
    ...
    /// Org radio row click.
    pub(in crate::app) fn tide_zed_pick_org(&mut self, org_id: String, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_mut() else {
            return;
        };
    ...
    pub(in crate::app) fn tide_auto_detect(&mut self, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_ref() else {
            return;
        };
        let base_url = wizard.base_url.read(cx).content().trim().to_owned();
    ...
    pub(in crate::app) fn tide_set_style(&mut self, style: &'static str, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_mut() else {
            return;
        };
    ...
    pub(in crate::app) fn tide_wizard_step(
        &mut self,
    ...
    pub(in crate::app) fn tide_refresh_models(&mut self, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_ref() else {
            return;
        };
        let (api_style, base_url) = (
    ...
    pub(in crate::app) fn tide_set_all_models(&mut self, checked: bool, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_mut() else {
            return;
        };
    ...
    pub(in crate::app) fn tide_toggle_model(&mut self, model_id: &str, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_mut() else {
            return;
        };
    ...
    pub(in crate::app) fn tide_save_wizard(&mut self, cx: &mut Context<Self>) {
        let Some(wizard) = self.tide.wizard.as_ref() else {
            return;
        };
    ...
    pub(in crate::app) fn tide_delete_provider(&mut self, provider_id: String) {
        self.tide_dispatch(client::Command::TideDeleteProvider { provider_id });
    }

    pub(in crate::app) fn tide_toggle_enabled(&mut self, provider_id: String, enabled: bool) {
        self.tide_dispatch(client::Command::TideUpdateProvider {
            provider_id,
            name: None,
            api_style: None,
    ...
    pub(in crate::app) fn drain_tide_ops_events(&mut self, cx: &mut Context<Self>) -> bool {
        let mut changed = false;
    ...
    pub(in crate::app) fn git_dispatch(&self, command: client::Command) {
    ...
    pub(in crate::app) fn dispatch_update_project_settings(
        &mut self,
        project_id: Uuid,
        cx: &mut Context<Self>,
    ) {
    ...
    pub(in crate::app) fn git_load_snapshot(&mut self) {
    ...
    /// save indicator.
    pub(in crate::app) fn git_set_attribution(
        &mut self,
    ...
    pub(in crate::app) fn git_set_background_model(
        &mut self,
        task: &str,
    ...
    pub(in crate::app) fn git_new_profile(&mut self, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn git_edit_profile(&mut self, profile_id: &str, cx: &mut Context<Self>) {
    ...
    pub(in crate::app) fn git_delete_profile(&mut self, profile_id: String) {
    ...
    pub(in crate::app) fn git_import_open(&mut self, cx: &mut Context<Self>) {
    ...
    /// closes the loop.
    pub(in crate::app) fn git_set_project_identity(
        &mut self,
        project_path: String,
        profile_id: String,
    ) {
    ...
    pub(in crate::app) fn git_clear_project_identity(&mut self, project_path: String) {
    ...
    pub(in crate::app) fn git_disconnect_account(&mut self, login: String) {
    ...
    /// refreshes.
    pub(in crate::app) fn git_connect_gh(&mut self, login: String, cx: &mut Context<Self>) {
        self.git_settings.gh_connecting = Some(login.clone());
        self.git_dispatch(client::Command::GithubConnectFromGhCli { login });
        cx.notify();
    }

    pub(in crate::app) fn drain_git_ops_events(&mut self, cx: &mut Context<Self>) -> bool {
        let mut changed = false;
