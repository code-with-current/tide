...
use crate::app::ComputerUsePreview;
use crate::app::PendingUserInput;
use crate::app::SessionRuntime;
use crate::app::StreamDeltaKind;
use crate::app::StreamPhase;
use crate::app::Tide;
use crate::app::task_notification_tag;
...
use crate::model::ReasoningBlock;
...
use uuid::Uuid;
impl Tide {
    pub(in crate::app) fn finish_streaming_assistant(&mut self, session_id: Uuid) {
    ...
    pub(in crate::app) fn append_text_delta(
        &mut self,
        session_id: Uuid,
    ...
    pub(in crate::app) fn append_reasoning_delta(
        &mut self,
        session_id: Uuid,
        runtime: &mut SessionRuntime,
        delta: String,
    ...
    pub(in crate::app) fn update_activity(
        &mut self,
        session_id: Uuid,
        runtime: &mut SessionRuntime,
        item: ActivityItem,
        ...
        // A compaction card carries the session-level record the
        // inspector's Context Window section reads.
        let mut compacted = false;
        if let Some(session) = self.state.session_mut(session_id) {
            if item.kind == ActivityKind::Compact
                && let Some(record) = item.compaction
            {
                session.last_compaction = Some(record);
                compacted = true;
            }
            for block in session.transcript_blocks.iter_mut().rev() {
                ...
                if let Some(activity) = matching {
                    ...
                    if item.compaction.is_some() {
                        activity.compaction = item.compaction;
                    }
    ...
    pub(in crate::app) fn complete_turn_blocks(&mut self, session_id: Uuid) {
        if let Some(session) = self.state.session_mut(session_id) {
    ...
    pub(in crate::app) fn turn_has_assistant_message(&self, session_id: Uuid) -> bool {
        self.state
            .sessions
            .iter()
    ...
    pub(in crate::app) fn accepts_turn_output(&mut self, session_id: Uuid) -> bool {
    ...
    /// continue.
    pub(in crate::app) fn handle_driver_event(
        &mut self,
        session_id: Uuid,
        runtime: &mut SessionRuntime,
        event: DriverEvent,
        ...
        match event {
            ...
            DriverEvent::TurnStarted => {
                ...
                if let Some(session) = self.state.session_mut(session_id) {
                    if session.active_turn_id().is_some() {
                        ...
                        // a `/goal` began: the engine's start confirms it.
                        session.mark_active_turn_provider_started();
                        session.status = SessionStatus::Working;
                    } else {
                        // The tide engine starts turns on its own when an
                        // active goal comes up for pursuit while the session
                        // is idle. Give the turn a transcript home — there is
            ...
            // Plan meters were a CLI-provider surface; the tide transport has
            // no account plan to report, so the event carries nothing to show.
...
pub(in crate::app) fn session_accepts_turn_output(session: &mut AgentSession) -> bool {
...
pub(in crate::app) fn should_refresh_branch_after_activity(
...
pub(in crate::app) fn push_transcript_activity(
...
pub(in crate::app) fn stream_delta_kind(event: &DriverEvent) -> Option<StreamDeltaKind> {
    match event {
...
pub(in crate::app) fn stream_delta_text(
    event: &DriverEvent,
...
pub(in crate::app) fn compact_driver_error(error: &str) -> String {
...
pub(in crate::app) fn pop_stream_batch(
...
pub(in crate::app) fn append_text_delta_to_session(
