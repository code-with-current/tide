...
/// Projectless projects are the ones living at the hardcoded home location or
/// under a historic generated-workspace layout — classified relative to the
/// hoisted projectless root so render paths avoid per-row slot reads.
fn sidebar_project_is_projectless(project: &Project, projectless_root: Option<&Path>) -> bool {
    projectless_root
        .is_some_and(|root| crate::projectless::is_projectless_path_under(&project.path, root))
}
...
impl Tide {
    ...
    fn ensure_sidebar_branch_labels(&self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |tide, cx| {
                    ...
                    .spawn(async move {
                        ...
                        for path in paths {
                            let branch = match workspace.request(
                                client::WorkspaceOperation::InspectBranches { cwd: path.clone() },
                            ) {
                                Ok(client::WorkspaceResult::Branches {
            ...
            let _ = tide.update(cx, |tide, cx| {
                if tide.sidebar_branch_scan_generation.get() != generation {
                    return;
                }
                *tide.sidebar_branch_labels.borrow_mut() = labels
                    .into_iter()
    ...
    pub(super) fn render_sidebar(
        ...
        div()
            ...
            .flex_col()
            .bg(if is_resizing {
                solid_background(theme.sidebar_drag_background)
            } else {
    ...
    fn render_sidebar_session_item(&self, session_id: Uuid, cx: &mut Context<Self>) -> AnyElement {
        ...
        let row = if renaming {
            ...
            context_menu(
                ...
                &menu,
                move |_| {
                    let rename_tide = tide.clone();
                    let remove_tide = tide.clone();
                    vec![
                        MenuItem::new(tr!("common.rename"), move |window, cx| {
                            let _ = rename_tide.update(cx, |tide, cx| {
                                tide.begin_session_rename(session_id, window, cx);
                            });
                        }),
                        MenuItem::Separator,
                        MenuItem::new(tr!("common.remove"), move |_, cx| {
                            let _ = remove_tide
                                .update(cx, |tide, cx| tide.remove_session(session_id, cx));
                        }),
                    ]
                },
            )
    ...
    pub(super) fn render_header(
        ...
        div()
            ...
            .child(self.render_header_jobs_pill(cx))
            .child(
                self.window_drag_region(
...
mod tests {
    ...
    fn sidebar_recency_uses_last_reply_with_creation_fallback() {
        ...
        let mut renamed_old_session = AgentSession::new(project_id, ProviderKind::Tide);
        renamed_old_session.created_at = 10;
        renamed_old_session.last_reply_at = Some(20);
        renamed_old_session.updated_at = 1_000;

        let mut newer_unanswered_session = AgentSession::new(project_id, ProviderKind::Tide);
    ...
    fn project_grouping_preserves_global_group_and_session_order() {
        ...
        let first = AgentSession::new(first_project, ProviderKind::Tide);
        let second = AgentSession::new(second_project, ProviderKind::Tide);
        let third = AgentSession::new(first_project, ProviderKind::Tide);
    ...
    fn projectless_sessions_share_one_trailing_group() {
        ...
        let first_projectless = AgentSession::new(first_projectless_project, ProviderKind::Tide);
        let ordinary = AgentSession::new(ordinary_project, ProviderKind::Tide);
        let second_projectless = AgentSession::new(second_projectless_project, ProviderKind::Tide);
    ...
    fn projectless_sidebar_projects_are_paths_under_the_workspace_root() {
        let root = Path::new("/tmp/.tide/projects");
