...
impl Tide {
    ...
    pub(super) fn open_find_action(
        ...
        if self.file_search_target_focused(window, cx) {
            ...
            // The v2 pane seam (the T21 guard precedent): when the v2
            // timeline owns the transcript, the same keystroke opens its
            // find bar instead of the legacy one.
            if self.timeline_v2 {
                self.timeline_v2_open_search(window, cx);
            } else {
    ...
    pub(super) fn close_find_action(
        ...
        if self.timeline_v2_search_open() {
            self.timeline_v2_close_search(true, window, cx);
    ...
    pub(super) fn find_next_action(
        ...
        if self.timeline_v2_search_open() {
            self.timeline_v2_navigate_search(false, cx);
        } else if self.transcript_search_open() {
    ...
    pub(super) fn find_previous_action(
        ...
        if self.timeline_v2_search_open() {
            self.timeline_v2_navigate_search(true, cx);
        } else if self.transcript_search_open() {
