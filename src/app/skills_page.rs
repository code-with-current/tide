...
//! ([`Tide::ensure_skills_catalog`]); frames read only the cached catalog.
...
impl Tide {
            ...
            let catalog = cx
                ...
                .spawn(async move {
                    match daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        client::Command::LoadSkills { projects },
                    )? {
                        client::ResponsePayload::SkillsCatalog { catalog } => Ok(catalog),
            ...
            let result = cx
                ...
                .spawn(async move {
                    daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        client::Command::SetSkillsEnabled { dirs, enabled },
                    )
                })
                .await;
            let _ = this.update(cx, |this, cx| {
            ...
            let result = cx
                ...
                .spawn(async move {
                    daemon.request(
                        Uuid::nil(),
                        Uuid::nil(),
                        client::Command::TrashSkills { dirs },
                    )
                })
                .await;
            let _ = this.update(cx, |this, cx| {
    ...
    pub(super) fn render_skills_settings(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if catalog.skills.is_empty() {
            ...
            .caption(tr!("skills.empty_description"))
            .size_full()
            .px(px(40.0))
            .py(px(40.0))
            .into_any_element();
    ...
    fn render_skills_source_filter(
        ...
        let sources = [SkillSource::Shared];
        dropdown_menu(
    ...
    fn render_skill_detail_pane(
        ...
        .icon("icons/pencil.svg", 11.0)
        .label(tr!("skills.open_file"))
        .flex_none();

        let reveal_button = chip(
        ...
        .icon("icons/folder.svg", 11.0)
        .label(tr!("skills.reveal"))
        .flex_none();

        let copy_feedback_id = format!("skill-copy-{}", skill.row_key);
        let copied = self.control_was_copied(&copy_feedback_id);
        let copy_button = chip(SharedString::from(copy_feedback_id.clone()), cx, {
            let dir = dir.clone();
