//! Immutable, render-ready Git diffs for Tide's Review surface.
//!
...
use protocol::git_panel::PanelDiffHunk;
...
pub enum Source {
    ...
    /// Tool-call diffs have no meaningful source; see [`from_file_changes`].
    #[default]
    Uncommitted,
}
...
/// Build a render-ready snapshot for the Git panel's selected-file sub-view
/// from the daemon's structured hunks (`gitDiff`), the same role
/// [`parse_collected`] plays for patch-text sources.
///
/// The hunk headers still carry `@@ -a,b +c,d @@` ranges, so the collapsed
/// context between hunks is derived exactly as [`parse`] derives it. Those
/// gaps keep no hidden lines — the Git panel widens context by refetching
/// from the daemon (tide's `diffSource.contextLines` ladder) rather than by
/// revealing retained rows, so the gap rows are deliberately
/// non-expandable-in-place.
pub fn from_panel_hunks(path: &str, hunks: &[PanelDiffHunk]) -> Snapshot {
    ...
    for hunk in hunks {
        if let Some(((old_start, old_count), (new_start, new_count))) =
            parse_hunk_ranges(&hunk.header)
        {
            let gap = old_start
                .saturating_sub(previous_old_next)
                .max(new_start.saturating_sub(previous_new_next));
            if gap > 0 {
                lines.push(Line {
                    file_index: 0,
                    old_line: None,
            ...
            // A hunk's ranges, not its last line number, tell where the next
            // one may pick up — a pure-addition hunk has no old-side rows.
            previous_old_next = old_start.saturating_add(old_count);
            previous_new_next = new_start.saturating_add(new_count);
        }
        saw_hunk = true;
        lines.push(Line {
            file_index: 0,
        ...
        for panel_line in &hunk.lines {
            // `text` carries the origin char; strip it into the kind, exactly
            // as `parse` splits a raw unified-diff row.
            let content = panel_line.text.get(1..).unwrap_or_default().to_owned();
            let (kind, old_no, new_no) = match panel_line.kind.as_str() {
                "add" => (LineKind::Addition, None, panel_line.new_no),
                "del" => (LineKind::Deletion, panel_line.old_no, None),
                _ => (LineKind::Context, panel_line.old_no, panel_line.new_no),
            };
            match kind {
...
fn parse_range(range: &str) -> Option<(u32, u32)> {
    ...
    // A single-number range means one line in Git's header grammar.
    let count = parts.next().map_or(1, |count| count.parse().unwrap_or(1));
    Some((start, count))
}

pub fn wire_source(source: Source) -> client::workspace::ReviewDiffSource {
    match source {
        Source::LastTurn {
            session_id,
            turn_id,
            turn_count,
        } => client::workspace::ReviewDiffSource::LastTurn {
            session_id,
            turn_id,
            turn_count,
        },
        Source::Uncommitted => client::workspace::ReviewDiffSource::Uncommitted,
    }
}
...
mod tests {
    use super::*;
    use protocol::git_panel::PanelDiffLine;
...
"#;
    ...
    #[test]
    fn panel_hunks_become_kinds_line_numbers_and_gaps() {
        let hunks = vec![
            PanelDiffHunk {
                ...
                lines: vec![
                    ...
                    PanelDiffLine {
                        ...
                        old_no: None,
                        new_no: Some(11),
                        text: "+let fresh = 2;".into(),
                    },
                    PanelDiffLine {
                        kind: "add".into(),
                        old_no: None,
                        new_no: Some(12),
                        text: "+return fresh;".into(),
                    },
                ],
            },
            PanelDiffHunk {
                header: "@@ -40,2 +41,2 @@".into(),
                lines: vec![PanelDiffLine {
                    kind: "context".into(),
                    old_no: Some(40),
                    new_no: Some(41),
                    text: " tail();".into(),
        ...
        let snapshot = from_panel_hunks("src/lib.rs", &hunks);
        assert_eq!(snapshot.files.len(), 1);
        assert_eq!(snapshot.files[0].path, "src/lib.rs");
        assert_eq!((snapshot.additions, snapshot.deletions), (2, 1));

        // The first hunk starts at line 10, so a leading gap of 9 opens the
        // snapshot.
        let LineKind::Gap(leading) = &snapshot.lines[0].kind else {
            panic!("leading gap before the first hunk");
        };
        assert_eq!(leading.count(), 9);
        assert_eq!(leading.position, GapPosition::Leading);

        // Hunk 1: header, then origin-stripped rows with mapped numbers.
        assert_eq!(snapshot.lines[1].kind, LineKind::HunkHeader);
        assert_eq!(snapshot.lines[1].content, "@@ -10,4 +10,5 @@");
        assert_eq!(snapshot.lines[2].kind, LineKind::Context);
        assert_eq!(snapshot.lines[2].content, "context();");
        assert_eq!(snapshot.lines[3].kind, LineKind::Deletion);
        assert_eq!(snapshot.lines[3].old_line, Some(11));
        assert_eq!(snapshot.lines[3].new_line, None);
        assert_eq!(snapshot.lines[4].kind, LineKind::Addition);
        assert_eq!(snapshot.lines[4].new_line, Some(11));
        assert_eq!(snapshot.lines[4].content, "let fresh = 2;");
        // Tokens were painted for a recognized language.
        assert!(!snapshot.lines[4].tokens.is_empty());

        // Between hunks: the old range ends at 14 and the next starts at 40
        // → 26 hidden; the new side (15 vs 41) agrees.
        let LineKind::Gap(gap) = &snapshot.lines[6].kind else {
            panic!("a gap separates the hunks");
        };
        assert_eq!(gap.count(), 26);
        assert_eq!(gap.position, GapPosition::Between);
        assert!(!gap.is_expandable());
        assert_eq!(snapshot.lines[7].content, "@@ -40,2 +41,2 @@");
        assert_eq!(snapshot.lines[8].old_line, Some(40));
        assert_eq!(snapshot.lines[8].new_line, Some(41));
    }

    #[test]
    fn panel_hunks_report_a_leading_gap_before_the_first_hunk() {
        let hunks = vec![PanelDiffHunk {
            header: "@@ -7,3 +7,3 @@".into(),
            lines: vec![PanelDiffLine {
                kind: "context".into(),
                old_no: Some(7),
        ...
        let snapshot = from_panel_hunks("a.txt", &hunks);
        let LineKind::Gap(gap) = &snapshot.lines[0].kind else {
            panic!("leading gap");
        };
        assert_eq!(gap.count(), 6);
        assert_eq!(gap.position, GapPosition::Leading);
        // An unrecognized language paints no tokens; nothing panics.
        assert!(snapshot.lines[2].tokens.is_empty());
    }
