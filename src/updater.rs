...
//! Tide's routing user driver keeps automatic checks in the sidebar, but
...
//! itself with a production build. `TIDE_PREVIEW_UPDATE=1` fakes only the
//! automatic sidebar result while retaining the real Sparkle flow for the
//! Check for Updates menu; `TIDE_FORCE_UPDATER=1` exercises everything for
...
/// The compact state rendered by Tide. Update details remain owned by
...
mod macos {
    ...
    struct UserDriverIvars {
        ...
        /// use Sparkle's own windows. Scheduled checks stay inside Tide.
    ...
    impl Updater {
        ...
        pub fn init() -> Option<Self> {
            let preview = cfg!(debug_assertions)
                && std::env::var_os("TIDE_PREVIEW_UPDATE").is_some_and(|value| value == "1");
            let forced = std::env::var_os("TIDE_FORCE_UPDATER").is_some_and(|value| value == "1");
            ...
            if handle.is_null() {
                ...
                eprintln!("Tide updater: failed to load Sparkle: {reason}");
                return None;
            }
            ...
            if !started {
                eprintln!("Tide updater: Sparkle rejected its updater configuration");
                return None;
            }
    ...
    /// (Contents/MacOS/Tide → Contents/Frameworks/Sparkle.framework/Sparkle).
    ...
    mod tests {
        ...
        fn routing_user_driver_satisfies_sparkle_protocols() {
            ...
            let library = target_dir
                .join("debug/Tide Debug.app/Contents/Frameworks/Sparkle.framework/Sparkle");
            if !library.exists() {
                return;
            }
...
mod feed {
    ...
    /// Compare dotted release numbers field by field. Tide's versions are
    ...
    <item>
      ...
      <enclosure url="https://releases.tide.codes/Tide-0.2.0-x86_64-Setup.exe" length="2048" type="application/octet-stream" sparkle:edSignature="newsig" />
        ...
        fn the_newest_signed_item_wins_regardless_of_feed_order() {
            ...
            assert!(item.url.ends_with("Tide-0.2.0-x86_64-Setup.exe"));
        }
...
mod windows {
    ...
    const PUBLIC_ED_KEY: &str = env!("TIDE_SPARKLE_PUBLIC_ED_KEY");
    ...
    impl Updater {
        pub fn init() -> Option<Self> {
            ...
            if verifying_key().is_none() {
                eprintln!("Tide updater: SUPublicEDKey is not a valid ed25519 key");
                return None;
            }
        ...
        pub fn set_automatically_checks_for_updates(&self, enabled: bool) {
            ...
            let _ = std::thread::Builder::new()
                .name("tide-updater-preference".into())
    ...
    fn download_and_verify(item: &AppcastItem) -> anyhow::Result<PathBuf> {
        ...
        let installer = directory.join("Tide-Setup.exe");
    ...
    mod tests {
        ...
        fn a_signature_from_the_release_script_verifies_here() {
            ...
            const PAYLOAD: &[u8] = b"Tide-0.0.0-x86_64-Setup.exe contents";
        ...
        fn an_absent_preference_file_leaves_automatic_checks_on() {
            let directory = std::env::temp_dir()
                .join(format!("tide-updater-preference-{}", std::process::id()));
            let _ = std::fs::remove_dir_all(&directory);
