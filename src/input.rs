...
/// Where input diagnostics land, if they are enabled at all. Debug builds
/// always trace (the watcher owns the repo-relative log); a release build
/// traces only under `TIDE_INPUT_TRACE=1`, and then to an absolute path so a
/// Finder-launched app bundle can open it too.
fn trace_path() -> Option<std::path::PathBuf> {
    if cfg!(debug_assertions) {
        return Some(std::path::PathBuf::from("temp/input-debug.log"));
    }
    let enabled = std::env::var_os("TIDE_INPUT_TRACE").is_some_and(|value| value == "1");
    enabled.then(|| std::env::temp_dir().join("tide-input-trace.log"))
}

/// Whether input diagnostics are on: debug builds always, release builds via
/// `TIDE_INPUT_TRACE=1`. The window-level unhandled-keystroke observer uses
/// the same switch.
pub(crate) fn input_trace_enabled() -> bool {
    trace_path().is_some()
}

/// Input-drop tracing: append every text mutation on every field to the
/// trace, so a reproducible "can't type" report can be traced to the layer
/// that dropped it.
pub(crate) fn debug_log_input(site: &str, before: &str, incoming: &str) {
    ...
    let _ = writeln!(
        file,
        "{:?} {site} before={before:?} incoming={incoming:?}",
        std::time::SystemTime::now()
    );
}

/// Input-drop tracing companion: a captured stack for the interesting case
/// (something resetting a non-empty field to empty).
pub(crate) fn debug_log_backtrace(site: &str) {
    use std::io::Write as _;
    let Some(path) = trace_path() else {
        return;
    };
...
impl TextInput {
    ...
    /// Cap this field's [`auto_height`](Self::auto_height) growth lower than
    /// the [`AUTO_HEIGHT_MAX`] default (e.g. a description box limited to a
    /// fixed number of visible lines); past the cap the text scrolls.
    pub fn auto_height_max(mut self, max: Pixels) -> Self {
        self.auto_height_cap = Some(max);
        self
    }
    ...
    pub fn set_content(&mut self, content: impl Into<SharedString>, cx: &mut Context<Self>) {
        ...
        debug_log_input("set_content", &self.content, &content);
        if !self.content.is_empty() && content.is_empty() {
            debug_log_backtrace("set_content_wipe");
        }
...
impl EntityInputHandler for TextInput {
    ...
    fn replace_text_in_range(
        ...
        debug_log_input("replace_text_in_range", &self.content, new_text);
        if self.read_only {
            return;
        }
    ...
    fn replace_and_mark_text_in_range(
        ...
        debug_log_input("replace_and_mark_text_in_range", &self.content, new_text);
        if self.read_only {
            return;
        }
