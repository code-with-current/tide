...
export interface McpServerRowProps {
  ...
  enabled: boolean;
  /** Whether this server uses OAuth auth (config.auth === 'oauth'). Drives
   *  the reauthenticate button on connected servers. */
  auth?: 'oauth';
...
export const McpServerRow = memo(function McpServerRow({
  ...
  // Show reauthenticate on connected OAuth servers too — the token may have
  // expired silently. Without this, the user sees a green "connected" server
  // with no way to re-authenticate until a tool call fails.
  const canReauthConnected = status === 'connected' && auth === 'oauth' && onReauthorize;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
        ...
        {/* Reauthenticate — shown on connected OAuth servers so users can
            refresh expired tokens proactively, without waiting for a failure. */}
        {canReauthConnected && (
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); onReauthorize!(); }}
            title="Reauthenticate (refresh OAuth token)"
            className="p-1 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-amber-500"
          >
            <KeyRound className="size-3" />
          </button>
        )}
