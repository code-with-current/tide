...
export function MainScreen() {
  ...
  useEffect(() => {
    window.tideIpc?.getAgentSettings().then((s) => {
      if (s?.defaultAutonomy && s.defaultAutonomy !== useUi.getState().autonomyMode) {
  ...
  const pendingOptions = useUi((s) => (activeSessionId ? s.pendingOptions[activeSessionId] : undefined));
  ...
  const {
    start, abort, approveToolCalls, rejectToolCalls, submitFollowup,
  } = useChatStream();
  ...
  const activeStream = useUi((s) => (activeSessionId ? s.streams[activeSessionId] : undefined));
  ...
  const hasAnyFinalMessage = useUi((s) => Object.values(s.streams).some((st) => st.finalMessage));
  ...
  useEffect(() => { streamsRef; }, [streamsRef]);
  ...
  useEffect(() => {
    ...
    api.getSession(activeSessionId).then((s) => {
      ...
      if (currentSessionRef.current === activeSessionId) {
        setChatHistory(migrateMessagesToBlocks((s.messages ?? []).map((m: any) => ({
          id: m.id,
          role: m.role,
          content: m.content,
  ...
  }, [sessions?.length, activeSessionId, sessionsPanelOpen, rightPanelOpen, setSessionsPanelOpen, setRightPanelOpen]);
  ...
  }, [chatHistory.length, streamingText, streamingReasoning, streamingToolCalls.length, isStreaming]);
  ...
  }, [isStreaming, streamingText, streamingTimeline, activeStream?.blocks,
      activeStream?.turn, streamingReasoning, streamingToolCalls, streamingUsage]);
  ...
  const handleSend = useCallback(async (payload: { text: string; promptText?: string; mentions?: Array<{ name: string; kind: string; source?: string; filePath?: string; description?: string }>; attachments?: MessageAttachment[]; worktree?: { enabled: boolean; branchName: string; baseBranch: string; configFiles?: string[] } }) => {
    ...
    if (isStreaming && activeSessionId) { abort(activeSessionId); return; }
    if (!activeWorkspaceId) { log.warn('No workspace selected'); return; }
    if (!modelOption || !selectedModelId) { log.warn('No model selected'); return; }
    ...
    if (!sessionId && activeWorkspaceId) {
      ...
      const placeholderTitle = stripCommandPrefix(text).slice(0, 50) || text.slice(0, 50);
      const newSession = await api.createSession(
        activeWorkspaceId,
        placeholderTitle,
        selectedModelId,
        { autonomyMode, thinkingLevel, providerId: selectedProviderId ?? undefined },
      );
    ...
    const userMsg: Message = {
      ...
      mentions: payload.mentions && payload.mentions.length > 0 ? payload.mentions as Message['mentions'] : undefined,
    };
    ...
    const [workspaceContext, referencedFiles] = await Promise.all([
      activeWorkspaceId ? api.getWorkspaceContext(activeWorkspaceId).catch(() => '') : Promise.resolve(''),
      activeWorkspaceId ? buildReferencedFilesBlock(activeWorkspaceId, text) : Promise.resolve(''),
    ]);
    const systemPrompt = buildSystemPrompt({
      ...
      worktree: payload.worktree?.enabled
        ? { branch: payload.worktree.branchName, baseBranch: payload.worktree.baseBranch }
        : undefined,
    });
    ...
    const apiMessages = [
      ...
      { role: 'user' as const, content: promptText, attachments: attachments.length > 0 ? attachments : undefined },
    ];
  ...
  }, [isStreaming, abort, modelOption, selectedModelId, selectedProviderId, activeSessionId, activeWorkspaceId, chatHistory, workspaces, autonomyMode, thinkingLevel, setActiveSession, setMainView, setSessionRunning, start]);
  ...
  }, [hasAnyFinalMessage, activeSessionId, activeWorkspaceId, qc, setSessionRunning, clearFinalMessage, markSessionUnread, markSessionRead]);
  ...
  const handleOptionsSubmit = useCallback(
    (selection: string[]) => {
      ...
      const answer = selection.length === 1
        ? selection[0]
        : selection.join(', ');
  ...
  return (
    <div className="backdrop-blur-xl frosted flex-1 flex min-h-0">
      ...
      <div ref={cardRef} className="flex-1 flex flex-col min-w-0 min-h-0 m-4 ml-0
        ...
        <WindowTopBar />
        <Group orientation="horizontal" className="flex-1 min-h-0 overflow-hidden">
          {sessionsPanelOpen && (
            <Panel id="sessions" defaultSize="15" minSize="12" maxSize="25" className="h-full">
              <SessionsPanel />
              ...
              {!activeWorkspaceId ? (
                ...
                <>
                  ...
                  <div className="flex-1 min-h-0 flex flex-col relative">
                    ...
                    <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto scroll px-6 py-5">
                    <div className="max-w-3xl mx-auto flex flex-col gap-3">
                      ...
                      {streamingMessage && (
                        <ChatMessage
                          ...
                          onApproveToolCalls={activeSessionId ? (ids, newMode, remember) => approveToolCalls(activeSessionId, ids, newMode, remember) : undefined}
                          onRejectToolCalls={activeSessionId ? (ids, reason) => rejectToolCalls(activeSessionId, ids, reason) : undefined}
                        />
                      )}
        ...
        {showRightPanel && (
          <Panel id="right" defaultSize="22" minSize="20" maxSize="35" className="h-full relative">
            <RightPanel />
        ...
        {fileViewerOpen && (
          <div className="absolute top-10 right-0 bottom-0 z-[2] shadow-2xl bg-card flex"
