...
import { usePermissionSurface } from '../permissions/permission-context';
...
/** Cap rendered tool output so expanding a huge result (a big grep / read of
 *  a massive file) can't freeze the UI — the browser would otherwise lay out
 *  the entire string even inside a max-height <pre>. Normal-sized outputs pass
...
function capOutput(text: string | undefined): { body: string; note?: string } {
  ...
  if (lines.length > MAX_RENDER_LINES) {
    body = lines.slice(0, MAX_RENDER_LINES).join('\n');
    note = `… ${(lines.length - MAX_RENDER_LINES).toLocaleString()} more lines truncated`;
  }
  if (body.length > MAX_RENDER_CHARS) {
    body = body.slice(0, MAX_RENDER_CHARS);
    note = note ? `${note} · output capped at ${MAX_RENDER_CHARS.toLocaleString()} chars` : `… output capped at ${MAX_RENDER_CHARS.toLocaleString()} chars`;
  }
  return { body, note };
}
 ...
 *  conclusion. Each child is a full ToolRow — expandable, with its
  ...
  return (
    <div className="py-2 space-y-2">
      ...
      {/* Sub-agent tool calls — nested, live-streaming. Each child is a full
          ToolRow, composition copied from ExploringGroup.
          ...
          {showToolCalls && (
            <div className="mt-1 ml-[3px] pl-3 border-l border-input space-y-0.5 animate-slide-up">
              {childToolCalls!.map(c => (
                <ToolRow key={c.id} call={c} onViewFile={onViewFile} />
              ))}
            </div>
          )}
        </div>
...
export const ToolRow = memo(function ToolRow({
  ...
  const handleApprove = approveOne
    ? (newMode?: 'plan' | 'ask' | 'edit' | 'full', remember?: boolean | 'session') =>
...
export const ExploringGroup = memo(function ExploringGroup({
  calls,
  onViewFile,
  streaming = false,
}: {
    ...
    <div className="relative pl-3">
      ...
      {open && (
        <div className="ml-[3px] pl-3 border-l border-input space-y-0.5 animate-slide-up">
          {calls.map((c) => (
            <ToolRow key={c.id} call={c} onViewFile={onViewFile} streaming={streaming} />
          ))}
        </div>
      )}
    </div>
