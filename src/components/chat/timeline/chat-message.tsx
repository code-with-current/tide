...
import type { UserMentionMeta } from './message/parts/user-ref-links';
...
const flattenAssistantTextParts = (parts: TimelinePart[]): string =>
  parts
    .filter((part): part is TimelinePart & { type: 'text'; text?: string } => part.type === 'text')
...
const ChatMessageImpl: React.FC<ChatMessageProps> = ({
  ...
  const turnActivityToolParts = React.useMemo<TimelineToolPart[]>(() => {
    if (isUser) {
      return [];
    }
    ...
    return records
      ...
      .filter((part): part is TimelineToolPart => part.type === 'tool');
  ...
  const messageTextContent = React.useMemo(() => {
    if (isUser) {
      const textParts = displayParts
        .filter((part): part is TimelinePart & { type: 'text'; text?: string; content?: string } => part.type === 'text')
        .map((part) => {
  ...
  const pendingPermissionParts = React.useMemo(() => {
    ...
    return toolParts.filter((part): part is TimelineToolPart => {
  ...
  const pendingFollowupParts = React.useMemo(() => {
    ...
    return entry.parts.filter((part): part is TimelineToolPart => {
                    ...
                    <div className={cn('max-w-[85%]', showStickyInlineHoverRow ? 'pb-5' : undefined)}>
                      ...
                      >
                        <MessageBody
                          ...
                          directory={directory}
                          userAttachments={
                            Array.isArray(entry.info.attachments)
                              ? (entry.info.attachments as import('@/types').MessageAttachment[])
                              : undefined
                          }
                          userMentions={
                            Array.isArray(entry.info.mentions)
                              ? (entry.info.mentions as UserMentionMeta[])
                              : undefined
                          }
                        />
                      </div>
...
function deriveTurnGroupingContext(props: ChatMessageProps): TurnGroupingContext | undefined {
