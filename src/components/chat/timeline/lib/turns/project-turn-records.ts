...
// Tide adapter never emits hidden/synthetic user messages, so the upstream
// display-normalization check collapses to constant false (its real
// implementation depends on message/part display filtering not ported here).
const isHiddenUserMessage = (_entry: unknown, _options: { planModeEnabled: boolean }): boolean => false;
