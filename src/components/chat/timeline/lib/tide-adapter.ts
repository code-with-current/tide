/** Tide→timeline adapter — NEW code (the port's seam), not upstream. Maps Tide `Message`/`Block` onto the vendored `TimelineMessage`/`TimelinePart` shapes consumed by the ported turn projection in `lib/turns/`.
 *
...
export function blockToPart(b: Block): TimelinePart {
  switch (b.kind) {
    case 'text':
      return {
        id: b.id,
...
export function hasParentToolCall(part: TimelinePart): boolean {
...
export function toChatMessageEntry(msg: Message): ChatMessageEntry {
  ...
  const parts: TimelinePart[] = msg.blocks && msg.blocks.length > 0
...
/**
 * Builds timeline entries from Tide messages — the timeline's production
