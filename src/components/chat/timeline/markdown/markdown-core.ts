...
import { contentFingerprint, HighlightResultCache, utf16Bytes } from './highlight-result-cache';
import { highlightCodeInWorker } from './markdown-worker';
import { escapeRawMarkdownHtml, isLocalFileUrl, MARKDOWN_FORBIDDEN_TAGS } from './markdown-security';
...
const renderMarkdownImageLabel = ({
  ...
  return `<span${titleAttr} class="inline-flex items-center gap-1 align-text-bottom text-muted-foreground" data-tide-markdown-image-label="true">${escapeAttr(label)}</span>`;
};
...
// ---------------------------------------------------------------------------
// Syntax highlighting (Shiki via the dedicated markdown worker)
// ---------------------------------------------------------------------------
...
const ensureSanitizeHook = (): void => {
  ...
  DOMPurify.addHook('uponSanitizeAttribute', (node, data) => {
    ...
    // (obsidian://, vscode://, ...). Upstream kept those for app links; Tide
    // has no app-link runtime, so only local file: URLs are force-kept.
...
// (upstream issue #2769). Content addressing makes identical blocks
