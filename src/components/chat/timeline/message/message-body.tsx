...
import type { TimelinePart, TimelineToolPart } from '../types/message-parts';
import type { StreamPhase, ToolPopupContent, AgentMentionInfo, ContentChangeReason } from './types';
import type { UserMentionMeta } from './parts/user-ref-links';
...
const partHasRenderableContent = (part: TimelinePart): boolean => {
...
type ShellActionPartLike = TimelinePart & {
  type: 'text';
  shellAction?: {
    command?: unknown;
    output?: unknown;
...
/** Loose cast: Tide's TimelinePart union has no 'subtask' member, but the adapter's extras may carry one. */
const isSubtaskPart = (part: TimelinePart): part is TimelinePart & SubtaskPartLike => {
  return (part as { type?: unknown }).type === 'subtask';
};
const isShellActionPart = (part: TimelinePart): part is ShellActionPartLike => {
...
interface MessageBodyProps {
  ...
  /** Tide wiring: persisted attachment metadata for user-bubble ref chips. */
  userAttachments?: MessageAttachment[];
  /** Tide wiring: composer mention metadata for inline `/name` chips in the user bubble. */
  userMentions?: UserMentionMeta[];
        ...
        {timestamp ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <span
                className="mr-1 flex items-center gap-1 text-xs tabular-nums text-muted-foreground/60"
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                variant="ghost"
                size="icon"
                className="h-5 w-5 text-muted-foreground bg-transparent hover:text-foreground hover:!bg-transparent active:!bg-transparent focus-visible:!bg-transparent focus-visible:ring-2 focus-visible:ring-primary/50"
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon"
                className={cn(
                  'h-5 w-5 bg-transparent hover:text-foreground hover:!bg-transparent active:!bg-transparent focus-visible:!bg-transparent focus-visible:ring-2 focus-visible:ring-primary/50',
...
const AssistantMessageBody = React.memo(({
  ...
  const toolParts = React.useMemo(() => {
    return visibleParts.filter((part): part is TimelineToolPart => part.type === 'tool');
  }, [visibleParts]);
  ...
  const isActiveTool = React.useCallback((toolPart: TimelineToolPart): boolean => {
  ...
  const isToolFinalized = React.useCallback((toolPart: TimelineToolPart) => {
    return isFinalizedToolStatus(toolPart.state?.status);
  }, []);

  const shouldShowTool = React.useCallback((toolPart: TimelineToolPart): boolean => {
  ...
  const activityByPart = React.useMemo(() => {
    const byRef = new Map<TimelinePart, TurnActivityRecord>();
  ...
  const renderedParts = React.useMemo(() => {
    ...
    while (i < visibleParts.length) {
      ...
      if (part.type === 'tool') {
        const toolPart = part as TimelineToolPart;
  ...
  if (isUser) {
    return (
      <UserMessageBody
        ...
        userAttachments={props.userAttachments}
