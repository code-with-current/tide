...
import { parseRefLinks, UserRefChips, type UserMentionMeta } from './user-ref-links';
...
type UserTextPartProps = {
  ...
  /** Tide wiring: session id for ref-chip clicks (opens the right-panel viewer). */
  sessionId?: string | null;
  /** Tide wiring: persisted attachment metadata for ref-chip resolution. */
  attachments?: MessageAttachment[];
  /** Tide wiring: composer mention metadata for inline `/name` chips. */
...
const UserTextPart: React.FC<UserTextPartProps> = ({
  ...
  // Lift composer ref links ([/label/](target)) out of the body into a chip
  // row above the text — otherwise they render as raw markdown links.
  const { chips: refChips, body } = React.useMemo(() => parseRefLinks(textContent), [textContent]);
  ...
  }, [agentMention, skillNames, body, mentions]);
  ...
  if ((!textContent || textContent.trim().length === 0) && refChips.length === 0) {
    return null;
  }

  return (
    <div className="relative" key={part.id || `${messageId}-user-text`}>
      <UserRefChips chips={refChips} attachments={attachments} sessionId={sessionId} />
