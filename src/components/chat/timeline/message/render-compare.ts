...
type MessageRecord = {
  info: TimelineMessage;
  parts: TimelinePart[];
};

const readPartId = (part: TimelinePart | undefined): string | null => {
  if (!part) return null;
  const candidate = (part as { id?: unknown }).id;
  return typeof candidate === 'string' && candidate.length > 0 ? candidate : null;
};

const readToolStatus = (part: TimelinePart | undefined): string | null => {
  const status = (part as { state?: { status?: unknown } } | undefined)?.state?.status;
  return typeof status === 'string' ? status : null;
};

const readToolStateRef = (part: TimelinePart | undefined): unknown => {
  return (part as { state?: unknown } | undefined)?.state;
};

const readPartMetadataRef = (part: TimelinePart | undefined): unknown => {
  return (part as { metadata?: unknown } | undefined)?.metadata;
};

const readPartTime = (part: TimelinePart | undefined) => {
...
const readPartText = (part: TimelinePart | undefined): string => {
...
export const areRenderRelevantPartsEqual = (left: TimelinePart[], right: TimelinePart[]): boolean => {
...
const areRenderRelevantMessageInfoEqual = (left: TimelineMessage, right: TimelineMessage): boolean => {
  if (left === right) return true;
