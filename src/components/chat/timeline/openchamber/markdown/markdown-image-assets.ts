/** Ported from openchamber/openchamber (MIT): packages/ui/src/components/chat/markdown/markdownImageAssets.ts.
 *  Adaptation seam (documented): upstream validated local markdown images via
 *  an OpenChamber server route (`/api/openchamber/sessions/:id/markdown-image-grants`,
 *  through `runtimeFetch` + a runtime URL resolver) or a VSCode workspace fs
 *  bridge. Tide (Electron renderer, no OpenChamber server) has neither, so the
