/** Ported from openchamber/openchamber (MIT): packages/ui/src/components/chat/lib/turns/turnProjectionCache.ts. Tide adaptation: upstream's VS Code / mobile runtime cache-size detection (`@/lib/desktop`, `@/lib/runtimeSurface`) is not ported, so a single desktop cache size is used. */
