/**
 * Ported from openchamber/openchamber (MIT): packages/ui/src/components/chat/message/toolRenderers.tsx.
 * Adaptations:
...
export const TIDE_TOOL_ALIASES: Record<string, string> = {
  read_file: 'read',
...
export const resolveRendererToolName = (toolName: string | undefined | null): string => {
  if (typeof toolName !== 'string') {
    return '';
  }
  return TIDE_TOOL_ALIASES[toolName] ?? toolName;
};
...
export const renderGrepOutput = (output: string, options?: { unstyled?: boolean }) => {
  try {
    const lines = output.trim().split('\n').filter(Boolean);
    if (lines.length === 0) return null;
      ...
      >
        ...
        {Object.entries(fileGroups).map(([filepath, matches]) => (
          <div key={filepath} className="space-y-1">
            <div className="font-medium text-muted-foreground typography-code">
              {filepath}
            </div>
...
export const renderGlobOutput = (output: string, options?: { unstyled?: boolean }) => {
  try {
      ...
      >
        ...
        {sortedDirs.map((dir) => (
          <div key={dir} className="space-y-1">
            ...
            <div className="pl-4 grid gap-1 grid-cols-2">
              {groups[dir].sort().map((filename) => (
                <div key={filename} className="flex items-center gap-2 min-w-0 typography-code">
