/** Ported from openchamber/openchamber (MIT): packages/ui/src/components/chat/message/normalizeUserDisplayParts.ts.
 *  Ported faithfully (re-indented 4-space → 2-space); SDK `Part` becomes
 *  Tide's structural `OcPart`. The synthetic GitHub issue/PR attachment parts
...
const buildGitHubAttachmentPart = (text: string): OcPart | null => {
...
export const normalizeUserDisplayParts = (parts: OcPart[], options?: { planModeEnabled?: boolean }): OcPart[] => {
  ...
  return parts
    ...
    .map((part) => {
      ...
      if (rawPart.type === 'compaction') {
        return { type: 'text', text: '/compact' } as OcPart;
      }
      if (rawPart.type === 'text') {
        ...
        if (text.startsWith('The following tool was executed by the user')) {
          return { type: 'text', text: '/shell' } as OcPart;
        }
      }
      return part;
    });
