/** Ported from openchamber/openchamber (MIT): packages/ui/src/components/chat/message/renderCompare.ts.
 *  Pure render-relevance comparators — ported verbatim; `Message`/`Part` become
 *  Tide's `OcMessage`/`OcPart`, and turns types come from Tide's ported
...
const readPartId = (part: OcPart | undefined): string | null => {
  if (!part) return null;
  const candidate = (part as { id?: unknown }).id;
  return typeof candidate === 'string' && candidate.length > 0 ? candidate : null;
};

const readToolStatus = (part: OcPart | undefined): string | null => {
  const status = (part as { state?: { status?: unknown } } | undefined)?.state?.status;
  return typeof status === 'string' ? status : null;
};

const readToolStateRef = (part: OcPart | undefined): unknown => {
  return (part as { state?: unknown } | undefined)?.state;
};

const readPartMetadataRef = (part: OcPart | undefined): unknown => {
  return (part as { metadata?: unknown } | undefined)?.metadata;
};

const readPartTime = (part: OcPart | undefined) => {
...
const readPartText = (part: OcPart | undefined): string => {
...
export const areRenderRelevantPartsEqual = (left: OcPart[], right: OcPart[]): boolean => {
...
const areRenderRelevantMessageInfoEqual = (left: OcMessage, right: OcMessage): boolean => {
  if (left === right) return true;
