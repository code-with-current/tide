/** Ported from openchamber/openchamber (MIT): packages/ui/src/components/chat/message/parts/WorkingPlaceholder.tsx.
 *  Adaptations: `useProviderLogo` + `useThemeSystem` dropped (ruling 2 — the
 *  provider logo never renders without the logo hook; the status row renders
 *  text-only); i18n (`useI18n`) → literal English ("{model}: {status}").
 *  Status queueing / min-display timing / retry countdown ported verbatim. */
...
export function WorkingPlaceholder({
  ...
  // Upstream: i18n key `chat.statusRow.modelStatus` ({ model }, { status }).
  const label = trimmedModelName.length > 0
    ? `${trimmedModelName}: ${displayedText}`
