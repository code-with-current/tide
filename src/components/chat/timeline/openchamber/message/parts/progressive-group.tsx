 ...
 *  - SDK types → Tide's `OcToolPart`; i18n already literal upstream ("Activity").
...
import type { OcToolPart } from '../../types/opencode-parts';
...
const isActivityRunning = (activity: TurnActivityPart): boolean => {
  ...
  const part = activity.part as OcToolPart;
...
const ExpandableToolRow: React.FC<ExpandableToolRowProps> = ({
  ...
  const content = (
    <ToolPart
      part={activity.part as OcToolPart}
...
const aggregateRows = (parts: TurnActivityPart[]): AggregatedRow[] => {
  ...
  while (i < parts.length) {
    ...
    // Tool part
    const toolPart = activity.part as OcToolPart;
