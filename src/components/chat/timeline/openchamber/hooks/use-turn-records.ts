/**
 * Ported from openchamber/openchamber (MIT): packages/ui/src/components/chat/hooks/useTurnRecords.ts — ADAPTED.
 * Wraps the T1-ported projection (`projectTurnRecords` + `turnProjectionCache`)
 * instead of upstream's. OpenChamber port seams: `streamPerfMeasure` perf
...
import { buildProjectionCacheKey, getCachedProjection, setCachedProjection } from '../lib/turns/turn-projection-cache';
