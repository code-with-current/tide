...
export function FileExplorerTab() {
  ...
  const { data, isLoading } = useFileTree(workspaceId);
  ...
  return (
    <div className="flex flex-col h-full min-w-0 overflow-hidden">
      ...
      {/* Tree */}
      <div className="flex-1 overflow-y-auto scroll py-1 min-w-0 min-h-0">
        {isLoading && (
...
function TreeNode({
    ...
    return (
      <div className="min-w-0">
        <span
          ...
          className={cn(
            'w-full flex items-center gap-1 py-0.5 text-[11px] hover:bg-secondary rounded-sm cursor-pointer transition-colors min-w-0',
          )}
        >
          <ChevronRight
            className={cn('size-3 flex-shrink-0 transition-transform text-muted-foreground', isOpen && 'rotate-90')}
          />
          <FolderIcon folderName={node.name} isOpen={isOpen} size={14} className="flex-shrink-0" />
          <span className={cn(
  ...
  return (
    <span
      ...
      onClick={() => {
        if (sessionId) {
          openFile(sessionId, { id: node.path, path: node.path, language: langFromName(node.name) });
        }
      }}
      className={cn(
        'w-full flex items-center gap-1 py-0.5 text-[11px] hover:bg-secondary rounded-sm cursor-pointer transition-colors min-w-0',
