/** AgentsTab — focused sub-agent dispatch stream. Clicking a dispatch row in
 *  the chat (or an entry below) focuses it here: header (agent, status, task,
 *  usage) + the dispatch's live tool stream, followed by its report.
 *
...
/** Minimal mirror of block-list's stream-branch grouping: direct children of
 *  the dispatch group into contiguous runs (non-tool blocks break the run);
 *  deeper descendants index by parent so they flatten under their own row. */
function groupChildRuns(list: Block[], focus: string) {
  ...
  for (const b of list) {
    ...
    if (b.parentToolCallId === focus) {
      const run = prevWasChild ? runs[runs.length - 1] : undefined;
      if (run) run.push(b);
      else runs.push([b]);
      prevWasChild = true;
      continue;
...
function flattenRun(run: ToolBlock[], childrenByParent: Map<string, ToolBlock[]>): ToolCall[] {
  const out: ToolCall[] = [];
  for (const b of run) {
...
export function AgentsTab({ session }: { session: Session }) {
  ...
  const liveBlocks = useUi((s) => s.streams[session.id]?.blocks);
  const { data: dispatches } = useDispatches(session.id);
              ...
              {(dispatches ?? []).map((h) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    disabled={!blockId}
                    onClick={() => blockId && setFocusedDispatch(session.id, blockId)}
                    className={cn(
  ...
  const childCount = resolved.list.filter(
    (b) => b.kind === 'tool' && b.parentToolCallId === focus,
  ).length;
  const { runs, childrenByParent } = groupChildRuns(resolved.list, focus);

  return (
    <div className="flex flex-col h-full min-h-0 min-w-0 overflow-x-hidden">
      {/* Header — agent badge, status, task, usage */}
      <div className="flex flex-col gap-1.5 border-b border-border px-3 py-2.5">
        ...
        {task && (
          <p className="whitespace-pre-wrap font-mono text-[11.5px] leading-[1.6] text-muted-foreground">
            {task}
          </p>
        )}
      </div>

      {/* Body — the dispatch's live tool stream, report last */}
      <div ref={bodyRef} className="flex-1 min-h-0 scroll min-w-0">
