import { FolderGit2, Plus, Settings, MoreHorizontal, Pencil, Archive, ArchiveRestore, Trash2, ChevronDown, ArchiveIcon } from 'lucide-react';
import { useState, useEffect, useRef } from 'react';
import { useWorkspaces, useSessions, useRenameWorkspace, useArchiveWorkspace, useUnarchiveWorkspace, useDeleteWorkspace } from '@/lib/queries';
...
function workspaceStatus(
  ...
  if (sessions.some((s) => runningSessionIds.includes(s.id))) return 'in_progress';
...
export function WorkspacesPanel() {
  ...
  return (
    <aside className="flex flex-col h-full overflow-hidden flex-shrink-0 p-2" style={{ width: 200 }}>
      {/* Top spacer — clears the native macOS traffic lights at (12, 12). */}
      <div className="h-10 flex-shrink-0" />
        <div className="px-3 py-2.5 flex items-center justify-between border-input flex-shrink-0 border-b ">
          <div className="text-[0.7143rem] uppercase tracking-wider text-muted-foreground/60 font-semibold">
            Workspaces
          </div>
        ...
        <div className="p-2 border-t border-input space-y-2">
          <Button
            variant="outline"
            onClick={() => openDialog('addWorkspace')}
            className="w-full flex items-center gap-2 px-2.5 py-1.5 text-xs text-muted-foreground rounded hover:bg-secondary hover:text-foreground"
          >
            <FolderGit2 className="size-3.5" /> Add workspace
          </Button>
          <Button
            variant="outline"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <FolderGit2 className={cn('size-3.5 flex-shrink-0', active ? 'text-primary' : 'text-muted-foreground/60')} />
        {isRenaming ? (
          <Input
            ref={inputRef}
          ...
          <div className={cn('font-medium flex-1 truncate', active && 'text-foreground')}>
          ...
          <>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40">
                ...
                {!archived ? (
                  <DropdownMenuItem onClick={() => archiveWorkspace.mutate(workspace.id)}>
                    <Archive className="size-3.5" /> Archive
                  </DropdownMenuItem>
                ) : (
                  <>
                    <DropdownMenuItem onClick={() => unarchiveWorkspace.mutate(workspace.id)}>
      ...
      <div className="text-[0.7143rem] text-muted-foreground/60 font-mono pl-5 truncate">
      ...
      <Dialog open={confirmDelete} onOpenChange={setConfirmDelete}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-xs text-muted-foreground px-1 -mt-1">
            "{workspace.name}" will be removed from Tide. All its archived sessions will be permanently deleted from disk. The repository folder on disk is not touched. This can't be undone.
          </p>
          <DialogFooter>
        ...
        <div className="space-y-1 mt-1">
          {workspaces.map((ws) => (
            <WorkspaceItem
              ...
              runningSessionIds={runningSessionIds}
              unreadSessionIds={unreadSessionIds}
              onSelect={() => { /* archived workspaces aren't selectable */ }}
            />
          ))}
        </div>
      )}
...
function shortenPath(p: string): string {
  if (!p) return '';
  if (cachedHome && p.startsWith(cachedHome)) return '~' + p.slice(cachedHome.length);
