use gpui::{
    App, Background, Global, Hsla, Rems, Window, WindowAppearance, hsla, linear_color_stop,
    linear_gradient, rems, rgb, solid_background, transparent_black,
};

pub use client::theme::ThemePreference;
...
pub fn sp(value: f32) -> Rems {
    rems(value / client::persistence::DEFAULT_UI_FONT_SIZE)
}
...
/// Tide's visual language, take two: neutral graphite surfaces in the spirit
/// of Cursor — color is reserved for meaning. On macOS the sidebar's semantic
/// tint is installed as a native layer above Sidebar vibrancy; the main
/// sidebar column also carries a translucent dark-blue gradient wash (see
/// `Theme::sidebar_background`), while other GPUI surfaces stay clear to
/// avoid incorrectly accumulating the alpha of nested Metal backgrounds.
/// Selected, hovered, and pressed rows remain a 6% neutral layer.
#[derive(Clone, Copy)]
pub struct Theme {
    pub is_dark: bool,
    pub canvas: Hsla,
...
impl Theme {
    pub fn current(cx: &App) -> Self {
        if cx.has_global::<ActiveTideTheme>() {
            cx.global::<ActiveTideTheme>().0
        } else {
            Self::dark()
        }
    }

    /// Backdrop for the main sidebar column. Dark macOS layers a translucent
    /// dark-blue wash over the native vibrancy tint, fading to transparent
    /// black top-to-bottom; light and non-macOS keep the solid `sidebar` color
    /// so the native material or flat fallback shows through untouched.
    pub fn sidebar_background(&self) -> Background {
        if cfg!(target_os = "macos") && self.is_dark {
            linear_gradient(
                180.,
                linear_color_stop(hsla(224.0 / 360.0, 0.80, 0.13, 0.15), 0.),
                linear_color_stop(transparent_black(), 1.),
            )
        } else {
            solid_background(self.sidebar)
        }
    }
...
#[derive(Clone, Copy)]
struct ActiveTideTheme(Theme);

impl Global for ActiveTideTheme {}
...
fn set_active_theme(theme: Theme, cx: &mut App) {
    cx.set_global(ActiveTideTheme(theme));
}
