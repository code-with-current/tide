...
pub use client::{
    ...
    model_catalog, persistence, projectless, skills, usage, usage_history, usage_report, worktree,
};

use gpui::{
    App, Application, Bounds, KeyBinding, Menu, MenuItem, TitlebarOptions,
    WindowBackgroundAppearance, WindowBounds, WindowHandle, WindowOptions, actions, point, px,
    size,
};
...
trait TideApplicationExt {
    fn with_main_window_reopen(self, daemon: client::DaemonSupervisor) -> Self;
}

impl TideApplicationExt for Application {
    fn with_main_window_reopen(self, daemon: client::DaemonSupervisor) -> Self {
        self.on_reopen(move |cx| {
            // One Tide, one window: a Dock click either activates the open
            // window or — after the user closed it while the app kept
            // running, which macOS allows — brings a fresh main window back.
            // Without this arm a relaunch could not restore the window,
            // which pushed straight into launching a second instance.
            match cx.windows().into_iter().next() {
                Some(window) => {
                    window
                ...
                None => {
                    open_main_window(daemon.clone(), cx);
                }
            }
            cx.activate(true);
        });
...
/// One Tide per desktop. Contenders for the lock either win it and hold it
/// for the life of the process, or activate the winner and exit — see
/// [`singleton`] for why the lock is kernel-arbitrated instead of a pid-file
/// check. The dev watcher kills and relaunches in a tight cycle, so a debug
/// build waits out a dying predecessor before conceding; release builds
/// concede immediately so a stray lock never delays a user launch.
fn ensure_single_instance() {
    ...
    let retry_wait = if cfg!(debug_assertions) {
        Duration::from_secs(2)
    } else {
        Duration::ZERO
    };
    match singleton::acquire(&lock_path, retry_wait) {
        Ok(singleton::Acquisition::Acquired(guard)) => guard.hold_for_process_lifetime(),
        Ok(singleton::Acquisition::HeldByOther { pid }) => {
            if let Some(pid) = pid.filter(|pid| *pid > 0) {
                activate_other_instance(pid);
            }
            eprintln!("Tide is already running; activating it instead.");
            std::process::exit(0);
        }
        // The pid-file scheme this replaces also ignored an unreadable lock
        // and started anyway; keep that resilience — a broken lock must not
        // brick the app, it only weakens the single-instance guarantee.
        Err(error) => {
            eprintln!("ignoring single-instance lock ({error}); continuing.");
        }
    }
}

#[cfg(target_os = "macos")]
fn activate_other_instance(pid: i32) {
...
pub fn run() {
    ...
    gpui_platform::application()
        ...
        .with_main_window_reopen(daemon.clone())
        .run(move |cx: &mut App| {
            ...
            crate::app::init_git_dialog_keys(cx);
            ...
            cx.bind_keys([
                ...
                KeyBinding::new("secondary-[", NavigateBack, Some("Tide")),
                KeyBinding::new("secondary-]", NavigateForward, Some("Tide")),
                KeyBinding::new("ctrl-tab", SwitchTaskForward, Some("Tide")),
                KeyBinding::new("ctrl-shift-tab", SwitchTaskBackward, Some("Tide")),
                KeyBinding::new("ctrl-escape", CancelTaskSwitch, Some("Tide")),
                KeyBinding::new("ctrl-shift-escape", CancelTaskSwitch, Some("Tide")),
                ...
                KeyBinding::new("escape", CancelTurn, Some("Tide")),
                KeyBinding::new("secondary-c", CopySelection, Some("Tide")),
                ...
                KeyBinding::new("secondary-f", OpenFind, Some("Tide")),
                // The text input's macOS-style Ctrl-F caret binding is more
                // specific than Tide's root context. Reassert the platform
                ...
                KeyBinding::new("secondary-f", OpenFind, Some("Tide > TextInput")),
                KeyBinding::new("secondary-alt-f", OpenFindReplace, Some("Tide")),
                KeyBinding::new("secondary-g", FindNext, Some("Tide")),
                KeyBinding::new("secondary-shift-g", FindPrevious, Some("Tide")),
                ...
                // Browser surface. Deeper than "Tide", so while focus is on the
            ...
            let _window = open_main_window(daemon.clone(), cx);
            ...
            cx.on_system_notification_response(|response, cx| {
                ...
                // The app holds at most one window, so resolve it live: a
                // window closed and reopened through the Dock replaces the
                // handle a startup-time capture would be stuck with.
                if let Some(window) = cx
                    .windows()
                    .into_iter()
                    .next()
                    .and_then(|window| window.downcast::<Tide>())
                {
                    window
                        .update(cx, |tide, window, cx| {
                            tide.open_task_from_notification(session_id, cx);
...
/// Open the one main window — at startup, and again on Dock reopen after the
/// user closed it. Both paths share the persisted placement, so the window
/// lands where the user last left it, on the display they left it on.
fn open_main_window(daemon: client::DaemonSupervisor, cx: &mut App) -> WindowHandle<Tide> {
    ...
    let window = cx
        .open_window(
            WindowOptions {
                ...
                // client fallback and Tide renders that frame itself.
                #[cfg(target_os = "linux")]
            ...
            move |window, cx| {
                ...
                let tide = Tide::new(window, cx, daemon);
                let composer_focus = tide.read(cx).composer_focus(cx);
                window.focus(&composer_focus, cx);
                tide
            },
        )
        .expect("failed to open Tide window");
        ...
        Menu {
            ...
            items: {
                ...
                items.extend([
                    ...
                    MenuItem::separator(),
                    MenuItem::action(tr!("menu.remote_control"), ToggleRemoteControl),
                    MenuItem::separator(),
