...
//
// <updates-dir> holds the packaged archives (e.g. Tide-0.2.0.zip) plus any
...
//   TIDE_DOWNLOAD_URL_PREFIX   base URL for enclosure links
...
export function findGenerateAppcast(): string | null {
  ...
  const cacheRoot = join(projectRoot, ".tide-cache", "sparkle");
  ...
  if (!generator) {
    throw new Error(
      "generate_appcast not found. Run scripts/bundle.sh once to populate " +
        ".tide-cache/sparkle, or set SPARKLE_BIN to a Sparkle tools bin/ dir.",
    );
  }
