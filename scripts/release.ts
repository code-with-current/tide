...
const appName = "Tide";
const executableName = "Tide";
const jsReplExecutableName = "tide_js_repl";
const computerUseHelperName = "Tide Computer Use";
const packageName = "tide";
const defaultNotaryProfile = "NOTARY";
const projectRoot = resolve(import.meta.dir, "..");

const help = `Build, notarize, and publish a production release of Tide.
...
https://releases.tide.codes. One-time setup lives in RELEASING.md.

Options:
  --local                       Build, notarize, and write the DMG + zip
                                without publishing to R2
  --force                       Publish even if this version is already in R2
  --output <path>               DMG output path (default: dist/Tide-<version>.dmg)
  --signing-identity <name>     Developer ID Application identity selector
                                (or TIDE_SIGNING_IDENTITY; required unless --adhoc)
  --notary-profile <name>       notarytool keychain profile
                                (default: NOTARY; or TIDE_NOTARY_PROFILE)
  --build-number <number>       CFBundleVersion override (or TIDE_BUILD_NUMBER;
                                default derives a monotonic number from the
                                Cargo version)
  --volume-name <name>          Mounted DMG name (default: Tide)
  --skip-build                  Reuse target/release/tide, tide_js_repl, and
  --skip-notarize               Unnotarized signed DMG (implies --local)
  --adhoc                       Ad-hoc sign, no notarization (implies --local)
  --help                        Show this help

Environment:
  TIDE_SIGNING_IDENTITY         Developer ID Application identity selector
  TIDE_ANALYTICS_ENDPOINT       analytics endpoint embedded at build time
  TIDE_ANALYTICS_WEBSITE_ID     analytics website ID embedded at build time
  TIDE_R2_REMOTE                rclone remote name (default: r2)
  TIDE_R2_BUCKET                R2 bucket name (default: tide-releases)
  TIDE_DOWNLOAD_URL_PREFIX      base URL served by the bucket
                                (default: ${defaultDownloadUrlPrefix})
  TIDE_HISTORY_COUNT            prior archives pulled for deltas (default: 15)
  TIDE_NO_HISTORY=1             skip pulling prior archives (no deltas)
  SPARKLE_BIN                   Sparkle tools dir (default: the bundle.sh cache
                                under .tide-cache/sparkle)
...
const configuredSigningIdentity =
  values["signing-identity"] ?? process.env.TIDE_SIGNING_IDENTITY;
// `TIDE_SIGNING_IDENTITY=-` means ad-hoc, same as --adhoc — the default on
// machines without an Apple Developer Program membership, so there is no
// identity to require and nothing to notarize.
const adhoc = values.adhoc ?? configuredSigningIdentity === "-";
const notaryProfile =
  values["notary-profile"] ??
  process.env.TIDE_NOTARY_PROFILE ??
  defaultNotaryProfile;
const explicitBuildNumber =
  values["build-number"] ?? process.env.TIDE_BUILD_NUMBER;
const analyticsEndpoint = process.env.TIDE_ANALYTICS_ENDPOINT?.trim();
const analyticsWebsiteId = process.env.TIDE_ANALYTICS_WEBSITE_ID?.trim();
...
const r2Remote = process.env.TIDE_R2_REMOTE ?? "r2";
const r2Bucket = process.env.TIDE_R2_BUCKET ?? "tide-releases";
...
const downloadUrlPrefix =
  process.env.TIDE_DOWNLOAD_URL_PREFIX ?? defaultDownloadUrlPrefix;
const historyCount = Number(process.env.TIDE_HISTORY_COUNT ?? "15");
const skipHistory = process.env.TIDE_NO_HISTORY === "1";

if (adhoc && values["signing-identity"] && values["signing-identity"] !== "-") {
  throw new Error("Use either --adhoc or --signing-identity, not both.");
}
if (!adhoc && !configuredSigningIdentity) {
  throw new Error(
    "Set TIDE_SIGNING_IDENTITY or pass --signing-identity (or use --adhoc).",
  );
}
...
if (!Number.isSafeInteger(historyCount) || historyCount < 0) {
  throw new Error("TIDE_HISTORY_COUNT must be a non-negative integer.");
}
if (!values["skip-build"] && (!analyticsEndpoint || !analyticsWebsiteId)) {
  throw new Error(
    "Set TIDE_ANALYTICS_ENDPOINT and TIDE_ANALYTICS_WEBSITE_ID before building a release.",
  );
}
    ...
    if (detail.includes("directory not found")) {
      throw new Error(
        `R2 bucket "${r2Bucket}" does not exist on remote "${r2Remote}". ` +
          ...
          "releases.tide.codes custom domain (see RELEASING.md), then re-run.",
      );
    }
  ...
  const requests = [
    {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "tide-release", version: "1" },
      },
    },
...
try {
  ...
  temporaryDirectory = await mkdtemp(join(tmpdir(), "tide-dmg-"));
  ...
  // archive as Tide-<version>.md; generate_appcast links it as the update's
  ...
  if (publishing) {
    ...
    console.log(`\nTide ${version} (build ${buildNumber}) is live:`);
