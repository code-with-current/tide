...
const appName = "Tide Debug";
const targetDir = resolve(root, process.env.CARGO_TARGET_DIR || "target");
const executableSuffix = process.platform === "win32" ? ".exe" : "";
const appPath = isMacOS
  ? join(targetDir, "debug/Tide Debug.app")
  : join(targetDir, `debug/tide${executableSuffix}`);
const daemonPath = join(
  targetDir,
  `debug/tide-debug-daemon${executableSuffix}`,
);
...
let appSupervisor: ReturnType<typeof setInterval> | undefined;
...
const hyprlandRuleKeys = [
  "tide_dev_workspace_rule",
  "tide_dev_background_rule",
] as const;
const hyprlandSubscriptionKey = "tide_dev_window_open_subscription";
const hyprlandLaunchArmedKey = "tide_dev_launch_armed";
const hyprlandOwnerKey = "tide_dev_owner";
...
async function prepareHyprlandLaunch(): Promise<void> {
  ...
  const code = `
    ...
    if _G[subscription_key] == nil then
      ...
      _G[subscription_key] = hl.on("window.open", function(window)
        if not _G[armed_key] or window.initial_class ~= "codes.tide.dev" then
          return
        end
        _G[armed_key] = false
        ...
        -- Swapping with each preceding singleton column rotates Tide into the
  ...
  if (result.exitCode !== 0) {
    if (!hyprlandWarningShown) {
      ...
      console.warn(
        `[tide-dev] Could not pin Tide to its Hyprland workspace${detail ? `: ${detail}` : "."}`,
      );
  ...
  if (!hyprlandRulesInstalled) {
    console.log(
      `[tide-dev] Keeping Tide beside the watcher on Hyprland workspace ${hyprlandWorkspace.name}.`,
    );
  }
...
async function build(target: BuildTarget): Promise<boolean> {
  ...
  console.log(`[tide-dev] Building ${isMacOS ? "app bundle" : "app"}...`);
  if (!(await buildDaemon())) {
    console.error(
      "[tide-dev] Daemon build failed; keeping the current app open.",
    );
    return false;
  }
  const result = isMacOS
    ? await $`${join(root, "scripts/bundle.sh")} debug`.nothrow()
    : await $`cargo build --package tide --bin tide --bin tide_js_repl`.nothrow();
  if (result.exitCode !== 0) {
    console.error("[tide-dev] Build failed; keeping the current app open.");
    return false;
  }
  return true;
}
...
async function buildDaemon(): Promise<boolean> {
  // The backend is served in-process now; backend changes rebuild the app
  // binary itself, so there is no separate daemon to link.
  return true;
}
...
async function stopApp(): Promise<void> {
  ...
  if (isMacOS) {
    ...
    await Bun.sleep(600);
    for (const name of names) {
      await $`pkill -KILL -x ${name}`.quiet().nothrow();
    }
    await waitForAppExit(names);
...
/// Block until no process answering to `names` remains, so a launch can
/// never race a dying predecessor. Bounded: if SIGKILL could not clear the
/// process, report it and continue — the launcher refuses to fork a rival
/// (no `open -n`), and the app's single-instance lock is the second wall.
async function waitForAppExit(names: string[]): Promise<void> {
  const deadline = Date.now() + 5_000;
  while (Date.now() < deadline) {
    const survivors: string[] = [];
    for (const name of names) {
      const probe = await $`pgrep -x ${name}`.quiet().nothrow();
      if (probe.exitCode === 0) survivors.push(name);
    }
...
function launchApp(): ReturnType<typeof Bun.spawn> {
  console.log(`[tide-dev] Launching ${appPath}`);
  // Plain `open`: no `-n` — a lingering survivor must be activated, never
  // forked into a second window (the app's single-instance lock is the
  // second wall) — and no `-W`, whose wait is bundle-scoped: any other
  // launch of this bundle would satisfy it while the real instance keeps
  // running, and this watcher would then tear a live app down. Instance
  // liveness is supervised by poll instead. The debug bundle id
  // (`codes.tide.dev`) never collides with the installed release app.
  const command = isMacOS ? ["open", appPath] : [appPath];
  ...
  if (isMacOS) {
    ...
    void launchedApp.exited.then(async (exitCode) => {
      ...
      console.log("[tide-dev] App exited; stopping the watcher.");
...
/// Watch the launched instance, not the bundle. `open` returns immediately,
/// so on macOS the watcher learns the app exited by observing that no
/// process answers to its executable name. A deliberate stop (rebuild,
/// cleanup) raises `restartingApp` first so the poll never mistakes its own
/// kill for a crash.
function superviseApp(): void {
  stopAppSupervisor();
  appSupervisor = setInterval(() => {
    void (async () => {
      if (stopping || restartingApp || app === undefined) return;
      const alive = await $`pgrep -x ${appName}`.quiet().nothrow();
      // Re-check after the await: a rebuild may have begun mid-probe.
      if (stopping || restartingApp || app === undefined) return;
...
function stopAppSupervisor(): void {
  if (appSupervisor === undefined) return;
  clearInterval(appSupervisor);
  appSupervisor = undefined;
}
...
function reportWatcherError(error: Error): void {
  console.error("[tide-dev] File watcher failed:", error);
  process.exitCode = 1;
  void cleanup();
}
    ...
    while (queuedBuild !== undefined && !stopping) {
      ...
      if (target === "daemon") {
        if (daemonChangeRevision === buildDaemonRevision) {
          console.log(
            "[tide-dev] Daemon rebuilt; Tide will swap the process without relaunching.",
          );
        }
        continue;
      }
      ...
      if (appChangeRevision !== buildAppRevision) {
        console.log(
          "[tide-dev] More changes arrived during the build; waiting to rebuild.",
        );
        continue;
      }
...
async function cleanup(): Promise<void> {
  if (stopping) return;
  stopping = true;
  console.log("[tide-dev] Stopping watcher and app...");
  closeWatchers();
  clearRebuildTimer();
  await stopApp();
  await releaseHyprlandRules();
...
if (appChangeRevision === initialAppRevision) {
  ...
  console.log(
    "[tide-dev] Changes arrived during the initial build; waiting to rebuild.",
  );
  if (queuedBuild !== undefined) void drainBuildQueue();
}

console.log(
  "[tide-dev] Watching for source changes. Daemon-only edits hot-reload without relaunching Tide.",
);
