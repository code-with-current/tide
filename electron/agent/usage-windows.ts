 ...
 *  (better-sqlite3, WAL) — event volume is one row per turn, so the table
 *  stays tiny; rows older than the longest window (7d + slack) are pruned
 *  on write. */
...
function getDb(): Database.Database {
  ...
  db = new Database(path.join(dir, 'usage.db'));
