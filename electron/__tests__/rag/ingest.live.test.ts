...
vi.mock('../../rag/local-onnx-embedder.js', async (importOriginal) => {
  const actual = (await importOriginal()) as typeof import('../../rag/local-onnx-embedder.js');
...
import { openRagStore } from '../../rag/store.js';
...
describeLive('ingestWorkspace (live)', () => {
  ...
  it(
    ...
    async () => {
      ...
      const { embedder } = await import('../../rag/resolve.js').then((m) =>
        m.resolveForQuery({
