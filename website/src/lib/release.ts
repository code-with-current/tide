...
const RELEASES_BASE = 'https://releases.tide.codes'
...
export const FALLBACK_DOWNLOAD_URL = `${RELEASES_BASE}/Tide-0.0.1.dmg`
...
export function windowsInstallerUrl(version: string, arch: string) {
  return `${RELEASES_BASE}/Tide-${version}-${arch}-Setup.exe`
}
...
const fetchLatestRelease = createServerFn({ method: 'GET' }).handler(
  async (): Promise<LatestRelease | null> => {
    try {
      ...
      return {
        version,
        url: `${RELEASES_BASE}/Tide-${version}.dmg`,
        pubDate,
      }
    } catch {
      return null
