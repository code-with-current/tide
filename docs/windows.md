...
`tide-<version>-<target>.zip` is the same build without an installer. Unpack it
anywhere and run `tide.exe`.

**Keep the two executables together.** Tide launches `tide-daemon.exe` from its
own directory, so moving `tide.exe` out on its own leaves it unable to start
...
Nothing else: Tide links the C runtime statically, so there is no Visual C++
...
Tide updates itself. It checks once per launch, and an available update
appears in the sidebar footer; clicking it downloads the installer, verifies
its signature, and runs it. Tide closes, is replaced in place, and reopens.
...
`%LOCALAPPDATA%\Tide\updater.json`.

## Where Tide keeps its data

| What | Path |
| --- | --- |
| Tasks, sessions, transcripts | `%LOCALAPPDATA%\Tide\app.db` |
| Attachments and blobs | `%LOCALAPPDATA%\Tide\blobs` |
| Settings | `%USERPROFILE%\.tide\app.json` |

Unpacking a new release over the old directory leaves all of it untouched.

## Agent CLIs

Tide detects the provider CLIs on `PATH` and, because a fresh `PATH` may
...
Tide hosts it in *visual* mode rather than as a child window: the page renders
...
- **Pen, touch and dragging files into the page are not wired up.** Visual
  hosting delivers no input of its own; Tide forwards mouse, wheel, cursor and
...
**The window opens black, or the app exits at startup.** Tide needs a working
...
shim on `PATH`. If the shell finds it but Tide does not, set the binary path in
**Settings → Providers** and file an issue with the install method.

**Git-backed features do nothing.** Tide shells out to `git`. Install Git for
