...
export async function fetchDocs(location: string, opts: FetchDocsOptions = {}): Promise<SourceDocument[]> {
  // A missing root (e.g. appDataDir before first boot) allows nothing rather
  // than crashing with ENOENT — the location then cleanly fails the check.
  const roots = (opts.allowedRoots ?? [appDataDir()])
    .map((root) => {
      try {
