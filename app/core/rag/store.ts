/** Per-workspace RAG storage (SQLite + FTS5 + sqlite-vec) at `<userData>/rag/<workspaceId>/index.db`. Sync sqlite writes (via the platform driver seam) wrapped in transactions; bump SCHEMA_VERSION and append a step in `migrate()` to evolve the schema. */
import { openDatabase, type TideDatabase } from '../../platform/sqlite.js';
import { sqliteVecLibraryPath } from '../../platform/native-assets.js';
...
export function openRagStoreAt(dbPath: string): RagStore {
  ...
  // sqlite-vec loads as a SQLite extension; the platform package's binary is
  // staged into the bundle by electrobun.config.ts build.copy and resolved
  // through the native-assets seam (dev falls back to node_modules).
  try {
    db.loadExtension(sqliteVecLibraryPath());
  } catch (e) {
    db.close();
    throw new Error(
...
function migrate(db: TideDatabase): void {
  db.exec(`
...
export class RagStore {
  constructor(private readonly db: TideDatabase) {}
  ...
  upsertChunks(rows: ChunkRow[]): { id: string; rowid: number }[] {
    ...
    const tx = this.db.transaction((rs: ChunkRow[]) => {
      ...
      `);
      for (const r of rs) {
        // RETURNING inside ON CONFLICT upsert works in sqlite ≥ 3.35; both
        // drivers ship a recent sqlite.
        const rowidRow = stmt.get({
          $id: r.id,
          $path: r.path,
          $symbol: r.symbol,
          $content: r.content,
