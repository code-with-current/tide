...
import { stagedTreeSitterDir } from '../../../platform/native-assets.js';
...
function resolveGrammarDir(): string {
  const candidates = [
    stagedTreeSitterDir(), // staged by electrobun.config.ts build.copy (packaged)
    path.join(__dirname, 'grammars'), // vendored next to the chunker (postinstall)
...
/** web-tree-sitter's core wasm (tree-sitter.wasm) defaults to a URL relative
 *  to its own module — inside the merged Electrobun bundle that points at
 *  bun/index.js with no wasm next to it. When the staged dir carries the
 *  core wasm, redirect just that one file; everything else keeps emscripten's
 *  default resolution. Undefined in dev/tests (the package resolves itself). */
function coreWasmModuleOptions(): { locateFile: (p: string) => string } | undefined {
  const staged = stagedTreeSitterDir();
  if (!staged) return undefined;
  const core = path.join(staged, 'tree-sitter.wasm');
  if (!fs.existsSync(core)) return undefined;
  return { locateFile: (p: string) => (p === 'tree-sitter.wasm' ? core : p) };
}

// Resolved on first use, not at module load: the Electrobun bundle pulls
// this module into the boot graph via the rag/sources RPC tiers, and the
// grammar dir may only be missing because packaging hasn't staged it yet —
// that must surface as a per-ingest failure (init 'failed' progress), not
// crash the whole process at import time.
let grammarDirMemo: string | null = null;
function grammarDir(): string {
  if (!grammarDirMemo) grammarDirMemo = resolveGrammarDir();
  return grammarDirMemo;
}
...
async function getParsers(): Promise<Partial<Record<LanguageName, Parser>>> {
  if (!parserPromise) {
    parserPromise = (async () => {
      await Parser.init(coreWasmModuleOptions());
      ...
      const entries = await Promise.all(
        (Object.entries(GRAMMAR_FILES) as [LanguageName, string][]).map(
          async ([lang, file]) => {
            const wasmPath = path.join(grammarDir(), file);
...
export async function chunkFile(absPath: string): Promise<Chunk[]> {
  ...
  do {
    ...
    if (node.type === 'export_statement') {
      // tree-sitter's `children` array is (Node | null)[]
      const inner = node.children.find(
        (c) => c != null && !['export', 'default', '*'].includes(c.type),
      );
...
function extractSymbolName(node: Node): string {
    ...
    for (const child of target.children) {
      if (child?.type === 'variable_declarator') {
        for (const grand of child.children) {
          if (grand != null && NAME_TYPES.has(grand.type)) return grand.text;
        }
      }
    }
  }
  ...
  for (const child of target.children) {
    if (child != null && NAME_TYPES.has(child.type)) {
      return child.text;
    }
  }
