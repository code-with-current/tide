...
// Shell hook: the Bun (Electrobun) main process swaps in an in-process
// onnxruntime embedder (bun-onnx-embedder.ts) — there is no utilityProcess
// child there. The override must keep id/dim/maxTokens = LOCAL_META so
// existing local-built indexes stay query-compatible.
let localEmbedderFactory: () => Embedder = () => localInstance;

/** Override the local-embedder implementation (Bun shell). null restores the
 *  default utilityProcess-backed LocalOnnxEmbedder. */
export function setLocalEmbedderFactory(factory: (() => Embedder) | null): void {
  localEmbedderFactory = factory ?? (() => localInstance);
}
...
export function resolveForBuild(input: ResolveInput): ResolveResult {
  ...
  if (localAvailable) {
    return { embedder: localEmbedderFactory(), embedderId: 'local-code-512' };
  }
